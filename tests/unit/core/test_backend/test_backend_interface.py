...
class TestBackendCapabilities:
    ...
    def test_backend_capability_values(self):
        ...
        assert (
            BackendCapability.ELEMENT_WISE_OPERATIONS.value == "element_wise_operations"
        )
        assert BackendCapability.BITMAP_OPERATIONS.value == "bitmap_operations"
        assert (
            BackendCapability.RANDOM_NUMBER_GENERATION.value
...
class TestBackendSelection:
    ...
    def test_determine_best_backend_cuda(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "feagi.core.backend.interface._BACKENDS",
            {
                ...
                BackendType.CUDA: MockCUDABackend,
        ...
        monkeypatch.setattr(
            "feagi.core.resource_mgr.ResourceManager", MockResourceManager
        )
    ...
    def test_determine_best_backend_cpu(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "feagi.core.backend.interface._BACKENDS", {BackendType.CPU: MockCPUBackend}
        )
...
class TestCPUBackend:
    ...
    def test_cpu_backend_capabilities(self):
        ...
        assert (
            backend.supports_capability(BackendCapability.MATRIX_MULTIPLICATION) is True
        )
        assert (
            backend.supports_capability(BackendCapability.ELEMENT_WISE_OPERATIONS)
            is True
        )
        assert backend.supports_capability(BackendCapability.BITMAP_OPERATIONS) is True
        assert (
            backend.supports_capability(BackendCapability.RANDOM_NUMBER_GENERATION)
            is True
        )
...
class TestBackendInstantiation:
    ...
    def setup(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "feagi.core.backend.interface._BACKENDS", {BackendType.CPU: CPUBackend}
        )
