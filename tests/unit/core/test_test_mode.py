...
def test_runner(mock_core_api):
    ...
    with patch("feagi.core.state_manager.FeagiStateManager") as mock_state_manager_cls:
        ...
        runner = FeagiTestRunner(
            core_api_service=mock_core_api, test_duration=2, frequency_hz=5
        )
...
class TestFeagiTestRunner:
    ...
    def test_init_sensory_data_generator(
        self, test_runner, mock_core_api, mock_activity_generator
    ):
    ...
    def test_inject_sensory_data(
        self, mock_bitmap_class, test_runner, mock_activity_generator
    ):
        ...
        area.get_neurons_at_position.assert_has_calls(
            [call((1, 1, 0)), call((2, 2, 0))], any_order=True
        )
...
class TestRunTestMode:
    ...
    @patch("feagi.test_mode.FeagiTestRunner")
    def test_run_test_mode_with_visualization(
        self, mock_test_runner_cls, mock_core_api
    ):
