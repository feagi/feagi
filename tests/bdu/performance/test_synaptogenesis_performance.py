...
def genome_file():
    ...
    essential_genome_path = os.path.join(project_root, "feagi_core/feagi/evo/defaults/genome/essential_genome.json")
    ...
    with tempfile.NamedTemporaryFile(suffix='.json', delete=False, mode='w') as temp:
        ...
        if os.path.exists(essential_genome_path):
            ...
            print(f"Warning: Essential genome not found at {essential_genome_path}, using minimal test genome")
            genome = {
                "version": "1.0",
                "neuron_morphologies": {
                    "default": {
...
def test_synaptogenesis_performance(embryo, genome_file):
    ...
    print(f"Created {total_neurons} neurons across {len(embryo.cortical_areas)} cortical areas")
    ...
    if synapse_count > 0:
        ...
        print(f"Average time per synapse: {synapse_time * 1000000 / synapse_count:.2f} μs")
    ...
    for func, (calls, _, cum_time, _, _) in list(stats.stats.items())[:10]:
        ...
        print("{:<40} {:>10d} {:>10.6f} {:>10.6f}".format(
