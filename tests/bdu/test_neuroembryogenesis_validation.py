...
def test_genome_file():
    ...
    test_genome_path = os.path.join(
        ...
        "neuroembryogenesis_test_genome.json"
    )
...
def test_extract_cortical_properties(embryo, test_genome_file):
    ...
    assert "bbx" in props, f"Missing bounding box x dimension for cortical ID {cortical_id}"
    assert "bby" in props, f"Missing bounding box y dimension for cortical ID {cortical_id}"
    assert "bbz" in props, f"Missing bounding box z dimension for cortical ID {cortical_id}"
    ...
    name_key = [k for k in blueprint.keys() if k.endswith(f"-{cortical_id}-cx-__name-t") or k.endswith(f"-{cortical_id}-cx-__name-t")][0]
    expected_name = blueprint.get(name_key, "")
    assert props["name"] == expected_name, f"Extracted name '{props['name']}' doesn't match expected '{expected_name}'"
    
    # Check numeric properties
    bbx_key = [k for k in blueprint.keys() if k.endswith(f"-{cortical_id}-cx-___bbx-i") or k.endswith(f"-{cortical_id}-cx-___bbx-i")][0]
...
def test_full_development_validation(embryo, test_genome_file):
    ...
    for cortical_id, area_id in embryo.reverse_cortical_id_map.items():
        ...
        actual_area_neurons = len(embryo.connectome_manager.get_neurons_by_area(area_id))
    ...
    assert DevelopmentStage.FAILED not in stages_logged, "Development failed according to progress logs"
...
def test_morphology_handling(embryo, test_genome_file):
    ...
    for morphology_id, morphology in morphologies.items():
        ...
        assert "parameters" in morphology, f"Morphology {morphology_id} missing parameters"
