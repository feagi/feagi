...
class TestMacM4GPUAcceleration:
    ...
    def test_feagi_array_backend_uses_mps(self):
        ...
        try:
            ...
            assert backend.backend_type == BackendType.PYTORCH, f"Expected PyTorch backend, got {backend.backend_type.value}"
            ...
            assert isinstance(test_array, torch.Tensor), f"Expected torch.Tensor, got {type(test_array)}"
            assert 'mps' in str(test_array.device), f"Expected MPS device, tensor is on {test_array.device}"
    ...
    def test_array_backend_device_selection_priority(self):
        ...
        try:
            ...
            if torch.cuda.is_available():
                ...
                assert backend.device == 'mps', "Should use MPS if CUDA not available but MPS is"
            else:
                assert backend.device == 'cpu', "Should fall back to CPU if neither CUDA nor MPS available"
