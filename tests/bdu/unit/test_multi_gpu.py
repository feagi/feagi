...
class TestMultiGPUConfig:
    ...
    def test_initialization(self):
        ...
        config = MultiGPUConfig(
            partition_method="balanced", sync_mode="batch", backend_type="numpy"
        )
        assert config.partition_method == PartitionMethod.BALANCED
        assert config.sync_mode == SyncMode.BATCH
...
@pytest.mark.skipif(
    not os.environ.get("FEAGI_TEST_MULTI_GPU"),
        ...
        with patch("feagi.bdu.multi_gpu.TORCH_AVAILABLE", True):
            with patch("feagi.bdu.multi_gpu.dist.is_initialized", return_value=False):
                with patch("feagi.bdu.multi_gpu.dist.init_process_group") as mock_init:
                    with patch(
                        "feagi.bdu.multi_gpu.torch.cuda.is_available", return_value=True
                    ):
                        with patch(
                            "feagi.bdu.multi_gpu.torch.cuda.device_count",
                            return_value=2,
                        ):
                            # Create config with PyTorch backend
                            config = MultiGPUConfig(
                                enabled=True, backend_type=BackendType.PYTORCH
                            )
    ...
    def test_synchronization(self):
        ...
        with patch("feagi.bdu.multi_gpu.TORCH_AVAILABLE", True):
            with patch(
                ...
                with patch(
                    "feagi.bdu.multi_gpu.torch.cuda.device_count", return_value=2
                ):
                    with patch(
                        "feagi.bdu.multi_gpu.torch.cuda.synchronize"
                    ) as mock_sync:
                        # Create config with PyTorch backend
                        config = MultiGPUConfig(
                            enabled=True,
