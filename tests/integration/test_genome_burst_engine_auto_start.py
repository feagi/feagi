...
class TestGenomeBurstEngineAutoStart:
    ...
    def test_genome_service_emits_genome_loaded_event(self, genome_service, event_system):
        ...
        with patch('feagi.bdu.embryogenesis.neuroembryogenesis.NeuroEmbryogenesis') as mock_neuro:
            mock_instance = Mock()
            ...
            with patch('feagi.api.core.services.brain.brain_service.BrainService') as mock_brain_service:
                mock_brain_service.return_value.start_burst_engine.return_value = True
                
                # Mock the burst engine update
                with patch('feagi.npu.burst_engine.BurstEngine.get_instance') as mock_get_instance:
    ...
    def test_process_manager_handles_genome_loaded_event(self, mock_connectome_manager, event_system):
        ...
        with patch('feagi.process_manager.FeagiStateManager') as mock_state_manager_class:
            mock_state_manager = Mock()
            mock_state_manager.get_burst_engine_state.return_value = ServiceState.UNINITIALIZED
            mock_state_manager_class.instance.return_value = mock_state_manager
            
            with patch('feagi.api.core.services.brain.brain_service.BrainService') as mock_brain_service_class:
                mock_brain_service = Mock()
                mock_brain_service.start_burst_engine.return_value = True
                mock_brain_service_class.return_value = mock_brain_service
                
                # Mock the ConnectomeManager.instance() call in ProcessManager
                with patch('feagi.bdu.connectome_manager.ConnectomeManager.instance') as mock_cm_instance:
                    ...
                    event_system.register_handler(EventType.GENOME_LOADED, process_manager._handle_genome_loaded_event)
                    event_system.start()
    ...
    def test_process_manager_skips_start_if_burst_engine_already_running(self, mock_connectome_manager, event_system):
        """Test that process manager doesn't start burst engine if already running."""
        with patch('feagi.process_manager.FeagiStateManager') as mock_state_manager_class:
            mock_state_manager = Mock()
            mock_state_manager.get_burst_engine_state.return_value = ServiceState.READY  # Already running
            mock_state_manager_class.instance.return_value = mock_state_manager
        ...
        with patch('feagi.bdu.embryogenesis.neuroembryogenesis.NeuroEmbryogenesis') as mock_neuro:
            ...
            with patch('feagi.api.core.services.brain.brain_service.BrainService'):
                with patch('feagi.npu.burst_engine.BurstEngine.get_instance') as mock_get_instance:
                    ...
                    broken_event_system.send_event.side_effect = Exception("Event system broken")
