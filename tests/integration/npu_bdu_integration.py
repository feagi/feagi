...
def test_fcl_manager_with_standard_connectome(test_brain_structure):
    ...
    assert any(neuron_id in fired_neurons[2] or neuron_id in fired_neurons[3] 
...
def test_converting_standard_to_gpu_maintains_fcl(test_brain_structure):
    ...
    fcl_before = {
        ...
        "events": {idx: std_cm.fcl_manager.get_fcl(idx) for idx in range(std_cm.fcl_manager.window_size)}
    }
    ...
    fcl_after = {
        ...
        "events": {idx: gpu_cm.fcl_manager.get_fcl(idx) for idx in range(gpu_cm.fcl_manager.window_size)}
    }
...
def test_performance_comparison():
    ...
    for pre, post, weight in synapse_specs:
        ...
        gpu_synapse_specs.append((gpu_neuron_ids[pre_idx], gpu_neuron_ids[post_idx], weight))
