...
class NeuronCoordinateExtractionProfiler:
    ...
    def setup_test_connectome(self, neuron_count: int = 100000) -> Dict[str, Any]:
        ...
        areas_per_dimension = max(1, int(np.cbrt(neuron_count / 1000)))  # ~1000 neurons per area
        ...
        for x in range(areas_per_dimension):
            for y in range(areas_per_dimension):
                for z in range(areas_per_dimension):
                    ...
                    area_position = (x * (area_size + 1), y * (area_size + 1), z * (area_size + 1))
                    ...
                    for nx in range(area_size):
                        for ny in range(area_size):
                            for nz in range(area_size):
                                ...
                                neuron_positions.append((nx, ny, nz))  # Relative coordinates within area
                            if len(neuron_positions) >= neurons_in_area:
                                break
                        if len(neuron_positions) >= neurons_in_area:
                            break
        ...
        return {
            ...
            'neurons_per_second_setup': created_neurons / setup_time if setup_time > 0 else 0
        }
    
    def profile_coordinate_extraction_methods(self, neuron_ids: List[int]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if dict_result:
                ...
                results['methods']['dictionary_api'] = {
                    ...
                    'neurons_per_second': valid_count / dict_time if dict_time > 0 else 0,
                    'valid_neurons': valid_count,
                    'memory_overhead_mb': self._estimate_dict_memory_overhead(dict_result),
                ...
                results['methods']['dictionary_api'] = {'success': False, 'error': 'No result returned'}
        ...
        try:
            ...
            if numpy_result is not None:
                ...
                results['methods']['numpy_api'] = {'success': False, 'error': 'No result returned'}
        ...
        try:
            benchmark_result = self.core_api.benchmark_neuron_coordinate_extraction(batch_size)
            if 'error' not in benchmark_result:
                results['methods']['benchmark_method'] = {
                    ...
                    'performance_comparison': benchmark_result.get('performance_comparison', {}),
    ...
    def _validate_coordinate_consistency(self, results: Dict[str, Any]) -> None:
        ...
        if dict_coords and numpy_coords is not None:
            try:
                ...
                validation['coordinate_count_match'] = len(valid_dict_coords) == len(numpy_coord_tuples)
                
                # Compare actual coordinate values (with tolerance for floating point)
                if len(valid_dict_coords) == len(numpy_coord_tuples) and len(valid_dict_coords) > 0:
                    coord_differences = []
                    for dict_coord, numpy_coord in zip(valid_dict_coords, numpy_coord_tuples):
                        diff = np.linalg.norm(np.array(dict_coord) - np.array(numpy_coord))
                    ...
                    validation['coordinates_consistent'] = max_diff < 0.001  # 1mm tolerance
                else:
    ...
    def _generate_performance_recommendations(self, results: Dict[str, Any]) -> None:
        ...
        if dict_success and numpy_success:
            ...
            if numpy_perf > dict_perf * 1.5:
                recommendations.append(f"NumPy API is {numpy_perf/dict_perf:.1f}x faster - recommend for batch size {batch_size}")
            elif dict_perf > numpy_perf * 1.2:
                recommendations.append(f"Dictionary API unexpectedly faster - investigate NumPy overhead for batch size {batch_size}")
            else:
                recommendations.append(f"Performance similar for batch size {batch_size} - choose based on API preference")
        ...
        for method_name, method_data in methods.items():
            if method_data.get('success') and 'neurons_per_second' in method_data:
                ...
                if perf < self.performance_thresholds['minimum_neurons_per_second']:
                    recommendations.append(f"{method_name}: Performance below minimum threshold ({perf:.0f} < {self.performance_thresholds['minimum_neurons_per_second']} neurons/sec)")
                elif perf >= self.performance_thresholds['optimal_neurons_per_second']:
                    recommendations.append(f"{method_name}: Optimal performance achieved ({perf:.0f} neurons/sec)")
                elif perf >= self.performance_thresholds['target_neurons_per_second']:
                    recommendations.append(f"{method_name}: Good performance ({perf:.0f} neurons/sec)")
        ...
        if benchmark_data.get('success'):
            ...
            if simd_analysis:
                ...
                if utilization < 50:
                    recommendations.append(f"Low SIMD utilization ({utilization:.1f}%) - consider larger batch sizes")
                elif utilization > 80:
                    recommendations.append(f"Excellent SIMD utilization ({utilization:.1f}%)")
    ...
    def run_batch_size_scaling_analysis(self) -> Dict[str, Any]:
        ...
        for batch_size in self.test_batch_sizes:
            self.logger.info(f"Testing coordinate extraction with batch size: {batch_size}")
            ...
            if hasattr(self.connectome_manager, 'neuron_array'):
                ...
                if max_neurons < batch_size:
                    self.logger.warning(f"Insufficient neurons ({max_neurons}) for batch size {batch_size}, skipping")
                    continue
                    
                # Create representative sample of neuron IDs
                neuron_ids = list(range(0, min(batch_size, max_neurons), max(1, max_neurons // batch_size)))
                if len(neuron_ids) < batch_size:
                    # Fill remaining with random valid IDs
                    remaining = batch_size - len(neuron_ids)
                    random_ids = np.random.choice(max_neurons, size=remaining, replace=False).tolist()
    ...
    def _analyze_scaling_characteristics(self, scaling_results: Dict[str, Any]) -> None:
            ...
            for method_name, method_data in batch_result.get('methods', {}).items():
                if method_data.get('success') and 'neurons_per_second' in method_data:
                    if method_name not in methods_performance:
                        methods_performance[method_name] = {'batch_sizes': [], 'neurons_per_second': []}
                    
                    methods_performance[method_name]['batch_sizes'].append(batch_size)
                    methods_performance[method_name]['neurons_per_second'].append(method_data['neurons_per_second'])
        ...
        for method_name, perf_data in methods_performance.items():
            ...
            if len(batch_sizes) >= 2:
                ...
                scaling_efficiency = self._calculate_scaling_efficiency(batch_sizes, performance)
                
                # Identify performance dropoff points
                dropoff_points = self._identify_performance_dropoffs(batch_sizes, performance)
                ...
                scaling_analysis[method_name] = {
                    ...
                    'scaling_trend': self._categorize_scaling_trend(batch_sizes, performance)
                }
        ...
        if scaling_analysis:
            ...
            if 'numpy_api' in scaling_analysis:
                scaling_results['optimal_batch_size'] = scaling_analysis['numpy_api']['optimal_batch_size']
            else:
                # Find method with highest optimal performance
                best_method = max(scaling_analysis.items(), key=lambda x: x[1]['optimal_performance'])
                scaling_results['optimal_batch_size'] = best_method[1]['optimal_batch_size']
    
    def _calculate_scaling_efficiency(self, batch_sizes: np.ndarray, performance: np.ndarray) -> float:
    ...
    def _identify_performance_dropoffs(self, batch_sizes: np.ndarray, performance: np.ndarray) -> List[int]:
    ...
    def _categorize_scaling_trend(self, batch_sizes: np.ndarray, performance: np.ndarray) -> str:
    ...
    def save_profiling_results(self, results: Dict[str, Any], output_path: Optional[Path] = None) -> Path:
        """
        ...
        if output_path is None:
            ...
            output_path = output_dir / f"neuron_coordinate_extraction_profile_{timestamp}.json"
...
def test_coordinate_extraction_setup(coordinate_profiler):
    ...
    assert setup_results['neurons_per_second_setup'] > 0, "Should have positive setup rate"
    
    # Performance expectations for setup
    assert setup_results['neurons_per_second_setup'] > 1000, "Setup should be reasonably fast (>1000 neurons/sec)"
    
    print(f"✅ Connectome setup: {setup_results['neuron_count']} neurons in {setup_results['setup_time_ms']:.1f}ms")
...
def test_small_batch_coordinate_extraction(coordinate_profiler):
    ...
    for batch_size in small_batch_sizes:
        ...
        assert len(successful_methods) > 0, f"At least one method should succeed for batch size {batch_size}"
        ...
        for method_name in successful_methods:
            ...
            if 'neurons_per_second' in method_data:
                assert method_data['neurons_per_second'] > 0, f"{method_name} should have positive performance"
                print(f"✅ {method_name} (batch {batch_size}): {method_data['neurons_per_second']:.0f} neurons/sec")
...
def test_large_batch_coordinate_extraction(coordinate_profiler):
    ...
    assert setup_results['neuron_count'] >= 50000, "Need sufficient neurons for large batch testing"
    ...
    for batch_size in large_batch_sizes:
        ...
        if numpy_method.get('success'):
            ...
            assert numpy_performance > 1000, f"NumPy method should handle large batches efficiently (got {numpy_performance:.0f} neurons/sec)"
            print(f"✅ NumPy API (batch {batch_size}): {numpy_performance:.0f} neurons/sec")
            
            # Memory efficiency check
            if 'memory_mb' in numpy_method:
                memory_per_neuron = numpy_method['memory_mb'] / len(neuron_ids) * 1024 * 1024  # bytes per neuron
                assert memory_per_neuron < 100, f"Memory usage should be efficient (<100 bytes/neuron, got {memory_per_neuron:.1f})"
...
def test_coordinate_extraction_accuracy(coordinate_profiler):
    ...
    if validation.get('coordinates_consistent') is not None:
        assert validation['coordinates_consistent'], "Dictionary and NumPy methods should return consistent coordinates"
        ...
        if 'max_coordinate_difference' in validation:
            ...
            assert max_diff < 0.001, f"Coordinate differences should be minimal (got {max_diff})"
    ...
    if benchmark_method.get('success'):
        ...
        if simd_config.get('available') and simd_analysis:
            ...
            if 'actual_speedup' in simd_analysis:
                ...
                assert speedup >= 1.0, f"SIMD should provide speedup >= 1.0x (got {speedup:.2f}x)"
...
def test_performance_scaling_analysis(coordinate_profiler):
    ...
    assert tested_batches >= 3, f"Should test multiple batch sizes (tested {tested_batches})"
    ...
    for method_name, analysis in scaling_analysis.items():
        ...
        print(f"   Optimal performance: {analysis['optimal_performance']:.0f} neurons/sec")
        print(f"   Scaling trend: {analysis['scaling_trend']}")
        print(f"   Scaling efficiency: {analysis['scaling_efficiency']:.2f}")
        
        # Validate analysis results
        assert analysis['optimal_batch_size'] > 0, "Should have valid optimal batch size"
        assert analysis['optimal_performance'] > 0, "Should have positive optimal performance"
        assert analysis['scaling_efficiency'] >= 0, "Should have non-negative scaling efficiency"
...
def test_coordinate_extraction_benchmark_suite(coordinate_profiler):
    ...
    for method_name, analysis in scaling_analysis.items():
        ...
        print(f"  • Peak performance: {analysis['optimal_performance']:,.0f} neurons/sec")
        print(f"  • Scaling trend: {analysis['scaling_trend']}")
    ...
    min_expected_performance = coordinate_profiler.performance_thresholds['minimum_neurons_per_second']
