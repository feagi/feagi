...
class FeagiModeComparator:
    ...
    def run_mode_test(self, mode_name: str, embedded: bool = False) -> Dict[str, Any]:
        ...
        while time.time() - start_time < self.runtime_seconds + 10:  # Extra time for startup
            try:
        ...
        if snapshots:
            memory_values = [s['memory_mb'] for s in snapshots if s['elapsed'] > 5]  # Skip startup
            ...
            if memory_values:
                stats = {
                    ...
                    'threads': {
                        'avg_count': sum(thread_values) / len(thread_values) if thread_values else 0,
    ...
    def compare_modes(self) -> Dict[str, Any]:
        ...
        if ('memory' in normal_stats and 'memory' in embedded_stats and
            'error' not in normal_stats and 'error' not in embedded_stats):
            
            normal_mem = normal_stats['memory']['avg_mb']
            embedded_mem = embedded_stats['memory']['avg_mb']
            memory_savings = normal_mem - embedded_mem
            memory_savings_percent = (memory_savings / normal_mem) * 100 if normal_mem > 0 else 0
            ...
            logger.info(f"   CPU: {normal_cpu:.1f}% → {embedded_cpu:.1f}% "
                       f"({cpu_savings:+.1f}%)")
            logger.info(f"   Threads: {normal_threads:.1f} → {embedded_threads:.1f} "
                       f"({thread_savings:+.1f})")
...
def test_embedded_mode_comparison(genome_path, logs_dir):
    ...
    try:
        ...
        if 'improvements' in results:
            ...
            any_improvement = (summary['memory_improved'] or 
                             summary['cpu_improved'] or 
