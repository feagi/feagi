...
def run_profiling_tests():
    ...
    if result.returncode == 0:
        ...
        result_files = list(logs_dir.glob("neuron_coordinate_extraction_profile_*.json"))
        ...
        if result_files:
            ...
            try:
                ...
                if 'setup_results' in data:
                    ...
                    print(f"Setup rate: {setup['neurons_per_second_setup']:,.0f} neurons/sec")
                    print()
                ...
                if 'scaling_analysis' in data:
                    ...
                    for method, analysis in data['scaling_analysis'].items():
                        print(f"  • {method}: {analysis['optimal_performance']:,.0f} neurons/sec")
                        print(f"    (optimal batch size: {analysis['optimal_batch_size']:,})")
                    print()
                
                # Recommendation
                if 'optimal_batch_size' in data:
                    print(f"🎯 Recommended batch size: {data['optimal_batch_size']:,} neurons")
...
if __name__ == "__main__":
    ...
    if success:
        ...
        print("  python -m pytest tests/profiling/test_neuron_coordinate_extraction.py -v")
        print()
        print("  # Run just the comprehensive benchmark:")
        print("  python -m pytest tests/profiling/test_neuron_coordinate_extraction.py::test_coordinate_extraction_benchmark_suite -v")
        print()
        print("  # Run quick small batch test:")
        print("  python -m pytest tests/profiling/test_neuron_coordinate_extraction.py::test_small_batch_coordinate_extraction -v")
    else:
