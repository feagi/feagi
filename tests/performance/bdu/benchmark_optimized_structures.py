...
try:
    ...
    logging.warning(
        "Optimized structures not available. Only benchmarking standard implementations."
    )
...
def benchmark_fcl(neuron_count, iterations=10):
    ...
    for i in range(iterations):
        # Add random neurons
        neurons_to_add = [
            random.randint(0, neuron_count - 1) for _ in range(neuron_count // 10)
        ]
...
def benchmark_connectome(neuron_count, connection_density=0.01, iterations=10):
    ...
    print(
        f"Created connectome with {standard_connectome.connection_count()} connections"
    )
...
def main():
    ...
    parser.add_argument(
        "--neuron-count", type=int, default=10000, help="Number of neurons"
    )
    parser.add_argument(
        "--connection-density", type=float, default=0.001, help="Connection density"
    )
    parser.add_argument(
