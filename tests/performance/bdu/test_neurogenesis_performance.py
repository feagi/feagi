...
class NeurogenesisPerformanceBenchmark:
    ...
    def _get_genome_file(
        self, modify_func: Optional[Callable[[Dict], Dict]] = None
    ) -> str:
        """
    ...
    def _create_test_area_genome(
        self, dimensions: Tuple[int, int, int], neurons_per_voxel: int
    ) -> Dict:
        """
        ...
        def modifier(genome: Dict) -> Dict:
            ...
            total_neurons = (
                dimensions[0] * dimensions[1] * dimensions[2] * neurons_per_voxel
            )
    ...
    def _get_config(self, backend_type: BackendType) -> FeagiConfig:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            config.set(
                "connectome.max_neurons", 100000
            )  # Use a reasonable max neuron count
    ...
    def _run_benchmark_for_backend(
        ...
        modifier_func: Callable[[Dict], Dict],
        description: str,
    ) -> Dict:
        """
        ...
        """
        logger.info(
            f"Running benchmark for {backend_type.value} backend with {description}..."
        )
        ...
        try:
            ...
            if "benchmark_settings" in embryo.genome:
                test_dims = embryo.genome["benchmark_settings"].get(
                    "test_area_dimensions"
                )
                test_neurons_per_voxel = embryo.genome["benchmark_settings"].get(
                    "test_area_neurons_per_voxel"
                )
                ...
                if test_dims and test_neurons_per_voxel:
                    ...
                    area = connectome_manager.add_cortical_area(
                        ...
                        position=(0, 0, 0),
                        properties={"neurons_per_voxel": test_neurons_per_voxel},
                    )
                    ...
                    embryo._extract_cortical_properties_cache = getattr(
                        embryo, "_extract_cortical_properties_cache", {}
                    )
                    ...
                    def patched_extract_cortical_properties(cortical_id):
                        if cortical_id == "benchmark-test-area" and hasattr(
                            embryo, "_extract_cortical_properties_cache"
                        ):
                            return embryo._extract_cortical_properties_cache.get(
                                cortical_id, {}
                            )
                        return original_extract_method(cortical_id)

                    embryo._extract_cortical_properties = (
                        patched_extract_cortical_properties
                    )

                    logger.info(
                        f"Added benchmark test area with dimensions {test_dims} and {test_neurons_per_voxel} neurons per voxel"
                    )
            ...
            for area_id, area in embryo.cortical_areas.items():
                ...
                area_stats[area.name] = {
                    ...
                    "neurons_per_voxel": (
                        neuron_count / voxel_count if voxel_count > 0 else 0
                    ),
                }
                ...
                if area_id == test_area_id:
                    expected_neurons = (
                        ...
                        * dimensions[1]
                        * dimensions[2]
                        * test_neurons_per_voxel
                    )
                    if neuron_count != expected_neurons:
                        logger.warning(
                            f"Expected {expected_neurons} neurons in test area, but got {neuron_count}"
                        )
            ...
            neurons_per_second = (
                total_neurons / neurogenesis_time if neurogenesis_time > 0 else 0
            )
            ...
            benchmark_results = {
                ...
                "neurons_per_voxel_avg": (
                    total_neurons / total_voxels if total_voxels > 0 else 0
                ),
            ...
            logger.info(
                f"Benchmark for {backend_type.value} with {description} completed: "
            ...
            logger.error(
                f"Error in benchmark for {backend_type.value} with {description}: {str(e)}"
            )
            import traceback
    ...
    def run_neuron_count_scaling(self) -> Dict:
        ...
        for count in self.neuron_count_scenario:
            ...
            for backend_type in available_backends:
                ...
                result = self._run_benchmark_for_backend(
                    backend_type, modifier, description
                )
    ...
    def run_all_backends(self) -> Dict:
        ...
        available_backends = get_available_backends()
        logger.info(
            f"Found {len(available_backends)} available backends: {[b.value for b in available_backends]}"
        )
    ...
    def print_summary(self) -> None:
        ...
        if "dimension_scaling" in self.results:
            ...
            print("-" * 40)
            for dimensions, backend_results in self.results[
                "dimension_scaling"
            ].items():
    ...
    def _print_result_summary(
        self, backend: str, result: Dict, indent: str = ""
    ) -> None:
        ...
        if result["status"] == "success":
            ...
            print(
                f"{indent}Neurogenesis time: {result['neurogenesis_time']:.3f} seconds"
            )
            print(
                f"{indent}Performance: {result['neurons_per_second']:.1f} neurons/second"
            )
    ...
    def print_system_specs(self):
        ...
        print(
            f"OS: {specs.get('os')} {specs.get('os_version')} {specs.get('os_release')}"
        )
        print(f"Python: {specs.get('python_version')}")
        print(f"Processor: {specs.get('processor')}")
        print(
            f"CPU Cores: {specs.get('cpu_count')} (Physical), {specs.get('cpu_count_logical')} (Logical)"
        )
        ...
        if "gpu_info" in specs:
            ...
            for i, (name, memory) in enumerate(
                zip(gpu_info.get("gpu_names", []), gpu_info.get("gpu_memory", []))
            ):
...
def main():
    ...
    parser.add_argument(
        "--log-dir",
        default=None,
        help="Directory to store logs (defaults to tests/performance/logs)",
    )
    parser.add_argument(
        "--neuron-count-only",
        action="store_true",
        help="Run only neuron count scaling tests",
    )
    parser.add_argument(
        "--dimension-only", action="store_true", help="Run only dimension scaling tests"
    )
    parser.add_argument(
        "--essential-only", action="store_true", help="Run only essential genome tests"
    )
    parser.add_argument(
        "--quick", action="store_true", help="Run a quick benchmark with minimal tests"
    )
    ...
    if args.quick:
        benchmark.neuron_count_scenario = [
            10,
            100,
            1000,
        ]  # Only test smaller neuron counts
        benchmark.dimension_scenario = [
            (1, 1, 1),
            (10, 10, 10),
        ]  # Only test smaller dimensions
