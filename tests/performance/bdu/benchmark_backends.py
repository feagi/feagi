...
def setup_benchmark_data(config: BenchmarkConfig, neuron_count: int, backend: ArrayBackend) -> Dict[str, Any]:
    ...
    try:
        ...
        sparse_mat = backend.sparse_csr(weights, post_neurons, 
                                      np.concatenate([[0], np.cumsum(np.bincount(pre_neurons, minlength=neuron_count))]), 
...
def benchmark_neuron_update(config: BenchmarkConfig, data: Dict[str, Any], backend: ArrayBackend) -> float:
...
def benchmark_synapse_propagation(config: BenchmarkConfig, data: Dict[str, Any], backend: ArrayBackend) -> float:
    ...
    if use_csr:
        ...
        if hasattr(synapses, "toarray"):  # scipy.sparse
            ...
            data_np = backend.to_numpy(synapses.data if hasattr(synapses, "data") else backend.to_numpy(data["synapse_weights"]))
            indices_np = backend.to_numpy(synapses.indices if hasattr(synapses, "indices") else backend.to_numpy(data["synapse_post"]))
            indptr_np = backend.to_numpy(synapses.indptr if hasattr(synapses, "indptr") else None)
...
def run_benchmarks(config: BenchmarkConfig) -> Dict[str, Dict[str, List[float]]]:
    ...
    for neuron_count in config.neuron_counts:
        ...
        for backend_type in BACKENDS_TO_TEST:
            ...
            if backend_type in [BackendType.CUPY, BackendType.WEBGPU] and neuron_count > 1_000_000:
                logger.info(f"    Skipping {backend_type.value} for {neuron_count} neurons (too large)")
            ...
            try:
                ...
                logger.error(f"    Error benchmarking {backend_type.value} with {neuron_count} neurons: {e}")
                results["neuron_update"][backend_type.value].append(float('nan'))
                results["synapse_propagation"][backend_type.value].append(float('nan'))
...
def main():
    ...
    parser.add_argument("--neuron-counts", type=int, nargs="+", default=[1000, 10000, 100000, 1000000],
                      help="List of neuron counts to benchmark")
    parser.add_argument("--synapse-density", type=float, default=0.01,
                      help="Fraction of possible connections that exist")
    parser.add_argument("--firing-rate", type=float, default=0.01,
