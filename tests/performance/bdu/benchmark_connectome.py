...
class ConnectomeBenchmark:
    ...
    def run_benchmark(self, max_neurons=1_000_000, n_areas=10, n_neurons_per_area=10000,
                     connectivity_density=0.01, n_signals=10000):
        ...
        if total_neurons > max_neurons:
            logger.warning(f"Total neurons ({total_neurons}) exceeds max_neurons ({max_neurons}). Adjusting parameters.")
            n_neurons_per_area = max_neurons // n_areas
            total_neurons = n_areas * n_neurons_per_area
        
        logger.info(f"Running benchmark with {n_areas} areas, {n_neurons_per_area} neurons per area "
                   f"({total_neurons} total neurons), and {connectivity_density:.1%} connectivity")
        
        # Run benchmarks for each implementation
        self._benchmark_impl("standard", max_neurons, n_areas, n_neurons_per_area, 
                           connectivity_density, n_signals)
        self._benchmark_impl("gpu_optimized", max_neurons, n_areas, n_neurons_per_area, 
                           connectivity_density, n_signals)
    ...
    def _benchmark_impl(self, impl_name, max_neurons, n_areas, n_neurons_per_area, 
                      connectivity_density, n_signals):
        ...
        if n_neurons_per_area > 1000:
            ...
            sample_size = 1000
            logger.info(f"Using sample of {sample_size} neurons for individual creation benchmark")
        else:
        ...
        self.results[impl_name]["create_neurons"] = (time.time() - start_time) * (n_neurons_per_area / sample_size)
        ...
        self.results[impl_name]["create_synapses"] = (time.time() - start_time) * (n_synapses / min(100, n_synapses))
        ...
        for neuron_id in active_neurons:
            manager.set_neuron_property(neuron_id, "membrane_potential", 2.0)  # Above threshold
    ...
    def _generate_plots(self):
        ...
        for op_name, _ in operations:
            if self.results["standard"][op_name] > 0:
                speedup = self.results["standard"][op_name] / self.results["gpu_optimized"][op_name]
        ...
        for op in update_ops:
            if self.results["standard"][op] > 0:
                speedup = self.results["standard"][op] / self.results["gpu_optimized"][op]
    ...
    def _save_results(self):
        ...
        with open(os.path.join(self.output_dir, 'connectome_benchmark_results.json'), 'w') as f:
...
def main():
    ...
    parser.add_argument('--max-neurons', type=int, default=1_000_000,
                      help='Maximum number of neurons in the connectome')
    parser.add_argument('--n-areas', type=int, default=5,
                      help='Number of cortical areas to create')
    parser.add_argument('--n-neurons-per-area', type=int, default=10000,
                      help='Number of neurons per area')
    parser.add_argument('--connectivity-density', type=float, default=0.01,
                      help='Connectivity density (0-1)')
    parser.add_argument('--n-signals', type=int, default=1000,
                      help='Number of signals to propagate')
