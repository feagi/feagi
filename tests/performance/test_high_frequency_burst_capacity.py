...
class HighFrequencyBurstCapacityTest:
    ...
    def setup_test_brain(self, neuron_count: int) -> Tuple[ConnectomeManager, BurstEngine]:
        """Set up a test brain with specified neuron count optimized for 10M target."""
        logger.info(f"Setting up test brain with {neuron_count:,} neurons (target: {self.target_operations_per_burst:,})")
        ...
        logger.info(f"Creating {areas_count} areas with ~{neurons_per_area:,} neurons each")
        ...
        for area_idx in range(areas_count):
            ...
            if hasattr(connectome, 'batch_create_neurons_vectorized'):
                area_neuron_ids = connectome.batch_create_neurons_vectorized(area_id, positions)
        ...
        connection_count = min(connection_count, 100000)  # Cap at 100K connections for performance
        
        logger.info(f"Creating {connection_count:,} synapses ({connectivity_density:.4%} density)")
        ...
        config = {
            ...
            'batch_size': min(10000, neuron_count // 20),  # Smaller batches for single core
        ...
        logger.info(f"Test brain setup complete: {neuron_count:,} neurons, "
    ...
    def measure_burst_performance(self, burst_engine: BurstEngine, neuron_count: int) -> Dict[str, Any]:
        ...
        logger.info(f"Measuring burst performance for {neuron_count:,} neurons at {self.target_frequency}Hz "
        ...
        while (time.perf_counter() - measurement_start < self.measurement_duration and 
               len(burst_times) < self.min_bursts_to_measure * 2):
        ...
        total_bursts = burst_count_end - burst_count_start + len(burst_times)  # Include test bursts
        ...
        estimated_time_at_10m = avg_burst_time_ms * (self.target_operations_per_burst / neuron_count)
        scalability_feasible = estimated_time_at_10m <= (self.max_burst_time * 1000)
        
        # Memory efficiency (estimate)
        memory_per_neuron_bytes = 64  # Rough estimate
        estimated_memory_10m_gb = (self.target_operations_per_burst * memory_per_neuron_bytes) / (1024**3)
        ...
        results = {
            ...
            'frequency_efficiency': (len(burst_times) / measurement_duration) / self.target_frequency,
            
            # Neural activity vs Processing metrics
            'avg_neurons_fired': np.mean(neurons_fired_counts),  # Only neurons that actually fired
            'max_neurons_fired': np.max(neurons_fired_counts),   # Peak firing in a single burst
            'total_neurons_fired': np.sum(neurons_fired_counts), # Total firings across all bursts
            ...
            'performance_level': self._classify_performance_with_target(overrun_count, len(burst_times), target_progress),
        }
        ...
        logger.info(f"Performance measurement complete: {results['avg_burst_time_ms']:.2f}ms avg, "
                   f"{results['overrun_percentage']:.1f}% overruns")
        
        return results
    
    def _classify_performance_with_target(self, overrun_count: int, total_bursts: int, target_progress: float) -> str:
    ...
    def find_maximum_capacity(self) -> Dict[str, Any]:
        ...
        logger.info("🎯 Finding maximum neuron capacity at 15Hz in EMBEDDED MODE (single CPU core)")
        ...
        for neuron_count in self.test_neuron_counts:
            try:
                ...
                if result['performance_level'] in ['EXCELLENT', 'GOOD', 'ACCEPTABLE', 
                                                 'EXCELLENT_SCALING', 'GOOD_SCALING',
                                                 'EXCELLENT_PROGRESS', 'GOOD_PROGRESS',
                                                 'TARGET_ACHIEVED', 'TARGET_NEAR']:
                    last_good_count = neuron_count
                    max_capacity_result = result
                    target_pct = result.get('target_progress_percent', 0)
                    logger.info(f"✅ {neuron_count:,} neurons: {result['performance_level']} "
                              f"({result['avg_burst_time_ms']:.2f}ms avg, {target_pct:.1f}% of 10M target)")
                else:
                    logger.warning(f"❌ {neuron_count:,} neurons: {result['performance_level']} "
        ...
        logger.info(f"🏁 Embedded mode test complete. Max capacity: {last_good_count:,} neurons")
    ...
    def generate_performance_report(self, results: Dict[str, Any]) -> str:
        ...
        report = []
        report.append("=" * 80)
        report.append("FEAGI 15Hz BURST CAPACITY PERFORMANCE REPORT - 10M TARGET ANALYSIS")  
        report.append("=" * 80)
        report.append("")
        ...
        # Target analysis
        report.append(f"🎯 TARGET: {self.target_operations_per_burst:,} neuron operations per burst at 15Hz")
        report.append(f"⏱️  TIME BUDGET: {self.max_burst_time * 1000:.1f}ms per burst (66.7ms)")
        report.append("")
        ...
        if max_result:
            ...
            report.append(f"   Average burst time: {max_result['avg_burst_time_ms']:.2f}ms")
            report.append(f"   Processing efficiency: {max_result['frequency_efficiency']:.1%}")
            ...
            if 'estimated_time_at_10m_ms' in max_result:
                ...
                report.append(f"   Within time budget: {'✅ YES' if feasible else '❌ NO'}")
        ...
        report.append("")
        report.append("Neuron Count | Target % | Avg Time | Performance Level   | MOps/sec")
        report.append("-" * 70)
        ...
        for neuron_count in sorted(all_results.keys()):
            ...
            report.append(f"{neuron_count:>11,} | {target_pct:>7.1f}% | {avg_time:>8.2f}ms | {perf_level:<18} | {megaops:>7.2f}")
        ...
        if max_result:
            ...
            if target_progress >= 100:
                report.append("✅ TARGET ACHIEVED! 10M neuron operations per burst at 15Hz")
            elif target_progress >= 75:
                report.append(f"🔥 Excellent scaling: {target_progress:.1f}% of target achieved")
                report.append("   Minor optimizations may reach 10M target")
            elif target_progress >= 50:
                report.append(f"⚡ Good progress: {target_progress:.1f}% of target achieved")
                ...
                report.append(f"❌ Limited scaling: Only {target_progress:.1f}% of 10M target")
...
def test_15hz_burst_capacity():
    ...
    try:
        ...
        report_file = f"feagi_core/tests/performance/logs/15hz_embedded_capacity_{timestamp}.txt"
        ...
        if results['max_capacity_result']:
            target_progress = results['max_capacity_result'].get('target_progress_percent', 0)
        
        logger.info(f"🏆 FINAL RESULT: {max_capacity:,} neurons ({target_progress:.1f}% of 10M target)")
        
        # Assertions based on embedded mode expectations
        assert max_capacity > 0, "Should be able to process at least some neurons in embedded mode"
        
        if target_progress >= 100:
            logger.info("🎉 TARGET ACHIEVED! 10M neuron operations per burst at 15Hz in embedded mode!")
        elif target_progress >= 50:
            logger.info(f"⚡ Good progress: {target_progress:.1f}% of 10M target achieved")
        elif target_progress >= 10:
            logger.info(f"🔧 Optimization needed: Only {target_progress:.1f}% of 10M target")
        else:
            logger.warning(f"⚠️  Significant architectural changes needed: {target_progress:.1f}% of target")
