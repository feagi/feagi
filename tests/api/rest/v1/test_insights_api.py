...
def test_activity_heatmap(client, mock_core_api):
    ...
    request_data = {"window": 1, "include_empty": False, "threshold": 0.1}
...
def test_neuron_activity_time_series(client, mock_core_api):
    ...
    request_data = {"neuron_ids": ["neuron1", "neuron2", "neuron3"], "window": 5}
...
def test_performance_stats(client, mock_core_api):
    ...
    if response.status_code == 200:
        ...
        assert (
            data["min_burst_duration"]
            <= data["average_burst_duration"]
            <= data["max_burst_duration"]
        )
...
def test_connectivity_graph(client, mock_core_api):
    ...
    response = client.get(
        "/v1/insights/connectivity/graph",
        params={"include_weights": True, "min_weight": 0.2},
    )
    assert response.status_code in (200, 400, 404, 422)
    if response.status_code == 200:
        data = response.json()
