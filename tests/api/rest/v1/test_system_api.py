...
def mock_core_api():
    ...
    with patch("feagi.api.gateway.APIGateway.core_api", new_callable=MagicMock) as mock:
        ...
        mock.get_configuration.return_value = {
            ...
            "simulation": {"burst_duration": 10, "max_firing_rate": 100},
            "connectivity": {"max_synapses_per_neuron": 1000},
        }
...
def test_get_configuration(client, mock_core_api):
    ...
    if response.status_code == 200:
        ...
        if data and isinstance(data, dict):
            ...
            if (
                "connectivity" in config
                and "max_synapses_per_neuron" in config["connectivity"]
            ):
...
def test_update_configuration(client, mock_core_api):
    ...
    update_data = {
        "parameters": {"log_level": "DEBUG", "simulation": {"burst_duration": 5}}
    }
    ...
    if response.status_code == 200:
        ...
        mock_core_api.update_configuration.assert_called_once_with(
...
def test_update_configuration_failure(client, mock_core_api):
    ...
    update_data = {"parameters": {"log_level": "DEBUG"}}
