...
BAREBONES_GENOME_PATH = os.path.join(
    REPO_ROOT, "feagi/evo/defaults/genome/barebones_genome.json"
)
ESSENTIAL_GENOME_PATH = os.path.join(
    REPO_ROOT, "feagi/evo/defaults/genome/essential_genome.json"
)
...
def test_upload_genome_file(genome_client):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            assert "message" in data
            assert (
                "uploaded and loaded successfully" in data["message"]
                or "uploaded successfully" in data["message"]
            )
    finally:
...
def test_upload_genome_string(genome_client):
    ...
    response = genome_client.post(
        "/v1/genome/upload/string", json={"genome": json.dumps(genome_data)}
    )
    ...
    if response.status_code == 200:
        ...
        assert (
            "uploaded and loaded successfully" in data["message"]
            or "Genome loaded successfully" in data["message"]
        )
            ...
            if not data.get("success", True):
                ...
                if "validation_errors" in data["details"]:
                    ...
                    if validation_errors:
                        ...
                        assert (
                            "blueprint" in error_text
                            or "neuron_morphologies" in error_text
                        )
...
def test_amalgamation_by_payload(genome_client):
    ...
    response = genome_client.post(
        "/v1/genome/amalgamation_by_payload", json=amalgamation_data
    )
...
def test_cortical_template(genome_client):
    ...
    if response.status_code == 200:
        ...
        assert any(
            template["name"] == "Test Template" for template in data["templates"]
        )
