...
class TestZMQAPICompliance:
    ...
    def test_zmq_socket_has_required_methods(self):
        ...
        for socket_type in socket_types:
            ...
            try:
                ...
                assert hasattr(socket, 'recv'), f"Socket type {socket_type} missing recv method"
                assert hasattr(socket, 'send'), f"Socket type {socket_type} missing send method" 
                assert hasattr(socket, 'bind'), f"Socket type {socket_type} missing bind method"
                assert hasattr(socket, 'connect'), f"Socket type {socket_type} missing connect method"
                assert hasattr(socket, 'close'), f"Socket type {socket_type} missing close method"
                
                # These methods should NOT exist (catch recv_into bug)
                assert not hasattr(socket, 'recv_into'), f"Socket type {socket_type} should not have recv_into"
                assert not hasattr(socket, 'send_into'), f"Socket type {socket_type} should not have send_into"
    ...
    def test_zmq_socket_types_consistency(self):
        ...
        for socket_type, name in test_cases:
            socket = context.socket(socket_type)
            try:
                assert socket.socket_type == socket_type, f"Socket type mismatch for {name}"
            finally:
                socket.close()
...
def test_zmq_version_compatibility():
    ...
    assert isinstance(version, str), f"ZMQ version should be a string, got {type(version)}"
...
def test_zmq_recv_into_error_prevention():
    ...
    try:
        ...
        assert not hasattr(socket, 'recv_into'), "ZMQ socket should not have recv_into method"
