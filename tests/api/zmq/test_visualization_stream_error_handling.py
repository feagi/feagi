...
class TestVisualizationStreamErrorHandling:
    ...
    @pytest.mark.xfail(reason="Test assumes specific atomic reference behavior that may have changed")
    ...
    def test_publish_data_attribute_error_handling(self, viz_stream):
        ...
        viz_stream.socket.send.side_effect = AttributeError("'NoneType' object has no attribute 'send'")
    ...
    @pytest.mark.xfail(reason="Mock setup doesn't work with method object - test implementation issue")
    ...
    def test_health_status_comprehensive(self, viz_stream):
        ...
        expected_keys = [
            'running', 'active_mode', 'socket_available', 'context_available',
            'worker_thread_count', 'stop_event_set', 'worker_threads', 'stats'
        ]
