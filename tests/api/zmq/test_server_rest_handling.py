...
async def server_with_mocks(mock_core_api_service, mock_context, mock_rest_adapter):
    ...
    server = ZmqServer(
        core_api=mock_core_api_service, context=mock_ctx, control_port=5559
    )
...
@pytest.mark.asyncio
async def test_handle_rest_api_message(
    server_with_mocks, mock_context, mock_rest_adapter
):
    ...
    response_data = json.dumps(
        {"status": 200, "body": {"status": "healthy"}, "timestamp": 1621234567890}
    ).encode("utf-8")
    ...
    with pytest.raises(asyncio.CancelledError):
        await asyncio.wait_for(
            server_with_mocks._handle_control_messages(), timeout=1.0
        )
...
async def test_handle_legacy_message(server_with_mocks, mock_context):
    ...
    server_with_mocks._process_control_message.assert_called_once_with(
        identity, legacy_message
    )
...
@pytest.mark.asyncio
async def test_handle_error_during_processing(
    server_with_mocks, mock_context, mock_rest_adapter
):
    ...
    # Set up the socket to return a REST API format message
    identity = b"client-id"
    rest_message = {"route": "/v1/system/health_check", "method": "GET"}
    rest_message_bytes = json.dumps(rest_message).encode("utf-8")
