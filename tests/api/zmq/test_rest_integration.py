...
pytest.skip(
    "ZMQ server tests need to be updated after protocol refactoring",
...
class TestZmqServer:
    ...
    async def send_request(
        self, route, method="GET", params=None, query=None, body=None
    ):
...
def core_api_mock():
    ...
    mock.update_configuration = AsyncMock(
        return_value={"message": "Configuration updated", "status": "success"}
    )

    # Mock genome data
    mock.get_genome_blueprint = AsyncMock(
        return_value={"test_area": {"id": "1", "name": "Test Area"}}
    )

    # Mock cortical area data
    mock.get_cortical_areas = AsyncMock(return_value=[{"id": "1", "name": "Test Area"}])

    # Mock single cortical area
    mock.get_cortical_area = AsyncMock(return_value={"id": "1", "name": "Test Area"})
...
async def zmq_server(core_api_mock):
    ...
    server = ZmqServer(core_api=core_api_mock, control_port=5559)
...
async def test_update_configuration(zmq_server, test_client):
    ...
    response = await test_client.send_request(
        "/v1/configuration", method="PUT", body={"burst_rate": 120}
    )
