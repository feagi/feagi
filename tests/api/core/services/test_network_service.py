...
class TestNetworkService:
    ...
    def test_get_connection_statistics_with_agents(
        self, network_service, mock_state_manager
    ):
    ...
    def test_test_connectivity_healthy(self, network_service):
        ...
        state_manager_test = next(
            (test for test in result["tests"] if test["component"] == "state_manager"),
            None,
        )
    ...
    def test_test_connectivity_with_target(self, network_service):
        ...
        external_test = next(
            (test for test in result["tests"] if "external" in test["component"]), None
        )
    ...
    def test_reset_network_statistics_with_error(self, network_service):
        ...
        with patch.object(
            network_service.logger, "info", side_effect=Exception("Log error")
        ):
            result = network_service.reset_network_statistics()
    ...
    def test_configure_bandwidth_limits_success(self, network_service):
        ...
        limits = {
            "incoming_mbps": 100,
            "outgoing_mbps": 50,
            "protocol_limits": {"zmq": 30, "websocket": 20},
        }
    ...
    def test_configure_bandwidth_limits_invalid_data(self, network_service):
        ...
        with patch.object(
            network_service.logger, "info", side_effect=Exception("Invalid data")
        ):
    ...
    def test_connection_limit_validation(self, network_service):
        ...
        # Valid limits
        valid_limits = {"max_upload_mbps": 100, "max_download_mbps": 200}
        ...
        invalid_limits = {"max_upload_mbps": -50, "max_download_mbps": "unlimited"}
