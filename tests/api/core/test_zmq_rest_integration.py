...
def core_api_service(mock_state_manager):
    ...
    with patch('feagi.api.core.services.core_api_service.CoreAPIService._create_burst_engine') as mock_create_burst:
        ...
        service.get_cortical_areas = MagicMock(return_value=connectome_manager.get_all_cortical_areas())
        service.get_cortical_area = MagicMock(side_effect=connectome_manager.get_cortical_area)
...
def test_get_configuration(zmq_client, core_api_service):
    ...
    core_api_service.get_configuration = MagicMock(return_value={
        "burst_rate": 60,
        "learning_rate": 0.01
    })
