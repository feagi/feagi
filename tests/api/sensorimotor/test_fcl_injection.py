...
class FCLInjectionTester:
    ...
    async def send_test_data(self, cortical_area="iic400"):
        ...
        categorized_data = {
            cortical_area: {"x": [], "y": [], "z": [], "potentials": []}
        }
        ...
        logger.info(
            f"Successfully sent {len(test_data)} activations to {cortical_area}"
        )
        return test_data
        ...
        try:
            ...
            for coord, sent_potential in sent_data.items():
                ...
                for i in range(len(response["neuron_data"]["x"])):
                    if (
                        response["neuron_data"]["x"][i] == x
                        and response["neuron_data"]["y"][i] == y
                        and response["neuron_data"]["z"][i] == z
                    ):
                        ...
                        if potential_diff > max_allowed_diff:
                            logger.warning(
                                f"Potential at {coord} differs too much: sent={sent_potential}, received={received_potential}"
                            )
                            verification_results.append(False)
                        else:
                            logger.info(
                                f"Verified neuron at {coord}: sent={sent_potential}, received={received_potential}"
                            )
            ...
            success_rate = (
                sum(verification_results) / len(verification_results)
