...
class SensorimotorClient:
    ...
    async def disconnect(self):
        ...
        if hasattr(self, '_motor_receiver_task') and not self._motor_receiver_task.done():
    ...
    async def send_sensory_data(self, channel_id: str, data, data_type="neurons") -> bool:
        """
        ...
        if self.feagi_state != "active":
            logger.warning(f"FEAGI is in {self.feagi_state} mode. Sensory data will be ignored.")
        ...
        try:
            ...
            logger.info(f"Sent {len(binary_data)} bytes of {data_type} data to channel {channel_id}")
            return True
    ...
    async def receive_motor_data(self, timeout=1.0):
        ...
        try:
            ...
            multipart = await asyncio.wait_for(
                self._motor_socket.recv_multipart(),
                timeout=timeout
            )
        ...
        while self.connected:
            try:
                ...
                if topic == "system":
                    ...
                    # Regular motor data
                    logger.debug(f"Received motor data on topic {topic}: {len(data)} bytes")
    ...
    def _setup_connection(self):
        ...
        logger.info(f"Connected to FEAGI at {self.feagi_host}:{self.sensory_port}/{self.motor_port}")
        self.connected = True

# Helper function to generate test neuron data
def generate_test_neuron_data(cortical_id, width=64, height=64, depth=1, activity_percentage=20):
    """
...
def generate_flat_neuron_data(neurons_per_area=20, num_areas=3):
    ...
    logger.info(f"Generated flat neuron data with {len(cortical_ids)} neurons across {num_areas} areas")
    return flat_data
...
def generate_test_image(width=64, height=64, channels=3, pattern="random"):
    ...
    if pattern == "random":
        ...
        if channels == 1:
            ...
            img = np.zeros((height, width, channels), dtype=np.uint8)
            for c in range(channels):
                img[:, :, c] = np.tile(np.linspace(0, 255, width, dtype=np.uint8), (height, 1))
...
async def main():
    ...
    try:
        ...
        logger.info("\n----- TEST 3: IMAGE DATA -----")
        image_data = generate_test_image(width=64, height=64, channels=3, pattern="checker")
        logger.info(f"Sending data with FEAGI in {client.feagi_state} mode")
