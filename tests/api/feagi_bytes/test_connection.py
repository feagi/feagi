...
pytest.skip(
    "Test needs to be updated after feagi_connector refactoring",
...
class CustomZmqFeagiClient:
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self.sensorimotor_socket.connect(
                f"tcp://{self.host}:{self.sensorimotor_port}"
            )

            # Create DEALER socket for visualization
            logger.info(
                f"Connecting to visualization port {self.visualization_port}..."
            )
            self.visualization_socket = self.context.socket(zmq.DEALER)
            self.visualization_socket.connect(
                f"tcp://{self.host}:{self.visualization_port}"
            )
    ...
    async def send_sensory_data(
        self, channel_id: int, data: Union[bytes, List[float]]
    ) -> bool:
        ...
        try:
            ...
            message = {
                ...
                "data": (
                    data if isinstance(data, list) else data.hex()[:100] + "..."
                ),  # Truncate hex representation for logging
                "timestamp": time.time(),
            }
            ...
            await self.sensorimotor_socket.send_multipart(
                [
                    b"",  # Empty delimiter
                    json.dumps(
                        {"message_type": "sensory", "channel_id": channel_id}
                    ).encode("utf-8"),
                    (
                        data
        ...
        try:
            ...
            try:
                ...
                response = await asyncio.wait_for(
                    self.req_socket.recv_json(), timeout=5.0
                )
...
def test_raw_data(
    data_type: str = "image", size: Tuple[int, int] = (28, 28), channels: int = 1
) -> bytes:
    """
    ...
    if data_type == "image":
        ...
        logger.info(
            f"Generated test image: {width}x{height}x{channels}, size: {len(raw_data)} bytes"
        )
        return raw_data
        ...
        logger.info(
            f"Generated test array: {len(values)} values, size: {len(raw_data)} bytes"
        )
        return raw_data
...
async def test_client_communication():
    ...
    client = CustomZmqFeagiClient(
        host="localhost", req_port=5555, sensorimotor_port=5558, visualization_port=5560
    )
