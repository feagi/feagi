...
class TestPublisher:
    ...
    async def send_test_messages(self, count=30):
        ...
        for i in range(1, count+1):
            ...
            activity_data = bytes([11, 1, 0, 1]) + os.urandom(data_length)  # 11 = NEURON_CATEGORIES_ID
            await self.socket.send_multipart([b"activity", activity_data])
            
            # Log what we sent
            logger.info(f"Sent message batch #{i}: system ({len(system_msg)} bytes), activity ({len(activity_data)} bytes)")
...
class TestSubscriber:
    ...
    def stop(self):
        ...
        self.running = False
        logger.info(f"Subscriber stopped. Received {self.message_count} total messages ({self.activity_count} activity messages)")
    ...
    def run(self, max_time=30):
        ...
        logger.info(f"Starting subscriber main loop (will run for up to {max_time} seconds)")
                    ...
                    if len(message) >= 2:
                        ...
                        if topic == b"system":
                            ...
                            try:
                                ...
                                logger.warning(f"Received binary system message: {len(data)} bytes")
                                
                        elif topic == b"activity":
                            # Handle activity messages
                            self.activity_count += 1
                            logger.info(f"Received activity message #{self.activity_count}: {len(data)} bytes")
                            ...
                            if len(data) > 0:
                                ...
                                logger.info(f"Activity data format: ID={format_id}, version={version}")
                        
                        else:
                            # Unknown topic
                            logger.warning(f"Received message with unknown topic: {topic}")
                    
                except zmq.Again:
                    # Timeout - no message available
                    elapsed = time.time() - start_time
                    if int(elapsed) % 5 == 0:  # Log every 5 seconds
                        logger.info(f"Waiting for messages... ({elapsed:.1f}s elapsed, {self.message_count} received)")
...
if __name__ == "__main__":
    ...
    if mode == "publisher":
        ...
        logger.info(f"Test complete. Received {result[0]} total messages ({result[1]} activity messages)") 