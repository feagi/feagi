...
def skip_if_mocked_backend(bitmap_or_fcl):
    ...
    if backend is not None and isinstance(
        getattr(backend, "device", None), unittest.mock.MagicMock
    ):
...
class TestGPUAcceleratedFCL(unittest.TestCase):
    def setUp(self):
        ...
        # Add some test data
        neurons_by_cortical = {1: BitMap({1, 2, 3}), 2: BitMap({10, 11, 12})}
        self.fcl.update_fcl(0, neurons_by_cortical)

        neurons_by_cortical = {1: BitMap({2, 3, 4}), 2: BitMap({11, 12, 13})}
