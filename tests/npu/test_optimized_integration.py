...
def test_create_optimized_core(rust_available):
    ...
    with patch('feagi.npu.optimized_integration.RUST_AVAILABLE', rust_available), \
         ...
         patch('feagi.npu.optimized_integration.FireCandidateList'), \
        ...
        if rust_available:
            ...
            assert set(result.keys()) >= {"gna", "fcl", "connectome", "current_timestep"}
...
def test_propagate_activations_dict(dict_core):
    ...
    with patch('feagi.npu.optimized_integration.RUST_AVAILABLE', False), \
         patch('feagi.npu.optimized_integration.propagate_activations', return_value=[]):
        
        # Create a non-empty outgoing connections dict
        dict_core["connectome"].get_outgoing_connections = MagicMock(return_value={
            10: (0.5, 100),  # synapse_id: (weight, target_neuron_id)
            20: (0.7, 200)
        })
        
        # Now call the original function (not our patched version)
        from feagi.npu.optimized_integration import propagate_activations as original_propagate
