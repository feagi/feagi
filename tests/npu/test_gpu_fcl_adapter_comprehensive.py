...
def test_gpu_bitmap_initialization_with_numpy_array():
    ...
    with patch('feagi.npu.gpu_fcl_adapter.get_backend') as mock_get_backend:
        ...
        pytest.skip("Known issue in gpu_fcl_adapter.py line 58 with numpy array boolean evaluation")
...
def test_gpu_accelerated_fcl_get_fcl_delta():
    ...
    fcl.cpu_fcl.current_window_index = (fcl.cpu_fcl.current_window_index + 1) % fcl.cpu_fcl.default_window_size
...
def test_gpu_bitmap_update_cache_sparse_bitmap():
    ...
    with patch('feagi.npu.gpu_fcl_adapter.get_backend') as mock_get_backend:
        ...
        gpu_bitmap = GPUBitMap([1, 100, 200])  # Will create chunks with zeros in between
        gpu_bitmap._cache_valid = False
