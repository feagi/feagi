...
from feagi.npu.special_area_handler import (
    SpecialAreaHandler,
    SpecialAreaConfig
)
...
class MockConnectomeManager:
    ...
    def __init__(self):
        self.cortical_areas = {
            ...
            'property_power': MockCorticalArea('property_power', {'__power_injection': True}),
            'property_modulator': MockCorticalArea('property_modulator', {'__modulator': True}),
            'complex_power': MockCorticalArea('complex_power', {
...
def test_detect_special_areas(handler):
    ...
    with patch('feagi.npu.special_area_handler.logger') as mock_logger:
        ...
        expected_power_areas = {'power_area_pwr', '___pwr', 'property_power', 'complex_power'}
...
def test_detect_special_areas_no_cortical_areas(mock_connectome_manager):
    ...
    with patch('feagi.npu.special_area_handler.logger') as mock_logger:
        ...
        mock_logger.warning.assert_called_with("No cortical areas available for special area detection")
        assert len(handler.special_areas) == 0
...
def test_get_power_area_neurons_error(handler):
    ...
    del handler.power_area_neurons['power_area_pwr']
    handler.connectome_manager.get_neurons_by_area = Mock(side_effect=Exception("Error"))
...
def test_refresh_all_caches(handler):
    ...
    handler.connectome_manager.cortical_areas['new_power_pwr'] = MockCorticalArea('new_power_pwr')
