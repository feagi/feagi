...
class SIMDCompatibilityTester:
    ...
    def test_numpy_vectorization_performance(self, size: int = 100000) -> Dict[str, float]:
    ...
    def test_rust_simd_availability(self) -> Dict[str, any]:
        ...
        if not RUST_SIMD_AVAILABLE:
            return {
                "available": False,
                "reason": "Rust SIMD module not available"
            }
    ...
    def test_roaring_bitmap_simd(self) -> Dict[str, any]:
        ...
        try:
            ...
            return {
                "available": False,
                "reason": "PyRoaring not available"
            }
        except Exception as e:
            return {
                "available": False,
    ...
    def run_comprehensive_test(self) -> Dict[str, any]:
        ...
        results["performance_tests"]["numpy_vectorization"] = self.test_numpy_vectorization_performance()
        
        print("🧪 Running membrane potential SIMD test...")
        results["performance_tests"]["membrane_potential"] = self.test_membrane_potential_simd()
    ...
    def _generate_recommendations(self, results: Dict[str, any]) -> List[str]:
        ...
        if not cpu_features.get("avx2", False) and not cpu_features.get("neon", False):
            recommendations.append("⚠️  No advanced SIMD features detected. Performance may be limited.")
        
        # Check NumPy SIMD
        numpy_perf = results["performance_tests"]["numpy_vectorization"]
        if numpy_perf["speedup"] < 2.0:
            recommendations.append("⚠️  NumPy vectorization speedup is low. Check BLAS configuration.")
        ...
        if not membrane_perf["effective_simd"]:
            recommendations.append("⚠️  Membrane potential updates not effectively using SIMD.")
        
        # Check Rust SIMD
        rust_simd = results["compatibility"]["rust_simd"]
        if not rust_simd["available"]:
            recommendations.append("💡 Consider building Rust extensions with SIMD support for optimal performance.")
...
class TestSIMDCompatibility:
    ...
    def test_numpy_simd_detection(self, simd_tester):
        ...
        assert numpy_info.get("config_available", False), "NumPy config should be available"
    ...
    def test_vectorization_performance(self, simd_tester):
        ...
        assert perf_results["speedup"] > 1.0, "Vectorized operations should be faster than scalar"
    ...
    def test_membrane_potential_simd_performance(self, simd_tester):
        ...
        assert membrane_results["speedup"] > 1.0, "SIMD membrane updates should be faster"
    ...
    def test_rust_simd_availability(self, simd_tester):
        ...
        assert rust_results["available"], "Rust SIMD should be available when module is imported"
        assert rust_results.get("gna_created", False), "Should be able to create Rust GNA"
    ...
    def test_comprehensive_simd_assessment(self, simd_tester):
        ...
        print(f"⚙️  Cores: {results['system_info']['cpu_count']} physical, {results['system_info']['cpu_count_logical']} logical")
