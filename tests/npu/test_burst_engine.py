...
class MockFCLManager:
    def __init__(self):
        ...
        self.cortical_fcl_history = {
            1: [set() for _ in range(3)]
        }  # Added for updated naming
...
class MockConnectomeManager:
    def __init__(self):
        self.cortical_areas = {
            1: types.SimpleNamespace(id=1, properties={"__shed": True})
        }
...
def test_fq_sampler_samples_and_stops():
    fire_queue_provider = (
        MockFCLManager()
    )  # Using same mock but treating as fire queue provider
    output_queue = Queue(maxsize=10)
    sampler = UnifiedFQSampler(
        fire_queue_provider, sample_frequency_hz=10, output_queue=output_queue
    )
...
def test_update_with_genome(engine):
    ...
    non_shed_areas = [
        area for area in engine.cortical_areas if area.id not in engine.shed_areas
    ]
...
def test_run_burst_engine_basics():
    ...
    with patch("feagi.npu.burst_engine.FeagiStateManager"):
        engine = BurstEngine(
            connectome_manager=cm, fcl_manager=fcl, config={"target_frequency": 60.0}
        )

        # Check basic properties - the engine uses the config value initially
        assert (
            engine.desired_frequency == 60.0
        )  # Uses desired_frequency, not target_frequency
...
def test_load_shedding_behavior():
    ...
    with patch("feagi.npu.burst_engine.FeagiStateManager") as mock_sm, patch(
        "feagi.npu.burst_engine.time"
    ) as mock_time:
...
def test_run_test_function(mock_logger, mock_time, engine):
    ...
    engine.connectome_manager.update_membrane_potentials = MagicMock(
        return_value=[1, 2, 3]
    )
