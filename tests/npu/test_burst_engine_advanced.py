...
class MockFCLManager:
    """Mock FCL manager for testing FQSampler."""
...
class MockConnectomeManager:
    ...
    def __init__(self):
        self._areas = {
            ...
            400: Mock(
                id=400, properties={"fq_sample_rate": 0}
...
def test_fcl_sampler_area_sampling():
    ...
    with patch("feagi.npu.burst_engine.time.perf_counter") as mock_time, patch(
        "feagi.npu.burst_engine.time.sleep"
    ) as mock_sleep, patch.object(output_queue, "put_nowait") as mock_put:
        ...
        assert (
            400 not in fcl_manager.get_cortical_fcl_calls
        )  # Zero rate, should be skipped
...
def test_fcl_sampler_queue_full():
    ...
    sampler = UnifiedFQSampler(
        fcl_manager=fcl_manager, sample_frequency_hz=20, output_queue=output_queue
    )

    # Mock time.sleep to avoid actual sleeping
    with patch("feagi.npu.burst_engine.time.sleep") as mock_sleep, patch(
...
def test_fcl_sampler_area_exception_handling():
    ...
    with patch("feagi.npu.burst_engine.logger") as mock_logger:
        ...
        calls = [
            call("FQSampler error (area 100): Test exception for area 100"),
...
def test_fcl_sampler_run_with_stop_flag():
    ...
    with patch("feagi.npu.burst_engine.time.sleep") as mock_sleep, patch(
        "feagi.npu.burst_engine.time.perf_counter", return_value=0.0
    ):
        sampler = UnifiedFQSampler(
            fcl_manager=fcl_manager, sample_frequency_hz=20, output_queue=output_queue
        )
...
def test_fcl_sampler_fallback_to_global():
    ...
    with patch("feagi.npu.burst_engine.time.perf_counter", return_value=0.0), patch(
        "feagi.npu.burst_engine.time.sleep"
    ):
