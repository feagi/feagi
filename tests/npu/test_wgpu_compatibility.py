...
from typing import Dict, List, Set, Tuple, Any, NamedTuple, Optional
...
class WGPUCompatibilityAnalyzer(ast.NodeVisitor):
    ...
    def visit_Import(self, node: ast.Import):
        ...
        for alias in node.names:
            ...
            if alias.name == 'numpy':
                ...
                self.medium_concerns.append(f"Line {node.lineno}: NumPy import (can be abstracted to WGPU buffers)")
            elif alias.name == 'os':
                self.environment_dependencies += 1
                self.poor_patterns.append(f"Line {node.lineno}: os import (environment access incompatible with WGPU)")
    ...
    def visit_ImportFrom(self, node: ast.ImportFrom):
        ...
        if node.module:
            ...
            for alias in node.names:
                ...
                if node.module == 'feagi.core.backend' and alias.name in ['get_backend', 'BackendType']:
                    self.backend_abstractions += 1
                    self.excellent_patterns.append(f"Line {node.lineno}: Backend abstraction import (perfect for WGPU)")
    ...
    def visit_Call(self, node: ast.Call):
        ...
        if func_name:
            ...
            if func_name == 'get_backend':
                self.backend_abstractions += 1
                self.excellent_patterns.append(f"Line {node.lineno}: get_backend() call (WGPU-ready)")
            
            # GOOD: GPU-friendly NumPy array creation
            elif func_name in ['np.zeros', 'np.ones', 'np.empty', 'numpy.zeros', 'numpy.ones', 'numpy.empty']:
                self._analyze_numpy_array_creation(node)
            
            # GOOD: Vectorized operations
            elif func_name in ['np.where', 'np.add.at', 'np.all', 'np.any', 'np.nonzero']:
                self.vectorized_operations += 1
                self.good_patterns.append(f"Line {node.lineno}: Vectorized operation {func_name} (WGPU-compatible)")
            
            # POOR: Environment variable access
            elif func_name == 'os.environ.get':
                self.environment_dependencies += 1
                self.poor_patterns.append(f"Line {node.lineno}: os.environ.get() (not available in WGPU)")
            
            # POOR: Print statements (stdout issues)
            elif func_name == 'print':
                self.stdout_operations += 1
                if self.stdout_operations <= 3:  # Don't spam for every print
                    self.poor_patterns.append(f"Line {node.lineno}: print() statement (stdout not available in WGPU)")
            
            # POOR: Dynamic memory allocation
            elif func_name in ['list', 'dict', 'set'] and self._is_in_hot_path(node):
                self.dynamic_allocations += 1
                self.poor_patterns.append(f"Line {node.lineno}: Dynamic allocation in hot path (incompatible with WGPU)")
            
            # CRITICAL: File operations
            elif func_name in ['open', 'file', 'read', 'write']:
                self.file_operations += 1
                self.critical_issues.append(f"Line {node.lineno}: File I/O operation (not available in WGPU)")
            ...
            # GOOD: Logging (better than print)
                ...
                if self.logging_operations <= 2:  # Don't spam
                    self.good_patterns.append(f"Line {node.lineno}: Logging usage (WGPU-compatible)")
    ...
    def visit_Attribute(self, node: ast.Attribute):
        ...
        if (isinstance(node.value, ast.Name) and node.value.id == 'os' and 
            node.attr == 'environ'):
            self.environment_dependencies += 1
            self.poor_patterns.append(f"Line {node.lineno}: os.environ access (not available in WGPU)")
        
        # Check for backend type checking
        elif node.attr == 'backend_type':
            self.backend_abstractions += 1
            self.excellent_patterns.append(f"Line {node.lineno}: backend_type access (WGPU-ready)")
        ...
        # Check for array order specification
            ...
            if "'C'" in line_content or '"C"' in line_content:
                ...
                self.excellent_patterns.append(f"Line {node.lineno}: Contiguous array order='C' (perfect for WGPU)")
        ...
        for target in node.targets:
            if isinstance(target, ast.Attribute):
                ...
                if attr_name in ['coordinates_x', 'coordinates_y', 'coordinates_z']:
                    self.soa_patterns += 1
                    self.excellent_patterns.append(f"Line {node.lineno}: SoA coordinates pattern (perfect for WGPU)")
                
                # Check for other SoA patterns
                elif attr_name in ['membrane_potentials', 'thresholds', 'refractory_periods', 
                                 'refractory_counters', 'enabled_flags', 'cortical_idxs']:
                    self.soa_patterns += 1
                    self.excellent_patterns.append(f"Line {node.lineno}: SoA neural data pattern (perfect for WGPU)")
    ...
    def _analyze_numpy_array_creation(self, node: ast.Call):
        ...
        if any(dtype in line_content for dtype in ['float32', 'uint32', 'int32']):
            ...
            self.excellent_patterns.append(f"Line {node.lineno}: GPU-friendly dtype (perfect for WGPU)")
        
        # Check for contiguous memory layout
        if "order='C'" in line_content or 'order="C"' in line_content:
            self.contiguous_arrays += 1
            self.excellent_patterns.append(f"Line {node.lineno}: Contiguous array layout (perfect for WGPU)")
        
        # Check for pre-allocation (capacity parameter)
        if 'capacity' in line_content:
            self.pre_allocated_buffers += 1
            self.good_patterns.append(f"Line {node.lineno}: Pre-allocated buffer (WGPU-compatible)")
    ...
    def _is_in_hot_path(self, node: ast.AST) -> bool:
        ...
        return any(keyword in context for keyword in ['for ', 'while ', 'update', 'process', 'run', 'loop'])
...
class NPUWGPUCompatibilityMonitor:
    ...
    def analyze_file(self, file_path: Path) -> WGPUAnalysisResult:
        ...
        try:
            ...
            return WGPUAnalysisResult(
                ...
                soa_patterns=0, contiguous_arrays=0, gpu_friendly_dtypes=0,
                backend_abstractions=0, vectorized_operations=0, pre_allocated_buffers=0,
                environment_dependencies=0, stdout_operations=0, dynamic_allocations=0,
                file_operations=0, non_contiguous_memory=0, mixed_dtypes=0,
                platform_specific_code=0, numpy_dependencies=0, exception_handling=0,
                logging_operations=0, excellent_patterns=[], good_patterns=[],
                medium_concerns=[], poor_patterns=[], 
    ...
    def generate_report(self, results: Dict[str, WGPUAnalysisResult]) -> str:
        ...
        for level in WGPUCompatibilityLevel:
            level_counts[level] = sum(1 for r in results.values() if r.compatibility_level == level)
        
        # Find critical and excellent modules
        critical_modules = [name for name, r in results.items() if r.compatibility_level == WGPUCompatibilityLevel.CRITICAL]
        ...
        for level in WGPUCompatibilityLevel:
            ...
            report.append(f"   {emoji} {level.value}: {count} modules ({percentage:.1f}%)")
        report.append("")
        ...
        if critical_modules:
            ...
            for module in critical_modules:
                ...
                report.append(f"   🔴 {module} (Score: {result.compatibility_score:.1f}/100)")
        ...
        for module_name, result in sorted(results.items(), key=lambda x: x[1].compatibility_score, reverse=True):
            ...
            if result.critical_issues:
                report.append(f"   🔴 Critical: {len(result.critical_issues)} blocking issues")
...
def test_npu_wgpu_compatibility():
    ...
    avg_score = sum(a.compatibility_score for a in analyses.values()) / total_modules if total_modules > 0 else 0
    
    # Find critical modules
    critical_modules = [name for name, analysis in analyses.items() 
                       if analysis.compatibility_level == WGPUCompatibilityLevel.CRITICAL]
    
    # Find modules with file I/O (absolute blocker)
    file_io_modules = [name for name, analysis in analyses.items() if analysis.file_operations > 0]
    ...
    if file_io_modules:
        failure_reasons.append(f"CRITICAL: File I/O found in modules: {', '.join(file_io_modules)}")
    
    # CRITICAL: Excessive environment dependencies (major WGPU blocker)
    if env_heavy_modules:
        failure_reasons.append(f"CRITICAL: Heavy environment dependencies in modules: {', '.join(env_heavy_modules)}")
    
    # CRITICAL: Overall poor compatibility
    if avg_score < 70:
        failure_reasons.append(f"CRITICAL: Average WGPU compatibility score too low: {avg_score:.1f}/100 (minimum: 70)")
    
    # CRITICAL: Too many critical modules
    if len(critical_modules) > 1:  # Allow max 1 critical module
        failure_reasons.append(f"CRITICAL: Too many modules with critical WGPU issues: {', '.join(critical_modules)}")
    
    # Warning for low compatibility scores (but don't fail)
    if avg_score < 80:
        import warnings
        warnings.warn(f"NPU WGPU compatibility score is below optimal: {avg_score:.1f}/100", UserWarning)
