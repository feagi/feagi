...
class TestPropagationLogic:
    ...
    def test_propagation_with_threshold(self, dict_core):
        ...
        dict_core["gna"].get_membrane_potential = MagicMock(
            return_value=0.8
        )  # Above threshold
        ...
        dict_core["gna"].find_fire_candidates = MagicMock(
            return_value=[1, 3]
        )  # Only neurons 1 and 3 exceed threshold
        ...
        dict_core["connectome"].get_connections_for_neuron = MagicMock(
            return_value=[{"target_id": 10, "weight": 0.5}]
        )
...
class TestRefractoryAndConsecutiveLogic:
    ...
    def test_refractory_and_consecutive_fire_logic(self, dict_core):
        ...
        if hasattr(dict_core["gna"], "get_refractory_period"):
            ...
            dict_core["gna"].get_refractory_period = MagicMock(
                side_effect=mock_get_refractory
            )
        else:
        ...
        dict_core["gna"].process_fired_neurons = MagicMock(
            side_effect=mock_process_fired
        )

        # Test with fire queue
        step_simulation_with_fire_queue(
            dict_core, mpf=True, puf=True, max_consecutive_fires=5
        )
