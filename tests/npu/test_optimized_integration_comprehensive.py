...
class MockGlobalNeuronArray:
    ...
    def find_fire_candidates(self, timestep):
        ...
        for i in range(self.capacity):
            if (
                self._membrane_potentials[i] >= self._thresholds[i]
                and self._refractory_counters[i] == 0
            ):
                candidates.append(i)
        return candidates
...
class MockConnectome:
    ...
    def add_connection(self, source_id, target_id, weight):
        ...
        self._connections[source_id].append({"target_id": target_id, "weight": weight})
...
def mock_optimized_structures():
    ...
    with patch("feagi.npu.optimized_integration.RUST_AVAILABLE", True), patch(
        "feagi.npu.optimized_integration.OptimizedFeagiCore", MockOptimizedFeagiCore
    ):
        yield
...
def test_get_core_property_dict_based():
    ...
    core = {"current_timestep": 5, "gna": "mock_gna", "fcl": "mock_fcl"}
...
def test_step_simulation_dict_based():
    ...
    core = {"gna": gna, "fcl": fcl, "current_timestep": 0}
...
def test_step_simulation_with_fire_queue_dict_based():
    ...
    core = {"gna": gna, "fcl": fcl, "connectome": connectome, "current_timestep": 0}
...
def test_propagate_activations_dict_based():
    ...
    core = {"gna": gna, "fcl": fcl, "connectome": connectome}
