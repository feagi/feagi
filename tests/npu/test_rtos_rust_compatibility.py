...
class RTOSCompatibilityAnalyzer(ast.NodeVisitor):
    ...
    MEDIUM_RISK_IMPORTS = {
        'random', 'pickle', 'json', 'yaml', 'configparser',
        'collections.defaultdict', 'collections.deque'
    }
    
    CRITICAL_FUNCTIONS = {
        'time.sleep', 'threading.Thread', 'multiprocessing.Process',
        'asyncio.run', 'asyncio.create_task', 'subprocess.run',
        'signal.signal', 'os.fork', 'os.system'
    }
    
    HIGH_RISK_FUNCTIONS = {
        'numpy.resize', 'numpy.append', 'list.append', 'dict.update',
    ...
    MEDIUM_RISK_FUNCTIONS = {
        ...
        'time.time', 'datetime.now', 'print', 'input',
    ...
    def _check_dynamic_allocation_patterns(self, node: ast.Call, func_name: str):
        ...
        if self.loop_depth > 0 and func_name in ['list.append', 'list.extend', 'list.insert']:
            self._add_issue(
                CompatibilityLevel.HIGH,
                "Dynamic Allocation",
    ...
    def _check_timing_patterns(self, node: ast.Call, func_name: str):
        ...
        if 'sleep' in func_name:
            severity = CompatibilityLevel.CRITICAL if self.in_main_loop else CompatibilityLevel.HIGH
            self._add_issue(
                severity,
    ...
    def _add_issue(self, level: CompatibilityLevel, category: str, description: str,
                   line_no: int, code_snippet: str, suggestion: str, impact: str):
...
class NPUCompatibilityMonitor:
    ...
    def generate_report(self) -> str:
        ...
        critical_count = sum(1 for a in self.analyses.values() 
                           if a.overall_level == CompatibilityLevel.CRITICAL)
        high_count = sum(1 for a in self.analyses.values() 
        ...
        for module_name in sorted(self.analyses.keys()):
            ...
            report_lines.append(f"   Compatibility Score: {analysis.compatibility_score:.1f}/100")
            ...
            if analysis.issues:
                ...
                for category in sorted(issues_by_category.keys()):
                    ...
                    for issue in sorted(issues, key=lambda x: x.level.value):
                        ...
                        report_lines.append(f"     {level_emoji[issue.level]} Line {issue.line_number}: {issue.description}")
        ...
        if critical_count > 0:
            ...
            report_lines.append("   3. Remove signal handlers (use RTOS signals instead)")
            report_lines.append("")
        ...
        report_lines.append("   1. Replace exception handling with Result<T,E> patterns")
...
def test_npu_rtos_rust_compatibility(compatibility_monitor, npu_path):
    ...
    high_risk_modules = [name for name, analysis in analyses.items() 
    ...
    avg_score = sum(analysis.compatibility_score for analysis in analyses.values()) / len(analyses)
    
    # Write detailed report to file for CI artifacts
    report_file = Path(npu_path).parent.parent / "tmp" / "npu_rtos_compatibility_report.txt"
    ...
    if avg_score < 70:
        import warnings
        warnings.warn(f"Average compatibility score is low: {avg_score:.1f}/100", UserWarning)
...
def test_specific_module_compatibility(compatibility_monitor, npu_path):
    ...
    critical_modules = ['burst_engine.py', 'optimized_structures.py', 'gpu_fcl_adapter.py']
