...
def test_fcl_injection_service_initialization(mock_fcl_manager, mock_special_area_handler):
...
def test_fcl_injection_service_custom_config(mock_fcl_manager, mock_special_area_handler):
...
def test_prepare_injection_batches(injection_service):
    ...
    during_burst_batches = injection_service._injection_batches[InjectionTiming.DURING_BURST]
    post_burst_batches = injection_service._injection_batches[InjectionTiming.POST_BURST]
...
def test_prepare_batches_invalid_timing(mock_fcl_manager, mock_special_area_handler):
    ...
    mock_special_area_handler.special_configs['power_area_1'].injection_timing = 'invalid_timing'
...
def test_inject_during_burst(injection_service):
    ...
    with patch('random.random', return_value=0.5):  # Below 0.8 threshold
        ...
        assert injection_service.injection_timing_stats[InjectionTiming.DURING_BURST] == 1
...
def test_inject_batch_no_fcl_methods(mock_special_area_handler):
    ...
    with patch('feagi.npu.fcl_injection_service.logger') as mock_logger:
        ...
        assert result == 0
        mock_logger.warning.assert_called_with("FCL manager does not support neuron injection")
...
def test_get_statistics(injection_service):
    ...
    with patch('random.random', return_value=0.5):  # Ensure during_burst injection succeeds
...
def test_set_injection_enabled_success(injection_service):
    ...
    assert result == True
    assert injection_service.special_area_handler.special_configs['power_area_1'].enabled == False
...
def test_get_power_injection_preview_with_batch_suffix(mock_fcl_manager, mock_special_area_handler):
