...
def mock_optimized_integration():
    ...
    with patch.dict(
        "sys.modules", {"feagi.npu.optimized_integration": MagicMock()}
    ) as mock_modules:
        mock_step = MagicMock()
        mock_modules[
            "feagi.npu.optimized_integration"
        ].step_simulation_with_fire_queue = mock_step
        yield mock_step
...
@pytest.mark.skip(
    reason="Simulation of logging behavior is hard to test with mocks; tested manually"
)
def test_run_with_fire_queue_log_performance():
    ...
    # Patch dependencies
    with patch("feagi.npu.burst_engine.time") as mock_time, patch(
        "feagi.npu.burst_engine.FeagiStateManager.instance",
        return_value=mock_state_manager,
        ...
        def patched_run_with_fire_queue(
            ...
            if self.burst_count % 100 == 0:
                ...
                mock_logger.info(
                    f"Processed {self.burst_count} bursts. "
...
def test_run_with_fire_queue_null_core():
    ...
    with patch(
        "feagi.npu.burst_engine.FeagiStateManager.instance",
        return_value=mock_state_manager,
    ), patch("feagi.npu.burst_engine.logger") as mock_logger, patch.dict(
        "sys.modules", {"feagi.npu.optimized_integration": MagicMock()}
    ):
        ...
        def patched_run_with_fire_queue(mpf=True, puf=False, max_consecutive_fires=10):
            ...
            if optimized_available:
                ...
                if core:
                    step_simulation_with_fire_queue(
                        core, mpf, puf, max_consecutive_fires
                    )
                else:
