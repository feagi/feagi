...
def test_enhanced_fcl_manager_consistency():
    ...
    # Measure consistency
    consistency = enhanced_fcl.get_memory_cortical_consistency(10, pattern_duration=1, window_duration=5)
...
def test_enhanced_fcl_manager_consistent_neurons():
    ...
    for i in range(5):
        enhanced_fcl.update_fcl(i, {10: [100, 101, 102 + i]})  # 100,101 consistent, 102+i not
        enhanced_fcl.advance_timestep()
...
def test_inject_neurons_into_fcl_with_timestep():
    ...
    inject_neurons_into_fcl(fcl_manager, cortical_idx=1, neuron_ids=[100, 101], timestep=1)
