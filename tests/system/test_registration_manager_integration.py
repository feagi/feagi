...
class TestRegistrationManagerIntegration:
    ...
    def test_agent_api_registration_flow(self, registration_manager, mock_core_api_service):
            ...
            with viz_stream._client_lock:
                ...
                if was_new_client:
                    ...
                    response = registration_manager.register_agent_direct(
                        ...
                        metadata={"connection_type": "zmq_heartbeat", "stream_port": viz_stream.port}
                    )
    ...
    def test_fq_sampler_coordination_status(self, registration_manager):
        ...
        registration_manager.register_agent(AgentRegistrationRequest(
            agent_id="viz_agent",
            agent_type="brain_visualizer",
            capabilities={"visualization": True},
            agent_data_port=8080,
    ...
    def test_concurrent_registration_safety(self, registration_manager):
        ...
        assert len(set(result.message for result in results)) >= 1  # At least one unique message
    ...
    def test_seamless_re_registration(self, registration_manager):
        ...
        agent_props = registration_manager.get_agent_properties("reregistration_test_agent")
        ...
        reregistration_response = registration_manager.register_agent(reregistration_request)
        ...
        updated_props = registration_manager.get_agent_properties("reregistration_test_agent")
        ...
        matching_agents = [a for a in agents_list["agents"] if a["agent_id"] == "reregistration_test_agent"]
        assert len(matching_agents) == 1
        
        # FQ sampler coordination should reflect new capabilities
        assert reregistration_response.fq_samplers_enabled.get("visualization", False) is False
            ...
            for i, line in enumerate(lines):
                if pattern in line:
                    ...
                    if "@architecture:acceptable" in context:
                        ...
                        assert False, f"Found forbidden hardcoded pattern '{pattern}' without @architecture:acceptable annotation at line {i+1}: {line.strip()}"
