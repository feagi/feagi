...
class TestCodebaseHardcodingDetection:
                    ...
                    for line_num, line in enumerate(lines, 1):
                        ...
                        for pattern in timeout_patterns:
                            if re.search(pattern, line):
                                ...
                                if self._is_acceptable_hardcoded_timeout(line, lines, line_num):
    ...
    def _is_acceptable_hardcoded_timeout(self, line: str, all_lines: List[str], line_num: int) -> bool:
        """
        ...
        if any(marker in line.lower() for marker in [
            '# fallback', '# emergency', '# @architecture:acceptable',
            '# emergency fallback', '# only used if configuration',
        ...
        context_window = '\n'.join(all_lines[max(0, line_idx - 5):min(len(all_lines), line_idx + 5)])
        shutdown_indicators = [
            'shutdown', 'cleanup', 'emergency', 'fallback', 
            'configuration.*unavailable', 'config.*fail',
            'last resort', 'when.*config.*missing'
        ]
        if any(re.search(indicator, context_window, re.IGNORECASE) for indicator in shutdown_indicators):
            return True
        ...
        if any(re.search(pattern, line, re.IGNORECASE) for pattern in fallback_variable_patterns):
            return True
        ...
        with patch.dict(os.environ, {
            ...
            try:
                ...
                if "API host is required" in str(e):
                    pytest.fail("ProcessManager failed due to missing host configuration despite environment overrides")
...
class TestRealConfigurationIntegration:
    ...
    def test_configuration_environment_override_integration(self):
        ...
        with patch.dict(os.environ, test_env, clear=False):
            ...
            with patch('feagi.config.toml_loader.load_feagi_config', return_value=minimal_config):
