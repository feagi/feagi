...
from typing import Dict, Type, Any, List, Tuple, Optional, ClassVar
...
class ProtocolRegistry:
    ...
    def register(self, protocol_class: Type[VersionedProtocol]) -> None:
        ...
        if version in self._registry[protocol_id]:
            raise ValueError(f"Protocol {protocol_id.name} version {version} already registered")
        
        self._registry[protocol_id][version] = protocol_class
    
    def get_protocol(self, protocol_id: ProtocolID, version: int) -> Type[VersionedProtocol]:
        """
        ...
        if version not in self._registry[protocol_id]:
            raise ValueError(f"Protocol {protocol_id.name} version {version} not registered")
    ...
    def get_compatible_version(self, protocol_id: ProtocolID, client_versions: List[int]) -> Optional[int]:
        """
