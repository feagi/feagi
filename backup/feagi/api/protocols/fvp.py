...
class FVPv1(VersionedProtocol):
    ...
    def _decode_neuron_activations(cls, data: bytes) -> Dict[str, Any]:
        ...
        return {"cortical_area_id": cortical_area_id, "activations": activations}
    ...
    def _decode_connection_strengths(cls, data: bytes) -> Dict[str, Any]:
        ...
        if (
            len(data) < 12
        ):  # At least source area ID, target area ID and number of connections
            raise ValueError("Connection strengths data too short")

        # Unpack area IDs and number of connections
        source_area_id, target_area_id, num_connections = struct.unpack(
            "!III", data[:12]
        )
        ...
        for _ in range(num_connections):
            ...
            source_neuron_id, target_neuron_id, strength = struct.unpack(
                "!IIf", data[offset : offset + 12]
            )
            connections.append(
                {
    ...
    def _decode_area_summary(cls, data: bytes) -> Dict[str, Any]:
        ...
        cortical_area_id, num_neurons, avg_activity, max_activity, num_active = (
            struct.unpack("!IIffi", data[:20])
        )
    ...
    def _decode_global_stats(cls, data: bytes) -> Dict[str, Any]:
        ...
        total_neurons, total_synapses, total_active, burst_duration, burst_frequency = (
            struct.unpack("!IIIIf", data[:20])
        )
    ...
    def _decode_structure_data(cls, data: bytes) -> Dict[str, Any]:
        ...
        for _ in range(area_count):
            ...
            areas[str(area_id)] = {"name": area_name, "neuron_count": neuron_count}
        ...
        for _ in range(connection_count):
            ...
            source_id, target_id, synapse_count = struct.unpack(
        ...
        return {"areas": areas, "connections": connections}
