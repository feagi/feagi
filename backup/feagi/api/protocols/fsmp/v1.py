...
class FSMPv1(VersionedProtocol):
    ...
    def decode(cls, data: bytes) -> Dict[str, Any]:
        ...
        channel_id, timestamp_ms, payload_length = FSMPMessageFormat.unpack_header(header_data)
        
        # Verify payload length
        if len(payload_data) != payload_length:
            raise ValueError(f"FSMP payload length mismatch: expected {payload_length}, got {len(payload_data)}")
    ...
    def _decode_neuron_potential_data(cls, data: bytes) -> Dict[str, Any]:
        ...
        for _ in range(cortical_area_count):
            ...
            reading_start_index, neuron_count = struct.unpack("!II", data[offset:offset+8])
            offset += 8
        ...
        for area in cortical_areas:
            ...
            area_start = area["reading_start_index"] - 1  # Convert from 1-indexed to 0-indexed
            ...
            for i in range(neuron_count):
                if p_offset + 4 > len(data):
                    raise ValueError("Neuron potential data truncated (potential values)")
    ...
    def _decode_sensory_data(cls, data: bytes) -> Dict[str, Any]:
        ...
        if len(data) < expected_length:
            raise ValueError(f"Sensory data truncated: expected {expected_length}, got {len(data)}")
    ...
    def _decode_proprioceptive_data(cls, data: bytes) -> Dict[str, Any]:
        ...
        if len(data) < expected_length:
            raise ValueError(f"Proprioceptive data truncated: expected {expected_length}, got {len(data)}")
