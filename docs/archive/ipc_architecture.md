# FEAGI IPC Architecture

This document describes the Inter-Process Communication (IPC) architecture used in FEAGI 2.1 to enable efficient communication between the API server and the burst engine.

## Overview

FEAGI 2.1 uses a high-performance shared memory approach for IPC rather than socket-based communication (ZMQ). This architecture provides several benefits:
...
1. **Latency**: < 100 microseconds for typical IPC operations (vs. 1-10ms for ZMQ)
