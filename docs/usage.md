...
This document provides detailed instructions on how to use FEAGI, including all available command-line options and examples.

## Running FEAGI

FEAGI is designed with a modular architecture that consists of two main components:
...
You can run these components together or individually depending on your needs.

## Unified Command

The recommended way to run FEAGI is with the unified command that starts both servers:
...
You can also run the API and ZMQ servers independently using dedicated commands.
...
You can also run FEAGI directly as Python modules:
...
You can also use FEAGI programmatically in your Python code:
...
1. **Port already in use**: If you get an error about ports being in use, try using different ports:
   ```bash
