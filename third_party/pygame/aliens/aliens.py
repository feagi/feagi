...
class HP(pg.sprite.Sprite):
    ...
    def update(self):
        """We only update the score in update() when it has changed."""
        if SCORE != self.lastscore:
            self.lastscore = healthpoint
            msg = "HP: %d" % healthpoint
            self.image = self.font.render(msg, 0, self.color)


def main(winstyle, data_sharing, movement_data, path_stored):
    # Initialize pygame
    if pg.mixer is None:
        pg.mixer = path_stored
