...
from feagi.bdu.models.array_backend import BackendType, PrecisionType, ArrayBackend
...
def create_test_brain(connectome, num_neurons=10_000, density=0.01):
    ...
    """
    logger.info(f"Creating test brain with {num_neurons} neurons and {density:.2%} connection density")
    ...
    for i in range(4):
        area = CorticalArea(
            ...
            dimensions=[50, 50, 10],
            position=[i * 60, 0, 0],
            cortical_function=CorticalFunction.SENSORY if i == 0 else CorticalFunction.ASSOCIATION
        )
    ...
    logger.info(f"Adding {num_connections} synapses ({density:.2%} of possible connections)")
    ...
    for i in range(len(areas) - 1):
        ...
        source_neurons = [nid for nid in neuron_ids 
                         if connectome.get_area_for_neuron(nid) == source_area.cortical_id]
        target_neurons = [nid for nid in neuron_ids 
    ...
    input_neurons = [nid for nid in neuron_ids 
...
def run_simulation(connectome, input_neurons, num_timesteps=100):
    ...
    for t in range(num_timesteps):
        ...
        if t % 5 == 0:  # Every 5 timesteps
            ...
            stimulate_neurons = np.random.choice(input_neurons, size=num_to_stimulate, replace=False)
            
            # Set membrane potentials above threshold
            for neuron_id in stimulate_neurons:
                idx = connectome.neuron_id_to_index[neuron_id]
                connectome.neuron_array.membrane_potentials[idx] = 1.0  # Well above threshold
    ...
    logger.info(f"  Average firing rate: {total_fired / num_timesteps:.2f} neurons/timestep")
...
def run_comparison(num_neurons=10_000, density=0.01, num_timesteps=100, backends=None):
    ...
    for backend in backends:
        ...
        elapsed_time_single, total_fired_single = run_simulation(connectome, input_neurons, num_timesteps)
        ...
        try:
            ...
            if multi_gpu_config.num_devices >= 2:
                ...
                neuron_ids, input_neurons = create_test_brain(connectome_multi, num_neurons, density)
                elapsed_time_multi, total_fired_multi = run_simulation(connectome_multi, input_neurons, num_timesteps)
...
def main():
    ...
    parser.add_argument("--density", type=float, default=0.01, help="Connection density")
    ...
    parser.add_argument("--all-backends", action="store_true", help="Test all available backends")
