...
def benchmark_standard_vs_gpu(num_neurons=10000, num_synapses=100000):
    ...
    for neuron_id in initial_firing:
        std_cm.set_neuron_property(neuron_id, "membrane_potential", 2.0)  # Above threshold
    
    for i in range(num_iterations):
        start_time = time.time()
        fired = std_cm.update_membrane_potentials()
        duration = time.time() - start_time
        print(f"  Iteration {i+1}: {duration:.4f} seconds, {len(fired)} neurons fired")
    ...
    for neuron_id in initial_firing:
        direct_gpu_cm.set_neuron_property(neuron_id, "membrane_potential", 2.0)  # Above threshold
    ...
    plt.subplot(1, 2, 1)
    plt.bar(['Standard', 'GPU (conversion)', 'GPU (direct)'], 
