...
def opu_handler():
    ...
    if runtime_data.fire_candidate_list['motor_opu']:
        ...
        idx = 0
        ctr = 0
        motor_opu_x_max = runtime_data.genome['blueprint']['motor_opu']['neuron_params']['block_boundaries'][0]
        
        while idx < (motor_opu_x_max - 1):
            if activity_report[idx][0][0] > 0 and activity_report[idx + 1][0][0] == 0:
                motor_data[ctr] = 'F'
            elif activity_report[idx + 1][0][0] > 0 and activity_report[idx][0][0] == 0:
                motor_data[ctr] = 'B'
            idx += 2
            ctr += 1
