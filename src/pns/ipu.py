...
class IPU:
    ...
    class Controller:
        ...
        def __init__(self):
            ...
            self.source = IPU.Source()
            self.battery = self.source.Battery()

        def proximity_controller(self):
            while not runtime_data.exit_condition:
                try:
                    self.source.Lidar.translate()
                ...
                if "sensory_data" in ipu_data["data"]:
                    for sensor_type in ipu_data["data"]["sensory_data"]:
                        ...
                        if 'battery' in sensor_type and ipu_data["data"]["sensory_data"][sensor_type] is not None:
                            try:
                                self.battery.translate(sensor_data=ipu_data["data"]["sensory_data"][sensor_type])
                            except Exception:
    ...
    class Source:
        ...
        class Battery:
            ...
            def __init__(self):
                print("Battery class initialized.")
                self.range = IPU.Processor.Range()

            def translate(self, sensor_data):
                """
                ...
                if sensor_data is not None:
                    for sensor in sensor_data:
                        ...
                        detections = self.range.range_to_coords(cortical_area=cortical_area,
                                                                range_data=int(float(sensor_data[sensor]) * 100),
                                                                range_min=0, range_max=100, threshold=10)

                        neurons = self.range.coords_to_neuron_ids(detections, cortical_area=cortical_area)
        ...
        class Lidar:
            def __init__(self):
                print("class lidar initialized")
                self.range = IPU.Processor.Range()

            def translate(self, proximity_data, type=None):
                """
                ...
                if proximity_data is not None:
                    ...
                    for sensor in proximity_data:
                        ...
                        if hasattr(proximity_data[sensor], '__iter__'):
                            detections = self.range.lidar_to_coords(proximity_data[sensor])
                        else:
                            detections = self.range.sonar_to_coords(proximity_data[sensor])

                        neurons = self.range.coords_to_neuron_ids(
    ...
    class Processor:
        class Range:
            ...
            def __init__(self):
                print("class Processor:Range initialized")

            def range_to_coords(self, cortical_area, range_data, range_min, range_max, threshold):
                """
                ...
                if hasattr(range_data, '__iter__'):
                    for idx, dist in enumerate(range_data):
                        if dist != inf:
                            dist_map = self.map_value(dist, range_min, range_max, 0, z_max)
                    ...
                    try:
                        dist_map = round(self.map_value(range_data, range_min, range_max, 0, z_max))
                    except TypeError:
                        dist_map = 0
            ...
            def lidar_to_coords(self, lidar_data, threshold=5):
                ...
                for idx, dist in enumerate(lidar_data):
                    if dist != inf:
                        dist_map = self.map_value(dist, LIDAR_MIN, LIDAR_MAX, 0, Z_MAX)
                        if dist_map <= threshold:
                            x = idx
                            y = Y_MAX // 2
                            z = dist_map
            ...
            def sonar_to_coords(self, sonar_data, threshold=10):
                ...
                try:
                    dist_map = round(self.map_value(sonar_data, SONAR_MIN, SONAR_MAX, 0, Z_MAX - 1))
                except TypeError:
                    dist_map = 0
                    print("Type Error in sonar_to_coords...")
