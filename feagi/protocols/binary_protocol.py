...
class BinaryProtocol(Protocol):
    ...
    def serialize(cls, data: Any, compression_level: int = 0) -> bytes:
        ...
        try:
            ...
            if isinstance(data, dict) or isinstance(data, list) or isinstance(data, str) or isinstance(data, int) or isinstance(data, float) or isinstance(data, bool) or data is None:
            ...
            # Create header
            header = struct.pack(">BBI", format_type, compression_level, len(binary_data))
    ...
    def deserialize(cls, data: bytes) -> Any:
        ...
        try:
            ...
            format_type, compression_level, data_length = struct.unpack(">BBI", data[:6])
            payload = data[6:]
            
            # Check data length
            if len(payload) != data_length:
                logger.warning(f"Data length mismatch: expected {data_length}, got {len(payload)}")
