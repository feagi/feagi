...
class NeuralStreamCompressor:
    ...
    def __init__(self, max_dimensions: Tuple[int, int, int] = (2048, 1080, 64)):
    ...
    def compress_frame(self, cortical_area: str, 
                      neuron_data: List[Tuple[int, int, int, float]]) -> NeuralStreamFrame:
        """Compress neural frame for network transmission."""
        timestamp = time.time()
        
        if not neuron_data:
            return NeuralStreamFrame(timestamp, cortical_area, self.max_dimensions, 
                                   b'', b'', {'neuron_count': 0})
        ...
        for x, y, z, mp in neuron_data:
            if (x < self.max_dimensions[0] and y < self.max_dimensions[1] and 
                z < self.max_dimensions[2]):
        ...
        return NeuralStreamFrame(timestamp, cortical_area, self.max_dimensions,
                               morton_compressed, mp_compressed, metadata)

    def decompress_frame(self, frame: NeuralStreamFrame) -> List[Tuple[int, int, int, float]]:
        ...
        morton_array = np.frombuffer(frame.active_morton_codes, dtype=np.uint64)
        ...
        if len(morton_array) != len(mp_array):
            logger.warning(f"Size mismatch: {len(morton_array)} vs {len(mp_array)}")
            return []
...
if __name__ == "__main__":
    ...
    print(f"   Size: {frame.metadata['original_size']:,} → {frame.metadata['compressed_size']:,} bytes")
    ...
    for sparsity in [0.01, 0.05, 0.1, 0.2]:
        ...
        print(f"   {sparsity*100:2.0f}% active: {perf['compression_ratio']:.1f}x compression, "
