    ...
    def _check_available_modules(self) -> None:
        ...
        if feagi_rust_available:
            try:
                ...
                logger.info(
                    f"Loaded feagi_rust module version {getattr(feagi_rust, '__version__', 'unknown')}"
                )
            ...
            if self.is_available(module_name):
                try:
                    ...
                    raise RustBindingError(
                        f"Error in Rust function {rust_fn_name}: {e}"
                    )
...
matrix_multiply = rust_integration.fallback_to_python(
    "feagi_rust.linear_algebra.matrix_multiply", _py_matrix_multiply
)
...
apply_plasticity = rust_integration.fallback_to_python(
    "feagi_rust.synapses.apply_plasticity", _py_apply_plasticity
)
...
generate_connections = rust_integration.fallback_to_python(
    "feagi_rust.connectivity.generate_connections", _py_generate_connections
)
