...
Usage:
    python kill_feagi.py              # Interactive mode with confirmation
    python kill_feagi.py --force      # Force kill without confirmation
    python kill_feagi.py --no-color   # Disable colored output

Safety Features:
- Excludes this script itself from being killed
- Uses graceful shutdown first (SIGTERM)
- Waits for processes to exit before force killing
...
class Colors:
    ...
    @classmethod
    def disable(cls):
        """Disable all colors (for Windows or when requested)"""
...
def find_feagi_processes() -> List[Tuple[int, str]]:
    ...
    # Define patterns that identify FEAGI processes
    FEAGI_PATTERNS = [
        'feagi/main.py',
        'feagi.main',
        'feagi/api',
...
def confirm_kill(force: bool = False) -> bool:
    ...
    print_colored("Do you want to kill these FEAGI processes? (y/N): ", Colors.YELLOW, end='')
...
def main():
    ...
    if remaining_pids:
        print_colored(f"{len(remaining_pids)} processes still running. Force killing...", Colors.YELLOW)
    ...
    if final_check:
        print_colored(f"Warning: {len(final_check)} FEAGI processes still detected", Colors.YELLOW)
