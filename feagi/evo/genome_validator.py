...
def blueprint_validator(genome):
        ...
        for gene in blueprint:
            ...
            if len(segments) >= 2:
                ...
                for check_gene in blueprint:
                    check_segments = check_gene.split(genome_properties["structure"]["segment_seperator"])
                    if (len(check_segments) >= 4 and 
                        check_segments[1] == cortical_area and 
                        check_segments[3] == "_group"):
                ...
                if not has_group_definition and cortical_area not in invalid_areas:
                    ...
                    for area_type in ["IPU", "OPU", "CORE"]:
                        if area_type in cortical_types and "supported_devices" in cortical_types[area_type]:
                            all_supported_ids.update(cortical_types[area_type]["supported_devices"].keys())
                    ...
                    if potential_fix:
                        logger.warning(f'INVALID CORTICAL AREA ID (LIKELY TYPO): {cortical_area}')
                        logger.warning(f'  → PROBLEM: Cortical area "{cortical_area}" is not defined and appears to be a typo')
                        logger.warning(f'  → SUGGESTED FIX: Change "{cortical_area}" to "{potential_fix}"')
                        logger.warning(f'  → ACTION: Skipping this cortical area and continuing validation')
                    else:
                        logger.warning(f'INVALID CORTICAL AREA ID: {cortical_area}')
                        logger.warning(f'  → PROBLEM: Cortical area "{cortical_area}" has no group definition')
                        logger.warning(f'  → ACTION: Skipping this cortical area and continuing validation')
    ...
    if invalid_areas:
        logger.info(f'Skipped {len(invalid_areas)} invalid cortical area(s): {", ".join(invalid_areas)}')
...
def genome_validator(genome):
    genome_validity = morphology_validator(genome=genome) and \
                      ...
                      validate_physiology_section(genome)["valid"]
...
def auto_correct_ipu_assignments(genome):
    ...
    try:
        ...
        logger.warning("Could not import supported_ipu_list from templates - skipping IPU validation")
        return {
            "genome": genome,
            "corrected_areas": [],
...
def remove_invalid_cortical_areas(genome):
    ...
    for area_id in all_areas:
        if area_id not in area_groups:
            ...
            logger.warning(f"REMOVING INVALID AREA: '{area_id}' has no group definition (likely structural typo)")
...
def sanitize_invalid_morphologies(genome):
    ...
    if physiology_recovery_result["recovery_summary"] != "Physiology section is complete":
    ...
    if cortical_type_correction_result["correction_summary"] != "No cortical area type corrections needed":
    ...
    if ipu_correction_result["correction_summary"] != "No IPU assignment corrections needed":
    ...
    if invalid_ipu_opu_correction_result["correction_summary"] != "No invalid IPU/OPU areas found to correct":
    ...
    if invalid_area_removal_result["removal_summary"] != "No invalid cortical areas found to remove":
    ...
    if "neuron_morphologies" not in genome:
        return {
            ...
            "recovery_summary": f"Auto-recovery: {'; '.join(fixed_references)}" if fixed_references else "Auto-recovery: No issues found to fix",
    ...
    if "blueprint" in genome:
        ...
        try:
            from feagi.evo.templates import cortical_template, cortical_property_mappings, cortical_structural_properties
        except ImportError:
            logger.error("CRITICAL: Cannot import cortical_template and property mappings from templates.py - auto-recovery will fail")
            return {
                "genome": genome,
                "removed_morphologies": removed_morphologies,
                "fixed_references": fixed_references,
                "recovery_summary": f"Auto-recovery: {'; '.join(fixed_references)}" if fixed_references else "Auto-recovery failed: cortical_template and mappings not available",
                "validation_warnings": validation_warnings
            }
        ...
        for cortical_area in cortical_areas:
            for prop_name, prop_pattern in cortical_property_mappings.items():
                ...
                if gene_key not in blueprint:
                    ...
                    default_value = cortical_template.get(prop_name, 0)  # 0 as final fallback
                    blueprint[gene_key] = default_value
                    missing_properties_count += 1
                    fixed_references.append(f"Added missing property {gene_key} = {default_value}")
        ...
        for cortical_area in cortical_areas:
            for prop_name, prop_pattern in cortical_structural_properties.items():
                ...
                if gene_key not in blueprint:
                    ...
                    blueprint[gene_key] = default_value
                    missing_properties_count += 1
                    fixed_references.append(f"Added missing structural property {gene_key} = {default_value}")
        
        if missing_properties_count > 0:
            fixed_references.append(f"Added {missing_properties_count} missing neuron and structural properties with default values from cortical_template")
    ...
    if removed_morphologies:
        recovery_actions.append(f"Removed {len(removed_morphologies)} invalid morphologies: {', '.join(removed_morphologies)}")
    if missing_properties_count > 0:
        recovery_actions.append(f"Added {missing_properties_count} missing neuron and structural properties")
    if len(fixed_references) > len(recovery_actions):
        recovery_actions.append(f"Fixed {len(fixed_references) - len(recovery_actions)} blueprint references")
...
def sanitize_missing_physiology(genome):
    ...
    try:
        from feagi.evo.templates import physiology_template, physiology_property_types
    except ImportError:
        logger.error("CRITICAL: Cannot import physiology_template from templates.py - physiology auto-recovery will fail")
        return {
            "genome": genome,
            "added_properties": [],
    ...
    if "physiology" not in genome:
        logger.info("MISSING PHYSIOLOGY SECTION: Creating physiology section with default values")
        genome["physiology"] = {}
    ...
    for prop_name, default_value in physiology_template.items():
        if prop_name not in physiology:
            ...
            logger.info(f"PHYSIOLOGY RECOVERY: Added missing property '{prop_name}' = {default_value}")
        else:
            ...
            if expected_type == "float" and not isinstance(current_value, (int, float)):
                try:
                    ...
                    physiology[prop_name] = default_value
                    added_properties.append(f"{prop_name} = {default_value} (invalid value corrected)")
...
def validate_physiology_section(genome):
    ...
    if "physiology" not in genome:
        errors.append("MISSING PHYSIOLOGY SECTION: Physiology section is required for FEAGI to function")
        return {
            "valid": False,
            "errors": errors,
    ...
    for prop_name, default_value in physiology_template.items():
        if prop_name not in physiology:
            errors.append(f"MISSING PHYSIOLOGY PROPERTY: '{prop_name}' is required (default: {default_value})")
        else:
            ...
            if expected_type == "float" and not isinstance(current_value, (int, float)):
                errors.append(f"INVALID PHYSIOLOGY TYPE: '{prop_name}' should be a number (float), got {type(current_value).__name__}")
            elif expected_type == "int" and not isinstance(current_value, int):
                errors.append(f"INVALID PHYSIOLOGY TYPE: '{prop_name}' should be an integer, got {type(current_value).__name__}")
            elif expected_type == "bool" and not isinstance(current_value, bool):
                errors.append(f"INVALID PHYSIOLOGY TYPE: '{prop_name}' should be a boolean, got {type(current_value).__name__}")
