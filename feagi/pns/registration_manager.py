...
class AgentRegistrationRequest:
    ...
    def __init__(self, agent_id: str, agent_type: str = "", capabilities: Optional[Dict[str, Any]] = None,
                 agent_data_port: Optional[int] = None, agent_version: str = "",
                 controller_version: str = "", agent_ip: str = "127.0.0.1",
...
class AgentRegistrationResponse:
    ...
    def __init__(self, success: bool, message: str, agent_id: str,
...
class RegistrationManager:
        ...
        with self._lock:
            try:
                ...
                if is_re_registration:
                    ...
                    logger.warning(f"⚠️ Agent '{agent_id}' re-registering - overwriting existing registration")
                    logger.warning(f"    Previous: type={existing_agent.get('agent_type', 'unknown')}, "
                                 f"IP={existing_agent.get('agent_ip', 'unknown')}, "
                                 f"capabilities={list(existing_agent.get('capabilities', {}).keys())}")
                    logger.warning(f"    New: type={request.agent_type}, "
                                 f"IP={request.agent_ip}, "
                                 f"capabilities={list(request.capabilities.keys())}")
                    
                    # Subtract old capability counts before overwriting
                    self._update_capability_counts(existing_agent.get('capabilities', {}), increment=False)
                else:
                ...
                fq_coordination_result = self._coordinate_fq_samplers_for_registration(request.capabilities)
                ...
                if self._state_manager:
                    try:
                        ...
                        logger.error(f"❌ Error calling state manager register_agent: {e}")
                
                # 9. Notify state change listeners
                self._notify_state_change('agent_registered', {
                    'agent_id': agent_id,
                    'agent_type': request.agent_type,
                    'capabilities': request.capabilities,
                ...
                registration_type = "re-registered (overwritten)" if is_re_registration else "registered"
                success_message = f"Agent '{agent_id}' {registration_type} successfully"
                
                logger.info(f"✅ {success_message} - FQ samplers coordinated: {fq_coordination_result}")
            ...
            try:
                ...
                if self._state_manager:
                    try:
                        ...
                        logger.error(f"❌ Error calling state manager deregister_agent: {e}")
                
                # 6. Coordinate FQ samplers based on remaining agents
                fq_coordination_result = self._coordinate_fq_samplers_for_deregistration(agent_capabilities)
    ...
    def get_fq_sampler_coordination_status(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            viz_agents = [aid for aid, adata in self._agents.items() 
                         if self._has_visualization_capabilities(adata.get('capabilities', {}))]
            motor_agents = [aid for aid, adata in self._agents.items() 
    ...
    def register_agent_direct(self, agent_id: str, agent_type: str = "", 
                             capabilities: Optional[Dict[str, Any]] = None,
                             metadata: Optional[Dict[str, Any]] = None) -> AgentRegistrationResponse:
        """
    ...
    def _validate_agent_request(self, request: AgentRegistrationRequest) -> Dict[str, Any]:
        ...
        if not request.agent_id:
            return {'valid': False, 'message': 'Agent ID is required', 'error_code': 'MISSING_AGENT_ID'}
        
        if not request.capabilities:
            return {'valid': False, 'message': 'At least one capability must be specified', 
    ...
    def _update_capability_counts(self, capabilities: Dict[str, Any], increment: bool) -> None:
        ...
        if self._has_visualization_capabilities(capabilities):
            self._capability_counts['visualization'] = max(0, self._capability_counts['visualization'] + delta)
        
        if self._has_motor_capabilities(capabilities):
            self._capability_counts['motor'] = max(0, self._capability_counts['motor'] + delta)
        
        if self._has_sensory_capabilities(capabilities):
            self._capability_counts['sensory'] = max(0, self._capability_counts['sensory'] + delta)

    def _has_visualization_capabilities(self, capabilities: Dict[str, Any]) -> bool:
        """Check if agent has visualization capabilities."""
        return (capabilities.get('visualization', False) or
                capabilities.get('3d_visualization', False) or
                capabilities.get('brain_visualizer', False) or
                capabilities.get('neural_visualization', False))
    ...
    def _has_motor_capabilities(self, capabilities: Dict[str, Any]) -> bool:
        ...
        has_motor_control = (capabilities.get('motor', False) or
                            capabilities.get('motor_control', False) or
                            capabilities.get('actuator_control', False) or
                            capabilities.get('robot_control', False))
        
        # Check for output capabilities, but exclude visualization-only agents
        has_output = capabilities.get('output', False)
        is_visualizer = (capabilities.get('visualization', False) or
                        capabilities.get('3d_visualization', False) or
                        capabilities.get('brain_visualizer', False) or
                        capabilities.get('neural_visualization', False))
        ...
        has_sensorimotor_control = (capabilities.get('sensorimotor', False) and 
                                   not is_visualizer and
                                   (capabilities.get('motor', False) or capabilities.get('motor_control', False)))
    ...
    def _has_sensory_capabilities(self, capabilities: Dict[str, Any]) -> bool:
        ...
        return (capabilities.get('sensory', False) or
                capabilities.get('input', False) or
                capabilities.get('sensorimotor', False) or
                capabilities.get('sensory_input', False))

    def _coordinate_fq_samplers_for_registration(self, capabilities: Dict[str, Any]) -> Dict[str, bool]:
        """Coordinate FQ samplers when agent registers."""
        fq_results = {}
        
        if not self._process_manager:
            logger.warning("⚠️ Process Manager not available - FQ samplers not coordinated")
            return fq_results
    ...
    def _coordinate_fq_samplers_for_deregistration(self, capabilities: Dict[str, Any]) -> Dict[str, bool]:
    ...
    def _update_state_manager(self) -> None:
        ...
        if self._state_manager:
            try:
                ...
                logger.debug(f"🔄 State Manager updated: {total_agents} agents registered")
            except Exception as e:
...
def create_registration_manager(state_manager=None, process_manager=None) -> RegistrationManager:
    """
    ...
    with _registration_manager_lock:
        if _registration_manager_instance is not None:
            logger.warning("🏛️ Registration Manager already exists - returning existing instance")
            return _registration_manager_instance
