"""ZMQ Server for FEAGI.

This module provides a minimal ZMQ-based messaging server for FEAGI
for efficient communication between components.
"""
import json
import logging
import threading
import sys
import os
import importlib.util
from typing import Dict, List, Optional, Any, Callable, Union

# Configure logging
logger = logging.getLogger(__name__)

# Define ZMQ_AVAILABLE before any imports
ZMQ_AVAILABLE = False

# Try to import ZMQ with detailed diagnostics
try:
    # First check if pyzmq is properly installed
    import pkg_resources
    zmq_version = pkg_resources.get_distribution("pyzmq").version
    logger.info(f"PyZMQ version: {zmq_version}")
    
    # Try direct import
    import zmq
    logger.info(f"ZMQ imported from: {getattr(zmq, '__file__', 'unknown')}")
    
    # Verify Context exists
    if hasattr(zmq, 'Context'):
        ZMQ_AVAILABLE = True
        logger.info(f"ZMQ.Context is available: {zmq.Context}")
    else:
        # Try to get Context by alternate methods
        logger.warning("ZMQ Context not found in main module, trying alternate methods")
        
        # Method 1: Import Context directly
        try:
            from zmq import Context
            zmq.Context = Context
            ZMQ_AVAILABLE = True
            logger.info("Successfully imported Context from zmq module directly")
        except (ImportError, AttributeError) as e:
            logger.warning(f"Failed to import Context directly: {e}")
            
            # Method 2: Try importing from zmq.sugar module
            try:
                from zmq.sugar.context import Context
                zmq.Context = Context
                ZMQ_AVAILABLE = True
                logger.info("Successfully imported Context from zmq.sugar.context")
            except (ImportError, AttributeError) as e:
                logger.warning(f"Failed to import Context from zmq.sugar: {e}")
                
                # Method 3: Check zmq.core or other locations
                if hasattr(zmq, 'core') and hasattr(zmq.core, 'context') and hasattr(zmq.core.context, 'Context'):
                    zmq.Context = zmq.core.context.Context
                    ZMQ_AVAILABLE = True
                    logger.info("Successfully found Context in zmq.core.context")
                
except ImportError as imp_err:
    import warnings
    logger.error(f"PyZMQ is not installed: {imp_err}")
    warnings.warn("PyZMQ is not installed. ZMQ server will not function.")
except Exception as e:
    logger.error(f"Error importing ZMQ: {e}")
    import warnings
    warnings.warn(f"Error importing ZMQ: {e}")

# Create minimal stub if ZMQ is not properly available
if not ZMQ_AVAILABLE:
    logger.warning("Creating stub ZMQ implementation as fallback")
    class _StubZMQ:
        PUB = 1
        SUB = 2
        RCVTIMEO = 3
        SUBSCRIBE = 4
        
        class Context:
            def __init__(self):
                logger.warning("Using stub ZMQ Context")
                pass
                
            def socket(self, socket_type):
                logger.warning("Using stub ZMQ socket")
                return _StubSocket()
                
            def term(self):
                pass
                
    class _StubSocket:
        def bind(self, address):
            logger.warning(f"Stub socket bind called with: {address}")
            
        def connect(self, address):
            logger.warning(f"Stub socket connect called with: {address}")
            
        def close(self):
            pass
            
        def setsockopt(self, option, value):
            pass
            
        def setsockopt_string(self, option, value):
            pass
            
        def send_string(self, message):
            logger.warning(f"Stub socket send_string called with: {message}")
            
        def recv_string(self):
            raise Exception("Stub socket cannot receive messages")
    
    # If ZMQ isn't available properly, use the stub
    if 'zmq' not in locals():
        zmq = _StubZMQ()
    else:
        # Add missing attributes to real zmq module
        if not hasattr(zmq, 'Context'):
            zmq.Context = _StubZMQ.Context

class ZMQServer:
    """
    Minimal ZMQ server for FEAGI.
    
    Provides functionality for:
    1. Publishing messages to topics
    2. Subscribing to topics with callbacks
    """
    
    def __init__(
        self,
        host: str = None,
        pub_port: int = None,
        sub_port: int = None,
        topics: List[str] = None,
        logger: Optional[logging.Logger] = None,
    ) -> None:
        """
        Initialize a ZMQ server.
        
        Args:
            host: The host to bind to
            pub_port: Port for the publisher socket
            sub_port: Port for the subscriber socket
            topics: List of topics to subscribe to
            logger: Optional logger instance
        """
        # Get configuration from environment variables if not provided
        self.host = host or os.environ.get("FEAGI_ZMQ_HOST", "127.0.0.1")
        self.pub_port = pub_port or int(os.environ.get("FEAGI_ZMQ_PUB_PORT", "5556"))
        self.sub_port = sub_port or int(os.environ.get("FEAGI_ZMQ_SUB_PORT", "5557"))
        
        # Get topics from environment if not provided
        if topics is None:
            env_topics = os.environ.get("FEAGI_ZMQ_TOPICS", "neural,metrics,heartbeat")
            self.topics = env_topics.split(",") if env_topics else []
        else:
            self.topics = topics
            
        self.logger = logger or logging.getLogger(__name__)
        
        # State tracking
        self.running = False
        self.subscribers = {}  # topic -> socket
        self.subscriber_threads = {}  # topic -> thread
        self.callbacks = {topic: [] for topic in self.topics}
        
        # ZMQ setup - initialize everything to None
        self.context = None
        self.publisher = None
        
        # Initialize ZMQ if available
        if ZMQ_AVAILABLE:
            try:
                self.logger.info("Creating ZMQ Context...")
                self.context = zmq.Context()
                self.logger.info("Creating ZMQ Publisher socket...")
                self.publisher = self.context.socket(zmq.PUB)
                self.logger.info("ZMQ context and publisher initialized")
            except Exception as e:
                self.logger.error(f"Error initializing ZMQ: {str(e)}")
                # Try to salvage if possible
                if self.context is None and hasattr(zmq, 'Context'):
                    try:
                        self.logger.info("Retrying Context creation...")
                        self.context = zmq.Context()
                    except:
                        pass
        else:
            self.logger.error("ZMQ is not available. Server will not function.")
    
    def start(self) -> bool:
        """
        Start the ZMQ server.
        
        Returns:
            bool: True if started successfully
        """
        if self.running:
            self.logger.warning("ZMQ server already running")
            return True
            
        if not ZMQ_AVAILABLE:
            self.logger.error("ZMQ is not available")
            return False
            
        if self.context is None:
            self.logger.error("ZMQ context initialization failed")
            return False
            
        if self.publisher is None:
            self.logger.error("ZMQ publisher initialization failed")
            return False
            
        try:
            self.logger.info(f"Starting ZMQ server on {self.host}")
            self.logger.info(f"PUB port: {self.pub_port}, SUB port: {self.sub_port}")
            
            # Bind publisher socket
            try:
                pub_address = f"tcp://{self.host}:{self.pub_port}"
                self.publisher.bind(pub_address)
                self.logger.info(f"Publisher bound to {pub_address}")
            except Exception as e:
                self.logger.error(f"Failed to bind publisher socket: {e}")
                return False
                    
            # Initialize subscribers for topics
            for topic in self.topics:
                self._create_subscriber(topic)
                    
            self.running = True
            self.logger.info("ZMQ server started successfully.")
            return True
        except Exception as e:
            self.logger.error(f"Error starting ZMQ server: {e}")
            self.shutdown()
            return False
    
    def publish(self, topic: str, message: Union[str, dict, list, Any]) -> bool:
        """
        Publish a message to a topic.
        
        Args:
            topic: Topic to publish to
            message: Message to publish (will be JSON serialized if not a string)
            
        Returns:
            bool: True if published successfully
        """
        if not self.running:
            self.logger.error("Cannot publish: ZMQ server not running")
            return False
            
        if self.publisher is None:
            self.logger.error("Cannot publish: Publisher not available")
            return False
            
        try:
            # Convert to JSON if needed
            if not isinstance(message, str):
                message = json.dumps(message)
                
            # Publish message
            self.publisher.send_string(f"{topic} {message}")
            return True
        except Exception as e:
            self.logger.error(f"Error publishing to {topic}: {e}")
            return False
    
    def subscribe(self, topic: str, callback: Callable[[str, Any], None]) -> bool:
        """
        Subscribe to a topic with a callback.
        
        Args:
            topic: Topic to subscribe to
            callback: Function to call when a message is received
            
        Returns:
            bool: True if subscribed successfully
        """
        if not self.running:
            self.logger.error("Cannot subscribe: ZMQ server not running")
            return False
            
        # Add callback to the list
        if topic not in self.callbacks:
            self.callbacks[topic] = []
            
        self.callbacks[topic].append(callback)
        
        # Create subscriber if it doesn't exist
        if topic not in self.subscribers:
            success = self._create_subscriber(topic)
            if not success:
                self.logger.error(f"Failed to create subscriber for {topic}")
                return False
                
        return True
    
    def _create_subscriber(self, topic: str) -> bool:
        """
        Create a subscriber for a topic.
        
        Args:
            topic: Topic to subscribe to
            
        Returns:
            bool: True if created successfully
        """
        if not ZMQ_AVAILABLE or self.context is None:
            return False
            
        try:
            # Create subscriber socket
            subscriber = self.context.socket(zmq.SUB)
            sub_address = f"tcp://{self.host}:{self.sub_port}"
            subscriber.connect(sub_address)
            subscriber.setsockopt_string(zmq.SUBSCRIBE, topic)
            
            # Store subscriber
            self.subscribers[topic] = subscriber
            self.logger.info(f"Created subscriber for topic: {topic}")
            
            # Start subscriber thread
            thread = threading.Thread(
                target=self._subscriber_loop,
                args=(topic, subscriber),
                daemon=True
            )
            thread.start()
            
            self.subscriber_threads[topic] = thread
            self.logger.info(f"Started subscriber thread for topic: {topic}")
            return True
        except Exception as e:
            self.logger.error(f"Error creating subscriber for {topic}: {e}")
            return False
    
    def _subscriber_loop(self, topic: str, subscriber) -> None:
        """
        Main subscriber loop for receiving messages.
        
        Args:
            topic: Topic this subscriber is for
            subscriber: ZMQ subscriber socket
        """
        self.logger.info(f"Subscriber loop started for topic: {topic}")
        
        try:
            subscriber.setsockopt(zmq.RCVTIMEO, 1000)  # 1 second timeout
            
            while self.running:
                try:
                    # Receive message with timeout
                    message = subscriber.recv_string()
                    
                    # Parse and dispatch message
                    topic_str, payload = message.split(' ', 1)
                    
                    # Process callbacks
                    for callback in self.callbacks.get(topic, []):
                        try:
                            # Try parsing as JSON, fall back to raw string
                            try:
                                data = json.loads(payload)
                            except:
                                data = payload
                                
                            callback(topic_str, data)
                        except Exception as e:
                            self.logger.error(f"Error in callback for {topic}: {e}")
                except zmq.Again:
                    # Timeout - normal case for polling
                    pass
                except Exception as e:
                    if self.running:  # Only log if we're still supposed to be running
                        self.logger.error(f"Error in subscriber loop for {topic}: {e}")
        finally:
            self.logger.info(f"Subscriber loop ended for topic: {topic}")
    
    def shutdown(self) -> None:
        """
        Shut down the ZMQ server.
        """
        if not self.running:
            return
            
        self.logger.info("Shutting down ZMQ server...")
        self.running = False
        
        # Close all subscribers
        for topic, subscriber in self.subscribers.items():
            try:
                subscriber.close()
                self.logger.info(f"Closed subscriber for topic: {topic}")
            except:
                pass
                
        # Close publisher
        if self.publisher:
            try:
                self.publisher.close()
                self.logger.info("Closed publisher socket")
            except:
                pass
                
        # Terminate context
        if self.context:
            try:
                self.context.term()
                self.logger.info("Terminated ZMQ context")
            except:
                pass
                
        self.logger.info("ZMQ server shutdown complete.")
    
    def is_healthy(self) -> bool:
        """
        Check if the ZMQ server is healthy.
        
        Returns:
            bool: True if healthy
        """
        return self.running and self.publisher is not None 