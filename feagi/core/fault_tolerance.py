...
class HealthMonitor:
    ...
    def register_process(
        ...
        with self._lock:
            ...
            logger.info(
                f"Registered process {process_name} (PID: {pid}) for health monitoring"
            )
    ...
    def _check_resource_thresholds(self, process_name: str) -> None:
        ...
        if cpu_usage >= health.cpu_warning_threshold:
            if current_state != ProcessState.CRITICAL:
                ...
                logger.warning(
                    f"Process {process_name} in CRITICAL state: high CPU usage ({cpu_usage:.1%})"
                )
        ...
        # Check for WARNING condition
            ...
            if current_state == ProcessState.RUNNING:
                health.state = ProcessState.WARNING
                logger.warning(
                    f"Process {process_name} in WARNING state: elevated CPU usage ({cpu_usage:.1%})"
                )
        ...
        with self._lock:
            ...
            for process_name, health in list(self.monitored_processes.items()):
                ...
                if time_since_heartbeat >= health.heartbeat_timeout:
                    ...
                    logger.error(
                        f"Process {process_name} failed: missing heartbeat for {time_since_heartbeat:.1f}s"
                    )
                    ...
                    if health.state == ProcessState.RUNNING:
                        health.state = ProcessState.WARNING
                        logger.warning(
                            f"Process {process_name} warning: delayed heartbeat for {time_since_heartbeat:.1f}s"
                        )
                        self._handle_warning_process(process_name)
                ...
                if self.resource_manager:
                    ...
                    if not alive and health.state != ProcessState.FAILED:
                        health.state = ProcessState.FAILED
                        logger.error(
                            f"Process {process_name} failed: process not running"
                        )
                        self._handle_failed_process(process_name)
    ...
    def _handle_failed_process(self, process_name: str) -> None:
        ...
        with self._lock:
            ...
            if health.should_restart() and self.resource_manager:
                ...
                # Log restart attempt
                logger.info(
                    f"Attempting to restart process {process_name} (attempt {health.restart_count}/{health.max_restarts})"
                )
                ...
                if success:
                    logger.info(
                        f"Successfully initiated restart of process {process_name}"
                    )
                else:
                ...
                logger.warning(
                    f"Process {process_name} failed but will not be restarted (policy: {health.restart_policy.name})"
                )
