...
class CPUAllocator:
    ...
    def __init__(self, total_cores: Optional[int] = None):
        ...
        self.priority_core_pools: Dict[int, List[int]] = self._initialize_priority_pools()
    ...
    def _detect_physical_cores(self) -> List[int]:
        ...
        try:
            ...
            if hasattr(psutil, "cpu_info") and hasattr(psutil.cpu_info(), "physical_core"):
    ...
    def allocate_cores(
        ...
        with self._lock:
            ...
            available_cores = [c for c in self.priority_core_pools[priority] if c not in self.reserved_cores]
            ...
            if len(available_cores) < num_cores:
                ...
                while lower_priority <= 3 and len(available_cores) < num_cores:
                    additional_cores = [c for c in self.priority_core_pools[lower_priority] 
            ...
            if len(available_cores) < num_cores and priority > 1:
                ...
                while higher_priority >= 1 and len(available_cores) < num_cores:
                    additional_cores = [c for c in self.priority_core_pools[higher_priority] 
                                       if c not in self.reserved_cores]
                    available_cores.extend(additional_cores)
                    higher_priority -= 1
            ...
            logger.info(f"Allocated {len(allocated_cores)} cores to {process_name} (priority {priority})")
    ...
    def rebalance_allocations(self) -> Dict[str, List[int]]:
        ...
        with self._lock:
            ...
            for priority in [1, 2, 3]:
                ...
                high_util = [p for p in processes if p.utilization >= high_util_threshold]
                low_util = [p for p in processes if p.utilization <= low_util_threshold and len(p.core_ids) > 1]
                ...
                if high_util and low_util:
                    ...
                    for high_process in high_util:
                        ...
                        # Record changes
                        changes[high_process.process_name] = high_process.core_ids.copy()
                        changes[low_process.process_name] = low_process.core_ids.copy()
                        
                        logger.info(f"Rebalanced: moved core {core_to_move} from {low_process.process_name} to {high_process.process_name}")
