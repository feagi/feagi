...
class AtomicU32:
    ...
    def __init__(self, initial: int = 0):
        ...
        if not (0 <= initial <= 0xFFFFFFFF):
            raise ValueError(f"AtomicU32 value must be 0 to 2^32-1, got {initial}")
    ...
    def store(self, value: int) -> None:
        ...
        if not (0 <= value <= 0xFFFFFFFF):
            raise ValueError(f"AtomicU32 value must be 0 to 2^32-1, got {value}")
...
class AtomicU64:
    ...
    def __init__(self, initial: int = 0):
        ...
        if not (0 <= initial <= 0xFFFFFFFFFFFFFFFF):
            raise ValueError(f"AtomicU64 value must be 0 to 2^64-1, got {initial}")
    ...
    def store(self, value: int) -> None:
        ...
        if not (0 <= value <= 0xFFFFFFFFFFFFFFFF):
            raise ValueError(f"AtomicU64 value must be 0 to 2^64-1, got {value}")
