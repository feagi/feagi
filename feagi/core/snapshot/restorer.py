...
def restore_brain_snapshot(snapshot_root: Path, snapshot_id: str, state_manager) -> bool:
    """
    ...
    if not snap_dir.exists() or not manifest_path.exists():
        raise SnapshotRestoreError(f"Snapshot not found or missing manifest: {snap_dir}")
