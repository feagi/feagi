...
class ProcessInfo(RustCompatible):
    ...
    last_heartbeat: float = rust_field(
        ownership=OwnershipType.MUTABLE_BORROWED, thread_safe=True
    )
...
class ResourceManager:
    ...
    def _get_available_memory(self) -> int:
        ...
        try:
            ...
            self.logger.warning(
                "psutil not available. Memory detection will be limited."
            )
            return 0
    ...
    def _allocate_resources(
        ...
        with self._lock:
            ...
            self.logger.info(
                f"Allocated CPU cores {allocated_cores} for {process_name}"
            )
    ...
    def terminate_process(self, name: str) -> bool:
        ...
        with self._lock:
            ...
            try:
                ...
                if process.is_alive():
                    self.logger.warning(
                        f"Process {name} did not terminate gracefully. Killing it."
                    )
                    process.kill()
                ...
                with self._lock:
                    ...
                    for name, proc_ref in list(self.processes.items()):
                        if proc_ref and proc_ref():
                            ...
                            self.logger.warning(
                                f"Process reference for {name} is invalid."
                            )
                            self._cleanup_process_resources(name)
                        ...
                        if name in self.process_info:
                            ...
                            if (
                                ...
                                self.logger.warning(
                                    f"Process {name} heartbeat timeout. Marking as dead."
                                )
                                self._cleanup_process_resources(name)
