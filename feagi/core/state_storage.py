    ...
    def load_state(self) -> Result[RustCompatibleState]:
        ...
        try:
            ...
            if len(data) < ctypes.sizeof(RustCompatibleState):
                ...
                logger.warning(
                    f"State file {self._file_path} too small, using default state"
                )
                return Result.ok(RustCompatibleState())
            ...
            if not state.validate_invariants():
                logger.warning(
                    f"Invalid state loaded from {self._file_path}, using default"
                )
                return Result.ok(RustCompatibleState())
            ...
            logger.error(
                f"Error loading state from file {self._file_path}: {e}"
            )
            return Result.err(StateError.STORAGE_FAILURE)
