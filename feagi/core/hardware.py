...
def get_cpu_info() -> Dict[str, Any]:
    ...
    info["has_sse4"] = any(f in info["features"] for f in ["sse4", "sse4.1", "sse4.2", "SSE4", "SSE4.1", "SSE4.2"])
        ...
        if "Metal" in output:
            ...
            for line in output.split("\n"):
                ...
                if current_device and "VRAM" in line:
                    ...
                    if "MB" in memory_str:
                        current_device["memory"] = int(memory_str.split()[0]) * 1024 * 1024
                    elif "GB" in memory_str:
                        current_device["memory"] = int(memory_str.split()[0]) * 1024 * 1024 * 1024
...
def get_webgpu_info() -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            adapter = wgpu.request_adapter(power_preference=wgpu.PowerPreference.HighPerformance)
            if adapter:
                webgpu_info["adapter"] = {
                    "name": adapter.name,
                ...
                for feature in dir(wgpu.FeatureName):
                    if not feature.startswith("_") and adapter.has_feature(getattr(wgpu.FeatureName, feature)):
