...
class ConnectomeManagerWebGPU:
    ...
    def __init__(self, connectome_manager_gpu):
        ...
        if not success:
            logger.warning(
                "WebGPU initialization failed. Falling back to CPU implementation."
            )
    ...
    def _allocate_buffers(self):
        ...
        buffer_sizes = {
            ...
            "is_active": max_neurons
            * 4,  # u32 array (using u32 instead of bool for alignment)
            "valid_mask": max_neurons
            * 4,  # u32 array (using u32 instead of bool for alignment)
        ...
        for name, size in buffer_sizes.items():
            ...
            aligned_size = (
                (size + MEMORY_ALIGNMENT - 1) // MEMORY_ALIGNMENT
            ) * MEMORY_ALIGNMENT
    ...
    def update_synapse_data(self):
        ...
        if "row_indices" not in self.buffers:
            self.buffers["row_indices"] = self.device.create_buffer(
                size=(sparse_matrix.shape[0] + 1)
                * 4,  # u32 array, length = num_rows + 1
        ...
        self.device.queue.write_buffer(
            self.buffers["row_indices"], 0, sparse_matrix.indptr.astype(np.uint32)
        )
        self.device.queue.write_buffer(
            self.buffers["col_indices"], 0, sparse_matrix.indices.astype(np.uint32)
        )
        self.device.queue.write_buffer(
            self.buffers["weights"], 0, sparse_matrix.data.astype(np.float32)
        )
    ...
    def _create_compute_pipelines(self):
        ...
        self.pipelines["neuron_update"] = self.device.create_compute_pipeline(
            ...
            compute={
                "module": self._shader_modules["neuron_operations"],
        ...
        self.pipelines["synapse_propagation"] = self.device.create_compute_pipeline(
            ...
            compute={
                "module": self._shader_modules["synapse_operations"],
