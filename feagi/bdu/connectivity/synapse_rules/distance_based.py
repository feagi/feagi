...
"""
Distance-based synapse rule implementation.

This module provides the DistanceBasedRule class for creating
connections based on spatial distance between neurons.
"""
...
class DistanceBasedRule(SynapseRule):
    """
    Rule that creates connections based on spatial distance between neurons.

    This rule connects neurons that are within a certain distance of each other,
    with connection probability decreasing as distance increases.
    """
    ...
    def generate_connections(
        ...
        # For each source neuron, find target neurons within distance
        for source_id in source_neurons:
            ...
            # Find all target neurons within maximum distance
            for target_id in target_neurons:
                ...
                if distance <= max_distance:
                    ...
                    probability = 1.0 / (1.0 + distance / distance_falloff)
                    candidates.append((target_id, probability))

            # Randomly select connections based on probabilities
            source_connections = []
            for target_id, probability in candidates:
