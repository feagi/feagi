...
def define_subregions(area_id: AreaId, 
                      parameters: Dict[str, Any], 
                      cortical_dimensions: Position) -> Set[BoundingBox]:
    """
                    ...
                    for y_i in range(pattern[1][0]):
                        while seed_pointer[2] <= depth:
                            ...
                            for z_i in range(pattern[2][0]):
                                if seed_pointer[0] + seed[0] <= width and \
                                        seed_pointer[1] + seed[1] <= height and \
                                        seed_pointer[2] + seed[2] <= depth:
...
def find_source_coordinates(src_pattern: List[Any], 
                           src_cortical_boundary: Position) -> Generator[Position, None, None]:
    """
...
def find_destination_coordinates(dst_cortical_boundary: Position,
                               src_coordinate: Position,
                               src_pattern: List[Any],
                               dst_pattern: List[Any]) -> Generator[Position, None, None]:
    """
...
def syn_reducer_x(src_area_id: AreaId, 
    ...
    for dst_x_index in range(dst_cortical_dim_x):
        if dst_x_index < len(src_neuron_bin_str) and int(src_neuron_bin_str[dst_x_index]):
...
def syn_lateral_pairs_x(neuron_id: NeuronId, 
                     area_id: AreaId, 
                     src_subregion: BoundingBox,
                     connectome_manager) -> Optional[Position]:
    """
...
def syn_block_connection(src_area_id: AreaId, 
                        dst_area_id: AreaId, 
                        src_neuron_id: NeuronId, 
                        src_subregion: BoundingBox,
                        connectome_manager,
                        scaling_factor: int = 10) -> Position:
    """
    ...
    if src_cortical_dim_x != dst_cortical_dim_x * scaling_factor:
        logger.warning(f"Areas {src_area_id} and {dst_area_id} don't have matching blocks for synaptogenesis")
    ...
    return (neuron_block_index_x // scaling_factor, 
            neuron_block_index_y, 
            neuron_block_index_z)


def syn_projector(src_area_id: AreaId, 
                 dst_area_id: AreaId, 
                 src_neuron_id: NeuronId, 
                 src_subregion: BoundingBox,
                 connectome_manager,
                 transpose: Optional[Tuple[str, str, str]] = None,
                 project_last_layer_of: Optional[str] = None) -> List[Position]:
    """
        ...
        for x in dst_vox_dict[0]:
            for y in dst_vox_dict[1]:
                for z in dst_vox_dict[2]:
                    ...
                    if (0 <= x < dst_dimensions[0] and 
                        0 <= y < dst_dimensions[1] and 
                        0 <= z < dst_dimensions[2]):
                        candidate_list.append((x, y, z))
    
    return candidate_list 


def syn_memory(src_area_id: AreaId, dst_area_id: AreaId, memory_register: Dict[AreaId, Set[AreaId]]) -> None:
    """
...
def neighbor_finder_extended(src_area_id: AreaId,
    ...
    try:
        ...
        if morphology_type == RuleType.VECTORS.value:
            for vector in morphologies_registry[neuron_morphology]["parameters"]["vectors"]:
