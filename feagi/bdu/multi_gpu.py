...
class MultiGPUConfig:
    ...
    def __init__(self, 
        ...
        if isinstance(partition_method, str):
            try:
                ...
                logger.warning(f"Unknown partition method: {partition_method}. Using CORTICAL_AREAS.")
        ...
        if len(self.device_ids) > self.num_devices:
            logger.warning(f"More device IDs provided than available devices. Using only the first {self.num_devices}.")
    ...
    def _validate_config(self):
        ...
        if self.num_devices < 2:
            logger.warning("Fewer than 2 devices available. Disabling multi-GPU operation.")
            self.enabled = False
...
class BrainPartition:
    ...
    def __init__(self, 
        ...
        self.boundary_outgoing: Dict[int, List[Tuple[int, float]]] = {}  # source_id -> [(target_id, weight), ...]
        self.boundary_incoming: Dict[int, List[Tuple[int, float]]] = {}  # target_id -> [(source_id, weight), ...]
    ...
    def _initialize_device(self):
        ...
        if self.backend_type == BackendType.PYTORCH and TORCH_AVAILABLE:
            ...
            if torch.cuda.is_available() and self.device_id < torch.cuda.device_count():
                ...
                logger.info(f"Partition {self.partition_id} using CUDA device {self.device_id}: {torch.cuda.get_device_name(self.device_id)}")
            else:
            ...
            logger.info(f"Partition {self.partition_id} using CUDA device {self.device_id}: {cp.cuda.runtime.getDeviceProperties(self.device_id)['name'].decode()}")
        elif self.backend_type == BackendType.WEBGPU and WGPU_AVAILABLE:
            # No explicit device selection for WebGPU yet
            self.adapter = wgpu.request_adapter()
            self.device = self.adapter.request_device()
            logger.info(f"Partition {self.partition_id} using WebGPU device: {self.adapter.request_adapter_info().description}")
        else:
            # CPU fallback
            self.device = "cpu"
            logger.info(f"Partition {self.partition_id} using CPU")
    ...
    def add_boundary_connection(self, source_id: int, target_id: int, weight: float):
        ...
        self.metrics["boundary_connections"] = sum(len(targets) for targets in self.boundary_outgoing.values())
...
class MultiGPUManager:
    ...
    def initialize(self, connectome):
        ...
        logger.info(f"Initialized multi-GPU manager with {len(self.partitions)} partitions")
    ...
    def _partition_grid(self):
        ...
        min_coords = [min(pos[i] for pos in neuron_positions.values()) for i in range(3)]
        max_coords = [max(pos[i] for pos in neuron_positions.values()) for i in range(3)]
