...
class CacheAlignedArray:
    ...
    def __init__(self, size: int, dtype: np.dtype, alignment: int = MEMORY_ALIGNMENT):
        ...
        oversized_elements = (
            (aligned_bytes + element_size - 1) // element_size
            + alignment
            + safety_margin
        )
...
class BlockSparseMatrix:
    ...
    def __setitem__(self, key: Tuple[int, int], value: float):
        """Set matrix element."""
        row, col = key
        block_row, block_col, in_block_row, in_block_col = self.get_block_coords(
            row, col
        )
                ...
                if br == block_row:
                    ...
                    if len(nonzero_cols) > 0:
                        ...
                        if np.any(valid_targets_mask):
                            ...
                            valid_weights = block[in_block_row, nonzero_cols][
                                valid_targets_mask
                            ]
...
if NUMBA_AVAILABLE:
    ...
    def simd_membrane_decay(
        potentials: np.ndarray, decay_rates: np.ndarray, valid_mask: np.ndarray
    ) -> None:
    ...
    @njit(parallel=True, fastmath=True)
    def simd_synaptic_integration(
        targets: np.ndarray, weights: np.ndarray, potentials: np.ndarray
    ) -> None:
...
class NeuronArray:
    ...
    def _init_optimized_backend(self, backend: Optional[str]):
        ...
        self._aligned_resting_potentials = CacheAlignedArray(
            self.aligned_capacity, np.float32
        )
        ...
        self._aligned_refractory_periods = CacheAlignedArray(
            self.aligned_capacity, np.int32
        )
        self.refractory_periods = self._aligned_refractory_periods.array

        self._aligned_refractory_counters = CacheAlignedArray(
            self.aligned_capacity, np.int32
        )
        self.refractory_counters = self._aligned_refractory_counters.array

        # Coordinate arrays (cache-aligned SoA)
        self._aligned_coordinates_x = CacheAlignedArray(
            self.aligned_capacity, np.uint32
        )
        self.coordinates_x = self._aligned_coordinates_x.array

        self._aligned_coordinates_y = CacheAlignedArray(
            self.aligned_capacity, np.uint32
        )
        self.coordinates_y = self._aligned_coordinates_y.array

        self._aligned_coordinates_z = CacheAlignedArray(
            self.aligned_capacity, np.uint32
        )
        ...
        self._aligned_temp_fired_mask = CacheAlignedArray(
            self.aligned_capacity, np.bool_
        )
        self._temp_fired_mask = self._aligned_temp_fired_mask.array

        self._aligned_temp_targets = CacheAlignedArray(
            self.aligned_capacity * 10, np.int32
        )
        self._temp_targets = self._aligned_temp_targets.array  # Pool for targets

        self._aligned_temp_weights = CacheAlignedArray(
            self.aligned_capacity * 10, np.float32
        )
        ...
        logger.info(
            f"Initialized optimized NeuronArray: {self.backend_type} backend, "
    ...
    def batch_update_coordinates(
        self, neuron_ids: List[int], coordinates: List[Tuple[int, int, int]]
    ) -> None:
        ...
        if self._use_rust:
            ...
            # Convert to indices
            indices = [
                self.id_to_index_map[nid]
                for nid in neuron_ids
                if nid in self.id_to_index_map
            ]
            if not indices:
                return
            ...
            x_vals = np.array(
                [max(0, coord[0]) for coord in coordinates], dtype=np.uint32
            )
            y_vals = np.array(
                [max(0, coord[1]) for coord in coordinates], dtype=np.uint32
            )
            z_vals = np.array(
                [max(0, coord[2]) for coord in coordinates], dtype=np.uint32
            )
    ...
    def simd_optimized_find_fire_candidates(self, timestep: int) -> List[int]:
        ...
        if self._use_rust:
            ...
            fire_mask = (
                (potentials >= thresholds) & (refractory == 0) & valid & (enabled > 0)
            )
    ...
    def embedded_synaptic_integration(
        self, fired_neurons: List[int], connectivity_matrix
    ) -> None:
        ...
        if isinstance(connectivity_matrix, BlockSparseMatrix):
            ...
            for targets, weights in connections:
                ...
                if target_count + batch_size <= len(self._temp_targets):
                    self._temp_targets[target_count : target_count + batch_size] = (
                        targets
                    )
                    self._temp_weights[target_count : target_count + batch_size] = (
                        weights
                    )
    ...
    def delete_neuron(self, neuron_id: int) -> bool:
        ...
        if not self._use_rust:
            # CRITICAL FIX: Mark as both invalid AND inactive for vectorized queries
            valid_mask_np = self.backend.to_numpy(self.valid_mask)
            valid_mask_np[index] = False
            self.valid_mask = self.backend.array(valid_mask_np)

            is_active_np = self.backend.to_numpy(self.is_active)
            is_active_np[index] = False
            self.is_active = self.backend.array(is_active_np)

            # Reset neuron properties to defaults to prevent corruption
            potentials = self.backend.to_numpy(self.membrane_potentials)
            potentials[index] = 0.0
            self.membrane_potentials = self.backend.array(potentials)

            # Clear cortical assignment to prevent lookup issues
            cortical_idxs_np = self.backend.to_numpy(self.cortical_idxs)
            cortical_idxs_np[index] = -1  # Invalid cortical area
            self.cortical_idxs = self.backend.array(cortical_idxs_np)
    ...
    def get_neuron_property(self, neuron_id: int, property_name: str) -> Any:
        ...
        if property_name == "membrane_potential":
            ...
            return (
                int(np.uint32(self.coordinates_x[index])),
    ...
    def get_neurons_by_cortical_area(self, cortical_id: int) -> List[int]:
        ...
        if len(valid_indices) > 0:
            ...
            if (
                hasattr(self, "_index_to_id_lookup_array")
                and self._index_to_id_lookup_array is not None
            ):
                ...
                neuron_ids = [
                    self.index_to_id_map.get(int(idx), -1) for idx in valid_indices
                ]
                neuron_ids = [nid for nid in neuron_ids if nid != -1]
    ...
    def _legacy_synaptic_integration(self, synapse_indices, synapse_data):
        ...
        if isinstance(synapse_data, torch.Tensor):
            ...
            for idx in synapse_indices:
                ...
                try:
                    ...
                    if targets.dim() == 0 and targets.numel() > 0:
                        ...
                        targets.dim() > 1
                    ):  # CRITICAL: Handle multi-dimensional nonzero results
                        targets = (
                            targets.squeeze(-1)
                    ...
                    if (
                        update_mask.shape[0] != targets.shape[0]
                        or update_mask.shape[0] != weights.shape[0]
                    ):
                        logger.warning(
                            f"BURST ENGINE: Tensor shape mismatch avoided - update_mask: {update_mask.shape}, targets: {targets.shape}, weights: {weights.shape}"
                        )
                        continue
                    ...
                    if update_mask.any():
                        ...
                        if (
                            valid_targets.shape[0] == valid_weights.shape[0]
                    ...
                    logger.warning(
                        f"BURST ENGINE: Synaptic integration error caught for neuron {idx}: {e}"
                    )
                    continue
        else:
            ...
            for idx in synapse_indices:
                ...
                if np.any(update_mask):
                    self.membrane_potentials[targets[update_mask]] += weights[
                        update_mask
                    ]
        ...
        if is_torch:
            self.membrane_potentials[valid_not_fired] *= self.decay_rates[
                valid_not_fired
            ]
        else:
            self.membrane_potentials[valid_not_fired] *= self.decay_rates[
    ...
    def decay_and_check_firing(self):
        ...
        if isinstance(self.membrane_potentials, torch.Tensor):
            self.membrane_potentials[can_update_mask] = (
                self.membrane_potentials[can_update_mask]
                * (1.0 - self.decay_rates[can_update_mask])
                + self.resting_potentials[can_update_mask]
                * self.decay_rates[can_update_mask]
            )
        else:
            decay_effect = self.membrane_potentials[can_update_mask] * (
                1.0 - self.decay_rates[can_update_mask]
            )
            rest_effect = (
                self.resting_potentials[can_update_mask]
    ...
    def batch_create_neurons(
        ...
        for param_name, param_value in [
            ...
            if isinstance(param_value, list) and len(param_value) != num_neurons:
                raise ValueError(
                    f"Length of {param_name} ({len(param_value)}) does not match length of positions ({num_neurons})"
                )
        ...
        if num_neurons == 0:
            ...
            new_indices = np.arange(
                self.next_index, self.next_index + num_new_indices, dtype=np.int32
            )
        ...
        if not isinstance(thresholds, list) and not isinstance(thresholds, np.ndarray):
            ...
            if len(thresholds_list) != num_neurons:
                raise ValueError(
                    f"thresholds length ({len(thresholds_list)}) must match num_neurons ({num_neurons})"
                )
        thresholds = self.backend.array(thresholds_list)

        if not isinstance(membrane_potentials, list) and not isinstance(
            ...
            if len(membrane_potentials_list) != num_neurons:
                raise ValueError(
                    f"membrane_potentials length ({len(membrane_potentials_list)}) must match num_neurons ({num_neurons})"
                )
        membrane_potentials = self.backend.array(membrane_potentials_list)

        if not isinstance(resting_potentials, list) and not isinstance(
            resting_potentials, np.ndarray
        ):
            ...
            if len(resting_potentials_list) != num_neurons:
                raise ValueError(
                    f"resting_potentials length ({len(resting_potentials_list)}) must match num_neurons ({num_neurons})"
                )
        resting_potentials = self.backend.array(resting_potentials_list)

        if not isinstance(decay_rates, list) and not isinstance(
            ...
            if len(decay_rates_list) != num_neurons:
                raise ValueError(
                    f"decay_rates length ({len(decay_rates_list)}) must match num_neurons ({num_neurons})"
                )
        decay_rates = self.backend.array(decay_rates_list)

        if not isinstance(refractory_periods, list) and not isinstance(
            refractory_periods, np.ndarray
        ):
            ...
            if len(refractory_periods_list) != num_neurons:
                raise ValueError(
                    f"refractory_periods length ({len(refractory_periods_list)}) must match num_neurons ({num_neurons})"
                )
        refractory_periods = self.backend.array(refractory_periods_list)

        # Extract position components as uint32
        coordinates_x = self.backend.array(
            [np.uint32(max(0, pos[0])) for pos in positions]
        )
        coordinates_y = self.backend.array(
            [np.uint32(max(0, pos[1])) for pos in positions]
        )
        coordinates_z = self.backend.array(
            [np.uint32(max(0, pos[2])) for pos in positions]
        )
        ...
        if isinstance(self.membrane_potentials, torch.Tensor):
            ...
            if max_idx >= self.membrane_potentials.shape[0]:
                raise ValueError(
                    f"Index out of bounds: max_idx={max_idx}, tensor_size={self.membrane_potentials.shape[0]}"
                )
            ...
            if min_idx < 0:
                raise ValueError(
                    f"Negative index detected: min_idx={min_idx}, this causes memory corruption"
                )

            # Verify tensor compatibility before index_copy operations
            expected_size = len(idx_array)
            if (
                membrane_potentials.shape[0] != expected_size
                or resting_potentials.shape[0] != expected_size
                or thresholds.shape[0] != expected_size
                or decay_rates.shape[0] != expected_size
                or refractory_periods.shape[0] != expected_size
            ):
                raise ValueError(
                    f"Tensor size mismatch: expected {expected_size}, got membrane_potentials={membrane_potentials.shape[0]}, resting_potentials={resting_potentials.shape[0]}, thresholds={thresholds.shape[0]}, decay_rates={decay_rates.shape[0]}, refractory_periods={refractory_periods.shape[0]}"
                )
            ...
            try:
                ...
                safe_indices = idx_tensor[
                    idx_tensor < self.membrane_potentials.shape[0]
                ]
                safe_count = len(safe_indices)

                if safe_count != len(idx_tensor):
                    raise ValueError(
                        f"Unsafe indices detected: {len(idx_tensor) - safe_count} indices out of bounds"
                    )
                ...
                self.membrane_potentials[safe_indices] = membrane_potentials[
                    :safe_count
                ].to(dtype=target_dtype)
                self.resting_potentials[safe_indices] = resting_potentials[
                    :safe_count
                ].to(dtype=target_dtype)
                self.thresholds[safe_indices] = thresholds[:safe_count].to(
                    dtype=target_dtype
                )
                self.decay_rates[safe_indices] = decay_rates[:safe_count].to(
                    dtype=target_dtype
                )
                self.refractory_periods[safe_indices] = refractory_periods[
                    :safe_count
                ].to(dtype=self.refractory_periods.dtype)
                self.coordinates_x[safe_indices] = coordinates_x[:safe_count].to(
                    dtype=self.coordinates_x.dtype
                )
                self.coordinates_y[safe_indices] = coordinates_y[:safe_count].to(
                    dtype=self.coordinates_y.dtype
                )
                self.coordinates_z[safe_indices] = coordinates_z[:safe_count].to(
                    dtype=self.coordinates_z.dtype
                )
                ...
                if not isinstance(valid_mask, np.ndarray):
                    raise RuntimeError(
                        "valid_mask is not a numpy array, memory corruption detected"
                    )

                if len(valid_mask) != self.aligned_capacity:
                    raise RuntimeError(
                        f"valid_mask size mismatch: expected {self.aligned_capacity}, got {len(valid_mask)}"
                    )
                ...
                if len(numpy_indices) > 0:
                    ...
                    if max_idx >= len(valid_mask):
                        raise ValueError(
                            f"Valid mask update would be out of bounds: max_idx={max_idx}, mask_size={len(valid_mask)}"
                        )
            ...
            if len(idx_array) > 0:
                ...
                if max_idx >= len(self.membrane_potentials):
                    raise ValueError(
                        f"Index out of bounds in NumPy path: max_idx={max_idx}, array_size={len(self.membrane_potentials)}"
                    )
            ...
            expected_size = len(idx_array)
            if (
                len(membrane_potentials) != expected_size
                or len(resting_potentials) != expected_size
                or len(thresholds) != expected_size
                or len(decay_rates) != expected_size
                or len(refractory_periods) != expected_size
                or len(coordinates_x) != expected_size
                or len(coordinates_y) != expected_size
                or len(coordinates_z) != expected_size
            ):
                raise ValueError(
                    f"Array size mismatch in NumPy path: expected {expected_size}"
                )
            ...
            if len(idx_array) > 0:
                ...
                if max_idx >= len(self.valid_mask):
                    raise ValueError(
                        f"Valid mask index out of bounds: max_idx={max_idx}, mask_size={len(self.valid_mask)}"
                    )
    ...
    def batch_update_membrane_potentials(
        self, neuron_ids: List[int], values: List[float]
    ) -> None:
        ...
        if len(neuron_ids) != len(values):
            raise ValueError(
                f"Length of neuron_ids ({len(neuron_ids)}) does not match length of values ({len(values)})"
            )

        # Get indices for the neuron IDs
        indices = np.array(
            [self.id_to_index_map[nid] for nid in neuron_ids], dtype=np.int32
        )
    ...
    def vectorized_indices_to_neuron_ids(
        ...
        if filter_invalid:
            ...
            valid_mask = (indices >= 0) & (
                indices < len(self._index_to_id_lookup_array)
            )
    ...
    def _build_index_to_id_lookup_array(self):
        ...
        try:
            self._index_to_id_lookup_array = np.full(
                self.aligned_capacity, -1, dtype=np.int64
            )
        except MemoryError:
            # Fallback to smaller allocation if memory is tight
            self._index_to_id_lookup_array = np.full(
                self.max_neurons, -1, dtype=np.int64
            )
        ...
        if self.id_to_index_map:
            ...
            if invalid_count > 0:
                ...
                logger.warning(
                    f"{invalid_count} neuron indices out of bounds (max invalid: {max_invalid}, array size: {len(self._index_to_id_lookup_array)})"
                )
    ...
    def process_incoming_signals(self, signal_matrix, batch_size=10000):
        ...
        if self.device == "cuda" and isinstance(self.membrane_potentials, torch.Tensor):
            ...
            return (
                fired_mask.cpu().numpy()
                if isinstance(fired_mask, torch.Tensor)
                else fired_mask
            )
    ...
    def use_best_available_device(self):
        ...
        try:
            ...
            float_arrays_count = (
                4  # membrane_potentials, resting_potentials, thresholds, decay_rates
            )
            int_arrays_count = (
                4  # refractory_periods, refractory_counters, cortical_idxs
            )
            bool_arrays_count = 2  # is_active, valid_mask
            positions_count = 3  # coordinates_x, coordinates_y, coordinates_z

            bytes_per_neuron = (
                (float_arrays_count * 4)
                + (int_arrays_count * 4)
                + (bool_arrays_count * 1)
                + (positions_count * 4)
            )
    ...
    def get_device_stats(self):
        ...
        # Each bool array
        bool_arrays = [self.is_active, self.valid_mask]
