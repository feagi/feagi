...
class SynapseManager:
    ...
    def _init_synapse_storage(self):
        ...
        self.weights = sp.lil_matrix(
            (self.max_neurons, self.max_neurons), dtype=np.float32
        )

        # Plasticity-related matrices (only allocated for plastic synapses)
        self.is_plastic = sp.lil_matrix(
            (self.max_neurons, self.max_neurons), dtype=bool
        )
        self.plasticity_coeffs = sp.lil_matrix(
            (self.max_neurons, self.max_neurons), dtype=np.float32
        )
        self.plasticity_decay = sp.lil_matrix(
            (self.max_neurons, self.max_neurons), dtype=np.float32
        )

        # Additional plasticity parameters (new)
        self.plasticity_type = sp.lil_matrix(
            (self.max_neurons, self.max_neurons), dtype=np.uint8
        )
        self.activity_factor = sp.lil_matrix(
            (self.max_neurons, self.max_neurons), dtype=np.float32
        )
        self.scaling_exponent = sp.lil_matrix(
            (self.max_neurons, self.max_neurons), dtype=np.float32
        )
        ...
        logger.info(
            f"Initialized synapse storage for up to {self.max_neurons} neurons "
    ...
    def add_synapse(
        ...
        if pre_neuron >= self.max_neurons or post_neuron >= self.max_neurons:
            logger.error(
                f"Cannot create synapse: neuron ID exceeds max neurons ({self.max_neurons})"
            )
            return False
        ...
        with self._synapse_lock:
            ...
            if post_neuron in self.outgoing_synapses[pre_neuron]:
                ...
                logger.debug(
                    f"Updated synapse: {pre_neuron} -> {post_neuron}, weight={weight}"
                )
                return True
            ...
            logger.debug(
                f"Added synapse: {pre_neuron} -> {post_neuron}, weight={weight}"
            )
            return True
    ...
    def get_outgoing_synapses(self, neuron_id: int) -> List[Tuple[int, float]]:
        ...
        return [
            (post_id, self.weights[neuron_id, post_id])
    ...
    def get_incoming_synapses(self, neuron_id: int) -> List[Tuple[int, float]]:
        ...
        return [
            (pre_id, self.weights[pre_id, neuron_id])
    ...
    def resize(self, new_max_neurons: int) -> bool:
        ...
        if new_max_neurons <= self.max_neurons:
            logger.warning(
                f"New size {new_max_neurons} is not larger than current size {self.max_neurons}"
            )
            return False

        with self._synapse_lock:
            # Create new matrices with larger size
            new_weights = sp.lil_matrix(
                (new_max_neurons, new_max_neurons), dtype=np.float32
            )
            new_is_plastic = sp.lil_matrix(
                (new_max_neurons, new_max_neurons), dtype=bool
            )
            new_plasticity_coeffs = sp.lil_matrix(
                (new_max_neurons, new_max_neurons), dtype=np.float32
            )
            new_plasticity_decay = sp.lil_matrix(
                (new_max_neurons, new_max_neurons), dtype=np.float32
            )
            new_plasticity_type = sp.lil_matrix(
                (new_max_neurons, new_max_neurons), dtype=np.uint8
            )
            new_activity_factor = sp.lil_matrix(
                (new_max_neurons, new_max_neurons), dtype=np.float32
            )
            new_scaling_exponent = sp.lil_matrix(
                (new_max_neurons, new_max_neurons), dtype=np.float32
            )
            ...
            new_plasticity_decay[: self.max_neurons, : self.max_neurons] = (
                self.plasticity_decay
            )
            new_plasticity_type[: self.max_neurons, : self.max_neurons] = (
                self.plasticity_type
            )
            new_activity_factor[: self.max_neurons, : self.max_neurons] = (
                self.activity_factor
            )
            new_scaling_exponent[: self.max_neurons, : self.max_neurons] = (
                self.scaling_exponent
            )

            # Extend adjacency lists
            new_outgoing_synapses = list(self.outgoing_synapses)
            new_outgoing_synapses.extend(
                [[] for _ in range(new_max_neurons - self.max_neurons)]
            )

            new_incoming_synapses = list(self.incoming_synapses)
            new_incoming_synapses.extend(
                [[] for _ in range(new_max_neurons - self.max_neurons)]
            )
            ...
            logger.info(
                f"Resized synapse storage from {self.max_neurons} to {new_max_neurons} neurons"
            )
            return True
