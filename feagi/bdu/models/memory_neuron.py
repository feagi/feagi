...
class MemoryPatternKey:
    ...
    def __eq__(self, other) -> bool:
        if not isinstance(other, MemoryPatternKey):
            return False
        return (self.pattern_data == other.pattern_data and 
                self.temporal_depth == other.temporal_depth and
                self.source_cortical_areas == other.source_cortical_areas)
...
class MemoryNeuronArray:
    ...
    def __init__(self, capacity: int):
        ...
        self.lifespan_initial = np.zeros(capacity, dtype=np.uint32)     # Initial lifespan value  
        self.lifespan_growth_rate = np.zeros(capacity, dtype=np.float32) # Growth rate on reactivation
        self.is_longterm_memory = np.zeros(capacity, dtype=np.bool_)    # Long-term memory flag
        
        # Lifecycle tracking
        self.creation_burst = np.zeros(capacity, dtype=np.uint64)       # Burst when neuron was created
        self.last_activation_burst = np.zeros(capacity, dtype=np.uint64) # Last time pattern was seen
        self.activation_count = np.zeros(capacity, dtype=np.uint32)     # Total activations since creation
        
        # Memory area association  
        self.cortical_area_id = [""] * capacity  # Which memory area this neuron belongs to
    ...
    def create_memory_neuron(
        ...
        if existing_idx is not None:
            ...
            if neuron_idx is None:
                logger.warning(f"Memory neuron capacity {self.capacity} exceeded for area {cortical_area_id}")
                return None
        ...
        logger.debug(f"Created memory neuron {neuron_idx} for pattern in area {cortical_area_id}")
        return neuron_idx
    ...
    def get_memory_neuron_info(self, neuron_idx: int) -> Optional[Dict[str, Any]]:
        """
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        longterm_mask = self.is_longterm_memory[:self.next_available_index] & active_mask
        ...
        return {
            ...
            "average_lifespan_active": float(np.mean(self.lifespan_current[:self.next_available_index][active_mask])) if np.any(active_mask) else 0.0,
            "total_activations": int(np.sum(self.activation_count[:self.next_available_index][active_mask]))
        }
    ...
    def _calculate_memory_usage(self) -> int:
        ...
        pattern_memory = len(self.pattern_to_index) * 200  # Rough estimate for pattern keys
        string_memory = len(self.cortical_area_id) * 50    # Rough estimate for strings
