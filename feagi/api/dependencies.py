...
from feagi.core.state_manager import FeagiStateManager, GenomeState, ConnectomeState, ServiceState
...
# Genome state checks
def check_active_genome(core_api_service: CoreAPIService = Depends(get_core_api_service)):
    """
    ...
    if not state_manager.is_genome_loaded():
        raise HTTPException(status_code=400, detail="No active genome found. Please load a genome first.")
...
def check_connectome_ready(core_api_service: CoreAPIService = Depends(get_core_api_service)):
    """
    ...
    if not hasattr(connectome_manager, 'cortical_areas') or not connectome_manager.cortical_areas:
        raise HTTPException(
            status_code=400,
...
def check_burst_engine_running(core_api_service: CoreAPIService = Depends(get_core_api_service)):
    """
...
def check_deployment_ready(
    ...
    if burst_state not in [ServiceState.READY, ServiceState.STOPPED, ServiceState.UNAVAILABLE]:
        raise HTTPException(
            status_code=409,  # Conflict
    ...
    connectome_state = state_manager.get_connectome_state()
    if connectome_state not in [ConnectomeState.READY, ConnectomeState.INITIALIZING, ConnectomeState.UNAVAILABLE]:
        raise HTTPException(
            status_code=409,  # Conflict
...
def check_cortical_area_exists(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error checking cortical area: {str(e)}"
        )
