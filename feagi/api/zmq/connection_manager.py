...
class ConnectionManager:
    ...
    def __init__(
        ...
        )  # Raise error if recipient not found
        ...
        if visualization_port is not None:
            ...
            logger.info(
                f"Visualization socket (PUB) bound to port {visualization_port}"
            )
        else:
    ...
    async def send_message(
        ...
        try:
            if protocol_type == "fcp":
                ...
                logger.warning(
                    "Cannot send sensory data to client (one-way from client to FEAGI)"
                )
                return False
                ...
                if self.visualization_socket is None:
                    logger.debug(
                        "Visualization socket disabled, cannot send visualization data"
                    )
                    return False
...
class ZMQConnectionManager:
    ...
    def instance(cls, core_api: Optional[CoreAPIService] = None, host: str = None):
        ...
        if cls._instance is None:
            if host is None:
                raise ValueError(
                    "Host must be provided when creating the initial instance"
                )
    ...
    def __init__(self, core_api: Optional[CoreAPIService] = None, host: str = None):
        ...
        if ZMQConnectionManager._instance is not None:
            raise RuntimeError(
                "Use ZMQConnectionManager.instance() to get the singleton instance"
            )

        if host is None:
            raise ValueError(
                "Host is required for ZMQ connection manager - no hardcoded defaults for deployment compatibility"
            )
    ...
    def create_server(self, server_type: str = "default", **kwargs) -> Any:
        ...
        if server_type in self.servers:
            logger.warning(
                f"Server of type {server_type} already exists, returning existing instance"
            )
    ...
    def _update_active_mode(self):
        ...
        if old_mode != self._active_mode:
            if self._active_mode:
                ...
                logger.info(
                    "ZMQConnectionManager: Entering STANDBY mode (no genome loaded)"
                )
    ...
    def _on_genome_state_change(self, old_state, new_state):
        ...
        """
        logger.debug(
            f"ZMQConnectionManager received genome state change: {old_state} → {new_state}"
        )
        ...
        if new_state == GenomeState.LOADED:
            ...
            self._active_mode = False
            logger.info(
                "ZMQConnectionManager: Entering STANDBY mode (genome not loaded)"
            )
