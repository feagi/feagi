...
class MessageHandler:
    ...
    def __init__(
        ...
        if protocol_type == "fcp":
            ...
            if self.socket is None:
                logger.info(
                    "FSMP handler disabled - sensory socket handled by dedicated stream"
                )
    ...
    async def start(self) -> None:
        ...
        if self.socket is None:
            logger.info(
                f"Skipping {self.protocol_type} message handler - socket is disabled"
            )
            return
...
async def start_message_handlers(
    ...
    handlers["handshake"] = HandshakeMessageHandler(
        connection_manager, message_processors["handshake"], translator
    )

    handlers["fcp"] = FCPMessageHandler(
        connection_manager, message_processors["fcp"], translator
    )

    handlers["fsmp"] = FSMPMessageHandler(
        connection_manager, message_processors["fsmp"], translator
    )
    ...
    if connection_manager.visualization_socket is not None:
        handlers["fvp"] = FVPMessageHandler(
            connection_manager, message_processors["fvp"], translator
        )
