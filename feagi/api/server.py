...
def main():
    ...
    parser.add_argument(
        "--zmq-stream-port", type=int, default=5558, help="ZeroMQ Stream port"
    )
    ...
    if args.zmq and not args.zmq_host:
        parser.error(
            "--zmq-host is required when using --zmq. No hardcoded defaults for deployment compatibility."
        )

    # Warning about standalone mode
    logger.warning(
        "Running FEAGI API server in standalone mode. For full functionality, use 'python -m feagi.main'"
    )
