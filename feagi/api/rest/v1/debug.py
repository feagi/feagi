...
async def disable_zmq_debug():
    ...
    try:
        ...
        return {
            "status": "disabled",
            "message": "All ZMQ debugging disabled"
        }
    except Exception as e:
...
async def set_message_type_filters(message_types: List[str]):
    ...
    try:
        if not message_types:
            ...
            set_message_filters([])
            return {"status": "configured", "filters": "none", "message": "All message types will be debugged"}
        ...
        raise HTTPException(status_code=500, detail=f"Error setting message filters: {str(e)}")
...
async def set_endpoint_filters(endpoints: List[str]):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error setting endpoint filters: {str(e)}")
...
async def set_zmq_rate_limit(limit: int):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error setting rate limit: {str(e)}")
