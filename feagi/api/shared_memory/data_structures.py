...
from .manager import SharedMemoryRegion, SharedMemoryManager
...
class SharedNeuronArray:
    ...
    def __init__(
        ...
        self.required_size = capacity * self.element_size + 1024  # Extra space for metadata
        ...
        self.logger.info(f"Initialized SharedNeuronArray '{name}' with capacity {capacity}")
...
class SharedSynapseArray:
    ...
    def __init__(
        ...
        # Initialize metadata
        self.count = 0
        self._pre_to_indices: Dict[int, List[int]] = {}  # Pre-neuron ID to synapse indices
        self._post_to_indices: Dict[int, List[int]] = {}  # Post-neuron ID to synapse indices
        
        # Initialize the array with zeros
        self.array.fill(0)
        
        self.logger.info(f"Initialized SharedSynapseArray '{name}' with capacity {capacity}")
    ...
    def add_synapse(
        ...
        if existing_idx is not None:
            ...
            if not self.array[existing_idx]['is_active']:
                ...
                self.logger.warning(f"Synapse from {pre_neuron_id} to {post_neuron_id} already exists")
                return False
    ...
    def _find_synapse_index(self, pre_neuron_id: int, post_neuron_id: int) -> Optional[int]:
    ...
    def get_synapse(self, pre_neuron_id: int, post_neuron_id: int) -> Optional[np.ndarray]:
        """
    ...
    def _write_dict(self, data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if len(encoded) > self.region.size:
                self.logger.error(f"Config dictionary too large for shared memory: {len(encoded)} bytes")
                return False
