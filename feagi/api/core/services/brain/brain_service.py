...
class BrainService(BaseService):
    ...
    def start_burst_engine(self) -> bool:
        ...
        try:
            ...
            #  ===== RUST BURST LOOP =====
            #  Pure Rust burst loop (NO Python overhead!)
            import threading
            
            # Get power neurons from genome/connectome
            power_neurons = []
            try:
                if hasattr(self, 'connectome_manager') and self.connectome_manager:
                    genome_data = getattr(self.connectome_manager, 'genome_data', None)
                    if genome_data and isinstance(genome_data, dict):
                        power_neuron_dict = genome_data.get('power_neurons', {})
                        for neuron_ids in power_neuron_dict.values():
                            if isinstance(neuron_ids, list):
                                power_neurons.extend(neuron_ids)
                    self.logger.info(f"🦀 [RUST-BURST] Power neurons: {len(power_neurons)} neurons")
            except Exception as e:
                self.logger.warning(f"Could not get power neurons: {e}")
            
            # Set power neurons on Rust NPU
            ...
            self.logger.info(f"🦀 [RUST-BURST] Starting Rust burst loop at {frequency_hz} Hz with {len(power_neurons)} power neurons")
