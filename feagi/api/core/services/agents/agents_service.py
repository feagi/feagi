        ...
        try:
            ...
            for agent_id, agent_info in connected_agents.items():
                if agent_filter:
                    ...
                    if "type" in agent_filter and agent_info.get("type") != agent_filter["type"]:
                        continue
                    if "status" in agent_filter and agent_info.get("status") != agent_filter["status"]:
                        continue
                
                target_agents.append(agent_id)
            
            # Log the broadcast
            self.logger.info(f"Broadcasting message to {len(target_agents)} agents: {message}")
