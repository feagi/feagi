...
class SystemService(BaseService):
    ...
    def _get_neuron_count_breakdown(self) -> Dict[str, int]:
        ...
        try:
            ...
            memory_count = 0
            if self._connectome_manager and hasattr(self._connectome_manager, 'memory_neuron_array'):
                try:
                    mem_stats = self._connectome_manager.memory_neuron_array.get_statistics()
                    memory_count = int(mem_stats.get('total_active_neurons', mem_stats.get('active_neurons', 0)))
                    self.logger.debug(f"📊 [HEALTH] Memory neuron count from ConnectomeManager: {memory_count}")
                except Exception as e:
                    self.logger.warning(f"Could not get memory neuron count from ConnectomeManager: {e}")
            ...
            if memory_count == 0 and self.state_manager:
                try:
                    ...
                    if isinstance(brain_stats, dict) and 'memory_neuron_count' in brain_stats:
                        memory_count = int(brain_stats.get('memory_neuron_count', 0))
