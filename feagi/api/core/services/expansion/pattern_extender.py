...
class PatternExtender:
    ...
    def extend_patterns_for_expansion(
        ...
        """
        self.logger.info(
            f"🔍 [PATTERN-EXTEND] Starting pattern extension for {cortical_id}"
        )
        self.logger.info(
            f"🔍 [PATTERN-EXTEND] Dimensions: {old_dimensions} → {new_dimensions}"
        )
        self.logger.info(
            f"🔍 [PATTERN-EXTEND] Extending patterns to {len(new_neurons)} new neurons"
        )
        self.logger.info(
            f"🔍 [PATTERN-EXTEND] New neuron IDs: {sorted(list(new_neurons))[:10]}{'...' if len(new_neurons) > 10 else ''}"
        )

        try:
            # Find existing cortical mappings for this area
            self.logger.info(
                "🔍 [PATTERN-EXTEND] Searching for existing mappings..."
            )
            existing_mappings = self._find_existing_mappings(cortical_id)

            self.logger.info(
                f"🔍 [PATTERN-EXTEND] Found {len(existing_mappings)} existing mappings"
            )

            if not existing_mappings:
                self.logger.warning(
                    f"🔍 [PATTERN-EXTEND] No existing mappings found for {cortical_id} - cannot extend patterns"
                )
                return 0
            ...
            for i, mapping in enumerate(existing_mappings):
                self.logger.info(
                    f"🔍 [PATTERN-EXTEND] Processing mapping {i + 1}/{len(existing_mappings)}"
                )
                self.logger.info(
                    f"🔍 [PATTERN-EXTEND]   - Source: {mapping.get('source', 'unknown')}"
                )
                self.logger.info(
                    f"🔍 [PATTERN-EXTEND]   - Destination: {mapping.get('destination', 'unknown')}"
                )
                self.logger.info(
                    f"🔍 [PATTERN-EXTEND]   - Morphology: {mapping.get('morphology', 'unknown')}"
                )
                ...
                self.logger.info(
                    f"🔍 [PATTERN-EXTEND] Created {synapses_created} synapses for this mapping"
                )

            self.logger.info(
                f"[PATTERN-EXTEND] Created {total_synapses_created} new synapses for {cortical_id}"
            )
            return total_synapses_created

        except Exception as e:
            self.logger.error(
                f"[PATTERN-EXTEND] Error extending patterns for {cortical_id}: {e}"
            )
            return 0
    ...
    def _find_existing_mappings(
        ...
        try:
            ...
            if not genome or "blueprint" not in genome:
                self.logger.warning(
                    "[PATTERN-EXTEND] No hierarchical genome found"
                )
                return []
            ...
            self.logger.info(
                f"[PATTERN-EXTEND] Searching hierarchical genome for mappings involving {cortical_id}"
            )
                            ...
                            if isinstance(connection_specs, list):
                                for spec in connection_specs:
                                    ...
                                    if isinstance(spec, dict):
                                        ...
                                        self.logger.info(
                                            f"[PATTERN-EXTEND] Found outgoing mapping (dict): {cortical_id} -> {dst_area} using {spec.get('morphology_id')}"
                                        )
                                    elif (
                                        ...
                                        existing_mappings.append(mapping)
                                        self.logger.info(
                                            f"[PATTERN-EXTEND] Found outgoing mapping (array): {cortical_id} -> {dst_area} using {morphology_id}"
                                        )
                                    else:
                                        self.logger.warning(
                                            f"[PATTERN-EXTEND] Unknown outgoing spec format: {type(spec)} - {spec}"
                                        )

            # 2. Find incoming mappings (others -> this area)
            for source_area_id, area_data in blueprint.items():
                if source_area_id != cortical_id and isinstance(
                    area_data, dict
                ):
                            ...
                            if isinstance(connection_specs, list):
                                for spec in connection_specs:
                                    ...
                                    if isinstance(spec, dict):
                                        ...
                                        existing_mappings.append(mapping)
                                        self.logger.info(
                                            f"[PATTERN-EXTEND] Found incoming mapping (dict): {source_area_id} -> {cortical_id} using {spec.get('morphology_id')}"
                                        )
                                    elif (
                                        isinstance(spec, list)
                                        and len(spec) >= 1
                                        ...
                                        existing_mappings.append(mapping)
                                        self.logger.info(
                                            f"[PATTERN-EXTEND] Found incoming mapping (array): {source_area_id} -> {cortical_id} using {morphology_id}"
                                        )
                                    else:
                                        self.logger.warning(
                                            f"[PATTERN-EXTEND] Unknown incoming spec format: {type(spec)} - {spec}"
                                        )

            self.logger.info(
                f"[PATTERN-EXTEND] Found {len(existing_mappings)} existing mappings for {cortical_id}"
            )
            for i, mapping in enumerate(existing_mappings):
                self.logger.info(
                    f"[PATTERN-EXTEND]   Mapping {i + 1}: {mapping['source']} -> {mapping['destination']} ({mapping['morphology']})"
                )
            ...
            self.logger.error(
                f"[PATTERN-EXTEND] Error finding mappings for {cortical_id}: {e}"
            )
            import traceback

            self.logger.error(
                f"[PATTERN-EXTEND] Traceback: {traceback.format_exc()}"
            )
            return []
    ...
    def _extend_mapping_pattern(
        ...
        try:
            ...
            if not all([source, destination, morphology_id]):
                self.logger.warning(
                    f"[PATTERN-EXTEND] Incomplete mapping: {mapping}"
                )
                return 0

            self.logger.info(
                f"[PATTERN-EXTEND] Extending mapping: {source} → {destination} using {morphology_id}"
            )
            ...
            if not morphology_def:
                self.logger.warning(
                    f"[PATTERN-EXTEND] Morphology {morphology_id} not found"
                )
                return 0

            # Check if this is a dimension-sensitive morphology
            dimension_sensitive = morphology_def.get(
            ...
            if dimension_sensitive:
                ...
                self.logger.info(
                    f"[PATTERN-EXTEND] Morphology {morphology_id} is dimension-agnostic - extending pattern"
                )
            ...
            self.logger.error(
                f"[PATTERN-EXTEND] Error extending mapping pattern: {e}"
            )
            return 0
    ...
    def _get_morphology_definition(
        ...
        try:
            ...
            self.logger.error(
                f"[PATTERN-EXTEND] Error getting morphology {morphology_id}: {e}"
            )
            return None
    ...
    def _apply_morphology_to_new_neurons(
        ...
        try:
            ...
            neuro_embryo = NeuroEmbryogenesis(
            ...
            formatted_mapping = {
                mapping["source"]: {mapping["destination"]: [connection_spec]}
            }
            ...
            try:
                ...
                if current_genome:
                    ...
                    self.logger.info(
                        f"[PATTERN-EXTEND] Set genome on embryogenesis, morphologies available: {len(current_genome.get('neuron_morphologies', {}))}"
                    )
                else:
                    self.logger.warning(
                        "[PATTERN-EXTEND] No genome available in state_manager"
                    )
            ...
            self.logger.info(
                f"[PATTERN-EXTEND] Applied {mapping['morphology']} to {len(new_neurons)} new neurons: {synapses_created} synapses created"
            )
            return synapses_created

        except Exception as e:
            self.logger.error(
                f"[PATTERN-EXTEND] Error applying morphology to new neurons: {e}"
            )
            import traceback
