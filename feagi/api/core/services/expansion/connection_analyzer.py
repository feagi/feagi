...
class ConnectionAnalyzer:
    ...
    def analyze_area_connectivity(self, cortical_id: str) -> Dict[str, Any]:
        ...
        """
        self.logger.info(f"🔍 [CONNECTION-ANALYZER] Starting connectivity analysis for {cortical_id}")
        ...
        try:
            ...
            if internal_mappings:
                ...
                for i, mapping in enumerate(internal_mappings):
                    ...
                    self.logger.info(f"🔍 [CONNECTION-ANALYZER]   [{i+1}] {source} -> {dest} ({morphology})")
            else:
                self.logger.warning(f"🔍 [CONNECTION-ANALYZER] No internal mappings found for {cortical_id}")
            ...
            if outgoing_mappings:
                ...
                for i, mapping in enumerate(outgoing_mappings[:3]):  # Show first 3
                    morphology = mapping.get("morphology", "unknown")
                    dest = mapping.get("destination", "unknown")
                    self.logger.info(f"🔍 [CONNECTION-ANALYZER]   [{i+1}] {cortical_id} -> {dest} ({morphology})")
                if len(outgoing_mappings) > 3:
                    self.logger.info(f"🔍 [CONNECTION-ANALYZER]   ... and {len(outgoing_mappings) - 3} more")
            ...
            for mapping_list in [incoming_mappings, outgoing_mappings, internal_mappings]:
                for mapping in mapping_list:
                    # Create a unique key for each mapping
                    mapping_key = (mapping.get("source"), mapping.get("destination"), mapping.get("morphology"))
            ...
            for mapping in unique_mappings:
                ...
                if morphology_id:
                    ...
                    if morphology_def:
                        is_sensitive = morphology_def.get("dimension_sensitive", False)
                        if is_sensitive:
            ...
            self.logger.info(f"[CONNECTION-ANALYZER] Analysis complete for {cortical_id}:")
            self.logger.info(f"  Total mappings: {analysis['total_mappings']}")
            self.logger.info(f"  Dimension-sensitive: {analysis['dimension_sensitive_count']}")
            self.logger.info(f"  Dimension-agnostic: {analysis['dimension_agnostic_count']}")
            self.logger.info(f"  Internal mappings: {analysis['internal_count']}")
            
            return analysis
            
        except Exception as e:
            self.logger.error(f"[CONNECTION-ANALYZER] Error analyzing {cortical_id}: {e}")
            return {
                "cortical_id": cortical_id,
                "error": str(e),
                "total_mappings": 0
    ...
    def _get_incoming_mappings(self, cortical_id: str) -> List[Dict[str, Any]]:
        ...
        try:
                            ...
                            if isinstance(connection_specs, list):
                                for spec in connection_specs:
                                    ...
                                    if isinstance(spec, dict):
                                        # Dict format
                                        if source_area_id != cortical_id:  # Only include if it's not an internal mapping
                                            mapping = {
            ...
            self.logger.info(f"🔍 [CONNECTION-ANALYZER] Found {len(mappings)} incoming mappings for {cortical_id}")
        except Exception as e:
            self.logger.error(f"[CONNECTION-ANALYZER] Error getting incoming mappings: {e}")
        return mappings
    ...
    def _get_outgoing_mappings(self, cortical_id: str) -> List[Dict[str, Any]]:
        ...
        try:
                            ...
                            for dst_area_id, connection_specs in cortical_mappings.items():
                                if isinstance(connection_specs, list):
                                    for spec in connection_specs:
                                        ...
                                        if isinstance(spec, dict):
                                            # Dict format
                                            if dst_area_id != cortical_id:  # Only include if it's not an internal mapping
                                                mapping = {
                                                    "source": cortical_id,
            ...
            self.logger.info(f"🔍 [CONNECTION-ANALYZER] Found {len(mappings)} outgoing mappings for {cortical_id}")
        except Exception as e:
            self.logger.error(f"[CONNECTION-ANALYZER] Error getting outgoing mappings: {e}")
        return mappings
    ...
    def _get_internal_mappings(self, cortical_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.info(f"🔍 [CONNECTION-ANALYZER] Debugging genome structure for {cortical_id}")
            self.logger.info(f"🔍 [CONNECTION-ANALYZER] Genome type: {type(genome)}")
            
            if genome:
                self.logger.info(f"🔍 [CONNECTION-ANALYZER] Genome keys: {list(genome.keys())}")
                
                if "blueprint" in genome:
                    blueprint = genome["blueprint"]
                    self.logger.info(f"🔍 [CONNECTION-ANALYZER] Blueprint type: {type(blueprint)}")
                    self.logger.info(f"🔍 [CONNECTION-ANALYZER] Blueprint keys: {list(blueprint.keys()) if isinstance(blueprint, dict) else 'not a dict'}")
                    
                    # Check if this cortical area has internal mappings (mappings to itself)
                    if cortical_id in blueprint:
                        area_data = blueprint[cortical_id]
                        self.logger.info(f"🔍 [CONNECTION-ANALYZER] Found {cortical_id} in blueprint")
                        self.logger.info(f"🔍 [CONNECTION-ANALYZER] Area data type: {type(area_data)}")
                        self.logger.info(f"🔍 [CONNECTION-ANALYZER] Area data keys: {list(area_data.keys()) if isinstance(area_data, dict) else 'not a dict'}")
                                ...
                                if isinstance(connection_specs, list):
                                    for spec in connection_specs:
                                        ...
                                        if isinstance(spec, dict):
                                            ...
                                            mappings.append(mapping)
                                            self.logger.info(f"🔍 [CONNECTION-ANALYZER] Added internal mapping (dict): {mapping}")
                                        elif isinstance(spec, list) and len(spec) >= 1:
                                            try:
                                                # Array format: ["lateral_+x", [1, 1, 1], 1.0, False, 1, 1, 1]
                                                morphology_id = spec[0] if len(spec) > 0 else None
                                                ...
                                                mappings.append(mapping)
                                                self.logger.info(f"🔍 [CONNECTION-ANALYZER] Added internal mapping (array): {mapping}")
                                                self.logger.info(f"🔍 [CONNECTION-ANALYZER] Current mappings count: {len(mappings)}")
                                            except Exception as mapping_ex:
                                                self.logger.error(f"🔍 [CONNECTION-ANALYZER] Exception adding internal mapping: {mapping_ex}")
                                                self.logger.error(f"🔍 [CONNECTION-ANALYZER] Spec that caused error: {spec}")
                                                import traceback
                                                self.logger.error(f"🔍 [CONNECTION-ANALYZER] Traceback: {traceback.format_exc()}")
                                        else:
                                            self.logger.warning(f"🔍 [CONNECTION-ANALYZER] Unknown spec format: {type(spec)} - {spec}")
                            else:
            ...
            self.logger.error(f"[CONNECTION-ANALYZER] Error getting internal mappings: {e}")
            import traceback
            self.logger.error(f"[CONNECTION-ANALYZER] Traceback: {traceback.format_exc()}")
            mappings = []  # Return empty list on error
        
        self.logger.info(f"🔍 [CONNECTION-ANALYZER] Final return: {len(mappings)} mappings")
        return mappings
    ...
    def _get_morphology_definition(self, morphology_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error(f"[CONNECTION-ANALYZER] Error getting morphology {morphology_id}: {e}")
        return None
    ...
    def get_expansion_recommendation(self, cortical_id: str) -> Dict[str, Any]:
        ...
        recommendation = {
            "cortical_id": cortical_id,
            "should_preserve_patterns": analysis["dimension_agnostic_count"] > 0,
            "should_reconstruct_patterns": analysis["dimension_sensitive_count"] > 0,
            "has_internal_connectivity": analysis["internal_count"] > 0,
            "preservation_candidates": analysis.get("dimension_agnostic_mappings", []),
            "reconstruction_candidates": analysis.get("dimension_sensitive_mappings", [])
        }
        
        self.logger.info(f"[CONNECTION-ANALYZER] Expansion recommendation for {cortical_id}:")
        self.logger.info(f"  Preserve patterns: {recommendation['should_preserve_patterns']}")
        self.logger.info(f"  Reconstruct patterns: {recommendation['should_reconstruct_patterns']}")
        self.logger.info(f"  Has internal connectivity: {recommendation['has_internal_connectivity']}")
