...
class BaseService:
    ...
    def _validate_state_consistency(self) -> bool:
        ...
        try:
            if not self.state_manager:
                self.logger.warning("Cannot validate state consistency - no state manager")
                return False
                
            if not self._connectome_manager:
                self.logger.warning("Cannot validate state consistency - no connectome manager")
                return False
            ...
            if missing_attributes:
                self.logger.warning(f"State manager missing critical attributes: {missing_attributes}")
                return False
            ...
            if hasattr(self._connectome_manager, 'cortical_areas'):
                ...
                if connectome_area_count != state_area_count:
                    self.logger.warning(f"Cortical area count mismatch: connectome={connectome_area_count}, state={state_area_count}")
                    return False
    ...
    def _sync_state_if_needed(self) -> bool:
        ...
        try:
            ...
            if not hasattr(self.state_manager, 'genome_validity'):
                self.state_manager.genome_validity = True  # Assume valid if genome is loaded
