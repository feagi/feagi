...
class ByteStructureEncoder:
    ...
    def encode_neuron_data(self, neuron_data: list) -> bytes:
        ...
        for cortical_id, coords in cortical_areas.items():
            ...
            # Create cortical ID using modern feagi-rust-py-libs approach
            ...
            try:
                ...
                #  modern format IDs
                cortical_id_obj = (
                    self.frpl.genome.CorticalID.try_new_from_string(area_str)
                )
            except ValueError:
                # Fallback for areas that can't be parsed directly
                if area_str == "_power":
                    cortical_id_obj = (
                        self.frpl.genome.CorticalID.new_core_cortical_area_id(
                    ...
                    cortical_id_obj = (
                        self.frpl.genome.CorticalID.new_custom_cortical_area_id(
                            f"c{area_str}"
                        )
                    )
            ...
            neurons_array = (
                self.frpl.neuron_data.xyzp.NeuronXYZPArrays.new_from_numpy(
