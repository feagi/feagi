...
class BinarySerializer:
    ...
    def decode_fcp(cls, data: bytes) -> Dict[str, Any]:
        ...
        if len(data) < cls.HEADER_SIZE + cls.FCP_HEADER_SIZE:
            raise BinaryProtocolError(
                f"Data too short for FCP message: {len(data)} bytes"
            )
        ...
        if protocol_id != ProtocolID.FCP:
            raise BinaryProtocolError(
                f"Expected FCP protocol (1), got {protocol_id.value}"
            )

        # Extract FCP header
        offset = cls.HEADER_SIZE
        command_type, message_length = struct.unpack(
            cls.FCP_HEADER_FORMAT, data[offset : offset + cls.FCP_HEADER_SIZE]
        )
        ...
        if len(data) < offset + message_length:
            raise BinaryProtocolError(
                f"Data too short for payload: expected {message_length} bytes"
            )
    ...
    def encode_fvp(
        ...
        fvp_header = struct.pack(
            cls.FVP_HEADER_FORMAT, frame_type, timestamp, data_length
        )
    ...
    def decode_fvp(cls, data: bytes) -> Dict[str, Any]:
        ...
        if len(data) < cls.HEADER_SIZE + cls.FVP_HEADER_SIZE:
            raise BinaryProtocolError(
                f"Data too short for FVP message: {len(data)} bytes"
            )
        ...
        if protocol_id != ProtocolID.FVP:
            raise BinaryProtocolError(
                f"Expected FVP protocol (2), got {protocol_id.value}"
            )

        # Extract FVP header
        offset = cls.HEADER_SIZE
        frame_type, timestamp, data_length = struct.unpack(
            cls.FVP_HEADER_FORMAT, data[offset : offset + cls.FVP_HEADER_SIZE]
        )
        ...
        if len(data) < offset + data_length:
            raise BinaryProtocolError(
                f"Data too short for payload: expected {data_length} bytes"
            )
    ...
    def encode_fsmp(
        ...
        data_length = len(payload)
        fsmp_header = struct.pack(
            cls.FSMP_HEADER_FORMAT, channel_id, timestamp, data_length
        )
    ...
    def decode_fsmp(cls, data: bytes) -> Dict[str, Any]:
        ...
        if len(data) < cls.HEADER_SIZE + cls.FSMP_HEADER_SIZE:
            raise BinaryProtocolError(
                f"Data too short for FSMP message: {len(data)} bytes"
            )
        ...
        if protocol_id != ProtocolID.FSMP:
            raise BinaryProtocolError(
                f"Expected FSMP protocol (3), got {protocol_id.value}"
            )
