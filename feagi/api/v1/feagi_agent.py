...
class FeagiAgentAPI:
    ...
    async def register_agent(self, request: AgentRegistrationRequest) -> SuccessResponse:
        try:
            ...
            from feagi.pns.registration_manager import get_registration_manager, AgentRegistrationRequest as RegistrationRequest
            ...
            if response.success:
                self.logger.info(f"✅ Agent '{request.agent_id}' registered via Registration Manager - "
    ...
    async def deregister_agent(self, request: AgentDeregistrationRequest) -> SuccessResponse:
        try:
            ...
            if response.success:
                self.logger.info(f"✅ Agent '{request.agent_id}' deregistered via Registration Manager - "
                               f"FQ samplers coordinated: {response.fq_samplers_enabled}")
                ...
                self.logger.error(f"❌ Registration Manager deregistration failed: {response.message}")
    ...
    async def get_fq_sampler_status(self) -> dict:
        ...
        try:
            ...
            coordination_status = registration_manager.get_fq_sampler_coordination_status()
