...
class SIMDMembraneProcessor:
    ...
    def _batch_update_impl(
        ...
        for i in range(0, len(batch_indices), chunk_size):
            ...
            for j in range(i, chunk_end):
                fired = self.vectorized_membrane_update(
                    batch_indices[j], batch_currents[j]
                )
        ...
        try:
            ...
            if len(active_sources) > 0:
                ...
                valid_sources = active_sources[
                    active_sources < sparse_input_matrix.shape[0]
                ]
    ...
    def _decay_only_impl(self) -> np.ndarray:
        ...
        can_update = self.active_mask[: self.capacity] & (
            self.refractory_counters[: self.capacity] <= 0
        )
...
def vectorized_membrane_update(
    ...
    if len(neuron_indices) > 0 and len(input_currents) > 0:
        ...
        potentials[neuron_indices[receiving_neurons]] += input_currents[
            receiving_neurons
        ]
