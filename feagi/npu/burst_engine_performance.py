...
class BurstEnginePerformanceMixin:
    ...
    def initialize_membrane_processor(self, capacity: int):
        ...
        if SIMD_AVAILABLE and self.membrane_processor is None:
            try:
                ...
                logger.info(f"[BRAIN] SIMD membrane processor initialized for {capacity} neurons")
            except Exception as e:
    ...
    def measure_actual_frequency(self, duration_seconds: float = 5.0, sample_count: int = 100) -> dict:
        ...
        try:
            ...
            measurement_result = {
                ...
                "actual_performance_ratio": actual_frequency_hz / desired_frequency if desired_frequency > 0 else 0.0,
                "potential_performance_ratio": potential_frequency_hz / desired_frequency if desired_frequency > 0 else 0.0,
            ...
            if debug_npu:
                logger.info(f"Frequency measurement complete - Actual: {actual_frequency_hz:.1f}Hz, Potential: {potential_frequency_hz:.1f}Hz (target: {desired_frequency:.1f}Hz)", emoji1="[STATS]")
    ...
    def burst(self, burst_size: Optional[int] = None, use_gpu: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            if SIMD_AVAILABLE and self.use_simd_profiling:
                with self.simd_profiler.profile_session(f"burst_{getattr(self, 'burst_count', 0)}"):
    ...
    def _execute_burst_impl(self, burst_size: Optional[int], use_gpu: bool, burst_start: float) -> Dict[str, Any]:
        ...
        if burst_size is None:
            if SIMD_AVAILABLE and hasattr(self, 'backend_selector') and self.backend_selector:
        ...
        if SIMD_AVAILABLE:
            ...
            if self.use_simd_profiling and hasattr(self, 'simd_profiler') and hasattr(self.simd_profiler, 'current_session'):
    ...
    def _get_fire_candidates_simd(self) -> List[int]:
        ...
        if hasattr(self, 'gna') and hasattr(self.gna, 'simd_optimized_find_fire_candidates'):
            return self.gna.simd_optimized_find_fire_candidates(getattr(self, 'burst_count', 0))
    ...
    def _report_performance(self):
        ...
        try:
            ...
            if self.use_simd_profiling and hasattr(self, 'simd_profiler') and hasattr(self.simd_profiler, 'sessions'):
                if self.simd_profiler.sessions:
                    report = self.simd_profiler.get_performance_report()
                    top_ops = report.get("top_operations", [])
                    if top_ops:
                        logger.info(f"[DEBUG] Top SIMD operation: {top_ops[0]['name']} "
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        metrics = {
            ...
            'simd_available': SIMD_AVAILABLE and self.simd_config.get('available', False),
        ...
        if self._processing_timing_buffer:
            ...
            metrics['avg_processing_time_ms'] = np.mean(self._processing_timing_buffer) * 1000
