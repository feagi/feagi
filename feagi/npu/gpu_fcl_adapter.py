...
from feagi.npu.fcl_manager import BitMap, BitMapProtocol, NeuronId, CorticalIdx, MembraneUpdate, FCLManager
...
class GPUBitMap:
    ...
    def copy(self) -> 'GPUBitMap':
        ...
        result = GPUBitMap()
        # Copy GPU data
        result.gpu_bitmap = self.backend.from_numpy(self.backend.to_numpy(self.gpu_bitmap))
        # Copy cache if valid
        if self._cache_valid:
