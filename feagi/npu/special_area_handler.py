...
class SpecialAreaHandler:
    ...
    def __init__(self, connectome_manager: Any, config: Optional[Dict[str, Any]] = None):
        ...
        logger.info("Special Area Handler initialized for core power area (cortical_idx=1)", status="[FAST]")
    ...
    def get_special_config(self, cortical_id: CorticalId) -> Optional[Any]:
        ...
        if cortical_id == "___pwr":
            ...
            logger.debug(f"No special config for area {cortical_id} (not a core power area)")
            return None
