...
class MemoryProcessor:
    ...
    def __init__(
        ...
        if hasattr(fcl_manager, "connectome_manager"):
            logger.info(
                f"🧠 [MEMORY-INIT] FCLManager has connectome_manager: {fcl_manager.connectome_manager is not None}"
            )
        else:
            logger.info(
        ...
        try:
            with self._processing_lock:
                ...
                if not self.active_memory_areas:
                    ...
                    self.stats.memory_neurons_died += lifecycle_result.get(
                        "neurons_died", 0
                    )
                    self.stats.memory_neurons_converted_to_longterm += (
                    ...
                    return {
                        ...
                        "stats": {
                            ...
                            "neurons_died": lifecycle_result.get(
                                "neurons_died", 0
                            ),
                            "neurons_converted": lifecycle_result.get(
                                "neurons_converted", 0
                            ),
                        },
                    }
                ...
                for i in range(0, len(memory_areas), self.batch_size):
                    ...
                    if mem_debug:
                        logger.info(
                            f"🧠 [MEMORY] Processing memory batch {i // self.batch_size + 1}: {batch}"
                        )

                    batch_result = self._process_memory_area_batch(
                        batch, current_burst
                    )
                ...
                if mem_debug:
                    logger.info(
                        "🧠 [MEMORY] Performing memory neuron aging and lifecycle management"
                    )
                lifecycle_result = self._perform_aging_and_lifecycle(
                    current_burst
                )
        ...
        for memory_area_id in memory_areas:
            try:
                area_results = self._process_single_memory_area(
                    memory_area_id, current_burst
                )
                batch_stats["patterns_processed"] += area_results[
                    "patterns_processed"
                ]
                batch_stats["neurons_created"] += area_results[
                    "neurons_created"
                ]
                batch_stats["neurons_reactivated"] += area_results[
                    "neurons_reactivated"
                ]
    ...
    def _process_single_memory_area(
        ...
        if temporal_pattern:
            ...
            if mem_debug:
                logger.info(
                    f"🧠 [MEMORY] Pattern detected for {memory_area_id}: creating/reactivating memory neuron"
                )
            ...
            if mem_debug:
                if existing_neuron_idx is not None:
                    logger.info(
                        f"🔍 [MEMORY] Pattern lookup result: FOUND existing neuron #{existing_neuron_idx}"
                    )
                else:
                    logger.info(
            ...
            if existing_neuron_idx is not None:
                ...
                if reactivated:
                    ...
                    if mem_debug:
                        logger.info(
                            f"🌟 [MEMORY] Pattern found: reactivated existing memory neuron {existing_neuron_idx}"
                        )
                        logger.info(
                ...
                try:
                    ...
                    if mem_debug:
                        logger.info(
                            "⭐ ⭐ ⭐ [MEMORY] 🆕 NEW MEMORY NEURON BORN! ⭐ ⭐ ⭐"
                        )
                        logger.info(
                            f"⭐ [MEMORY] Created memory neuron #{new_neuron_idx} for area {memory_area_id}"
                        )
                    ...
                    logger.error(
                        f"🚨 [MEMORY] Exception details: {type(e).__name__}: {str(e)}"
                    )
                    import traceback
        ...
        try:
            ...
            for timestep_offset in range(temporal_depth):
                ...
                if timestep < 0:
                    if mem_debug:
                        logger.info(
                            f"🧠 [MEMORY] Timestep {timestep} is negative - using empty pattern"
                        )
                    ...
                    try:
                        ...
                        if area_bitmap and len(area_bitmap) > 0:
                            ...
                            if mem_debug:
                                ...
                                more_indicator = (
                                    "..." if len(area_bitmap) > 10 else ""
                                )
                                logger.info(
                                    f"🔍 [MEMORY] Timestep {timestep}: area {upstream_area_id} has {neuron_count} firing neurons: {firing_neurons}{more_indicator}"
                                )
                                logger.info(
            ...
            if mem_debug:
                logger.info(
                    f"🧠 [MEMORY] PATTERN CREATED: {len(non_empty_patterns)}/{temporal_depth} non-empty timesteps, source_areas={pattern_key.source_cortical_areas}"
                )
                logger.info(
                    f"🔍 [MEMORY] Pattern details: temporal_depth={temporal_depth}, pattern_data_length={len(pattern_key.pattern_data)}"
                )
                # Show pattern fingerprint for debugging
                pattern_fingerprint = (
                    hash(pattern_key.pattern_data) % 10000
    ...
    def _update_state_manager_neuron_count(self, increment: int) -> None:
        ...
        try:
            ...
            reg = int(
                current_stats.get(
                    "non_memory_neuron_count", max(0, total - mem)
                )
            )
            ...
            logger.info(
                f"🔍 [MEMORY] Has is_err attr: {hasattr(result, 'is_err')}"
            )
            ...
            if hasattr(result, "is_ok") and hasattr(result, "is_err"):
                ...
                if result.is_err:
                    try:
                        ...
                        err = "unknown"
                    logger.error(
                        f"🚨 [MEMORY] FAILED to update StateManager counts: {err}"
                    )
                else:
                    logger.info(
                ...
                if not result:
                    logger.error(
                        "🚨 [MEMORY] FAILED to update StateManager neuron count: returned False"
                    )
                else:
                    logger.info(
                        f"⭐ [MEMORY] ✅ StateManager counts updated: total {total}→{total_new} (Δ{increment:+d}), "
                ...
                logger.error(
                    f"🚨 [MEMORY] StateManager returned unexpected type: {type(result)} = {result}"
                )
                logger.error(
    ...
    def _perform_aging_and_lifecycle(
        ...
        try:
            ...
            if neurons_died:
                ...
                self._update_state_manager_neuron_count(
                    increment=-neurons_died
                )
                if mem_debug:
                    try:
                ...
                logger.info(
                    "🧠 [MEMORY] No neurons died this burst; brain stats unchanged by aging"
                )
            ...
            for threshold in sorted(thresholds):
                converted = self.memory_neuron_array.check_longterm_conversion(
                    longterm_threshold=threshold
                )
                for idx in converted:
                    neurons_converted.add(idx)
            if mem_debug and neurons_converted:
                logger.info(
                    f"🧠 [MEMORY] Long-term conversions this burst: {len(neurons_converted)} (ids sample: {list(neurons_converted)[:10]})"
                )

        except Exception as e:
            logger.error(
                f"🧠 [MEMORY] Error in aging/lifecycle processing: {e}"
            )

        return {
            "neurons_died": neurons_died,
            "neurons_converted": len(neurons_converted),
        }
    ...
    def _get_cortical_idx_for_area(self, cortical_id: str) -> Optional[int]:
        ...
        try:
            if self.connectome_manager and hasattr(
                self.connectome_manager, "get_cortical_idx_for_id"
            ):
                return self.connectome_manager.get_cortical_idx_for_id(
                    cortical_id
                )
        except Exception as e:
            logger.error(
                f"🧠 [MEMORY] Error resolving cortical_idx for {cortical_id}: {e}"
            )
        return None
    ...
    def _debug_log_memory_area_snapshot(
        ...
        try:
            ...
            if n == 0:
                logger.info(
                    f"[MEMORY] Snapshot for {memory_area_id}: no neurons available"
                )
                return
            rows = []
            ...
            if total == 0:
                logger.info(
                    f"[MEMORY] Snapshot for {memory_area_id}: no neurons in area yet"
                )
                return
            # Apply limit
