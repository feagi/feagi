...
class BurstEngineDebugMixin:
        ...
        if self._fq_samplers:
            ...
            for i, sampler in enumerate(self._fq_samplers):
                sampler_info = {
                    ...
                    'performance_stats': getattr(sampler, 'get_performance_stats', lambda: {})()
                }
    ...
    def debug_burst_performance(self, burst_duration: float, processing_duration: float) -> None:
        """
        ...
        try:
            ...
            for i, sampler in enumerate(self._fq_samplers):
                if hasattr(sampler, 'max_neurons_per_sample'):
                    buffer_size_mb = sampler.max_neurons_per_sample * 6 * 4 / 1024 / 1024  # 6 floats * 4 bytes
    ...
    def debug_validate_state(self) -> Dict[str, bool]:
        ...
        """
        validation = {
            'fcl_manager_available': hasattr(self, 'fcl_manager') and self.fcl_manager is not None,
            'connectome_manager_available': hasattr(self, 'connectome_manager') and self.connectome_manager is not None,
            'state_manager_available': hasattr(self, 'state_manager') and self.state_manager is not None,
