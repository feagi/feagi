...
class ZMQDebugger:
    ...
    def enable_outbound(self, enabled: bool = True):
        ...
        with self._lock:
            ...
            logger.info(f"ZMQ outbound debugging {'enabled' if enabled else 'disabled'}")
    ...
    def set_message_filters(self, message_types: List[MessageType]):
        ...
        with self._lock:
            ...
            if message_types:
                logger.info(f"ZMQ debug filtering enabled for: {[t.value for t in message_types]}")
            else:
    ...
    def log_outbound(self, endpoint: str, data: Union[bytes, List[bytes]], 
    ...
    def log_inbound(self, endpoint: str, frames: List[bytes],
                   message_type: MessageType = MessageType.UNKNOWN,
                   context: str = ""):
        """
    ...
    def _log_message(self, direction: str, endpoint: str, frames: List[bytes],
                    message_type: MessageType, topic: str = "", context: str = ""):
...
def log_outbound(endpoint: str, data: Union[bytes, List[bytes]], 
...
def log_inbound(endpoint: str, frames: List[bytes],
               message_type: Union[MessageType, str] = MessageType.UNKNOWN,
               context: str = ""):
...
def log_pub_message(endpoint: str, topic: Union[str, bytes], data: bytes, context: str = ""):
    """Log a PUB/SUB outbound message."""
    topic_str = topic.decode('utf-8') if isinstance(topic, bytes) else topic
    log_outbound(endpoint, [topic.encode() if isinstance(topic, str) else topic, data], 
                MessageType.UNKNOWN, topic_str, context)
