...
class SystemResourceMonitor:
    ...
    def _capture_resource_snapshot(self) -> ResourceSnapshot:
        ...
        cpu_cores_used = (cpu_percent / 100.0) * cpu_core_count  # Convert percentage to cores
        ...
        try:
            ...
            process_cpu_cores_used = (process_cpu_percent / 100.0) * cpu_core_count  # Convert to cores
    ...
    def get_usage_summary(self, last_n_entries: int = 10) -> Dict[str, Any]:
        ...
        # Calculate averages
        avg_cpu = sum(entry.cpu_cores_used for entry in recent_entries) / len(recent_entries)
        avg_memory_mb = sum(entry.memory_mb for entry in recent_entries) / len(recent_entries)
        avg_process_cpu = sum(entry.process_cpu_cores_used for entry in recent_entries) / len(recent_entries)
        avg_process_memory_mb = sum(entry.process_memory_mb for entry in recent_entries) / len(recent_entries)
        ...
        peak_process_memory_mb = max(entry.process_memory_mb for entry in recent_entries)
        
        summary = {
            'entries_analyzed': len(recent_entries),
            'time_span_minutes': (recent_entries[-1].timestamp - recent_entries[0].timestamp).total_seconds() / 60,
            'system': {
                'cpu_avg': avg_cpu,
                'cpu_peak': peak_cpu,
        ...
        if recent_entries[0].gpu_usage:
            ...
            for gpu_idx in range(len(recent_entries[0].gpu_usage)):
                gpu_utils = [entry.gpu_usage[gpu_idx]['utilization'] for entry in recent_entries if len(entry.gpu_usage) > gpu_idx]
                if gpu_utils:
    ...
    def print_detailed_report(self) -> None:
        ...
        if summary:
            ...
            print(f"   FEAGI Memory: avg {summary['process']['memory_mb_avg']:.1f} MB, peak {summary['process']['memory_mb_peak']:.1f} MB")
            
            if 'gpu' in summary:
                print("   GPU Performance:")
                for gpu_summary in summary['gpu']:
                    print(f"      GPU {gpu_summary['index']}: avg {gpu_summary['utilization_avg']:.1f}%, peak {gpu_summary['utilization_peak']:.1f}%")
